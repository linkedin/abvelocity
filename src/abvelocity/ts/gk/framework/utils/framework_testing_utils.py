...
import numpy as np
import pandas as pd
from abvelocity.ts.common.constants import ACTUAL_COL, PREDICTED_COL, PREDICTION_BAND_COVERAGE, TIME_COL, VALUE_COL
from abvelocity.ts.common.evaluation import EvaluationMetricEnum, fraction_outside_tolerance
from abvelocity.ts.common.python_utils import assert_equal
from abvelocity.ts.common.testing_utils import assert_eval_function_equal
from abvelocity.ts.gk.framework.constants import FRACTION_OUTSIDE_TOLERANCE_NAME
from abvelocity.ts.gk.framework.output.univariate_forecast import UnivariateForecast
from abvelocity.ts.gk.framework.pipeline.pipeline import validate_pipeline_input
from abvelocity.ts.gk.framework.pipeline.utils import get_score_func_with_aggregation
...
from abvelocity.ts.gk.sklearn.sklearn_scorer import _PredictScorerDF
from sklearn.model_selection import RandomizedSearchCV
from sklearn.pipeline import Pipeline


def assert_refit(refit, expected_metric, expected_greater_is_better):
...
def assert_scoring(scoring, expected_keys=None, agg_periods=None, agg_func=None, relative_error_tolerance=None):
    ...
    for name, scorer in scoring.items():
        ...
        if name in name_func:
            expected_func = get_score_func_with_aggregation(score_func=name_func[name], agg_periods=agg_periods, agg_func=agg_func)[0]
...
def check_forecast_pipeline_result(
    ...
    Parameters
    ----------
    result : :class:`~blah.greykite.framework.pipeline.pipeline.ForecastResult`
...
def assert_basic_pipeline_equal(actual: Pipeline, expected: Pipeline):
    ...
    for key in check_keys:
        assert actual_params[key] == expected_params[key], f"{key} is different, found {actual_params[key]}, expected {expected_params[key]}"
...
def assert_forecast_pipeline_result_equal(actual, expected, is_silverkite=True, rel=1e-5):
    ...
    if actual.backtest is None:
        ...
        assert_equal(actual.backtest.train_evaluation, expected.backtest.train_evaluation, rel=rel)
        assert_equal(actual.backtest.test_evaluation, expected.backtest.test_evaluation, rel=rel)
    assert_equal(actual.forecast.train_evaluation, expected.forecast.train_evaluation, rel=rel)
    assert_equal(actual.forecast.test_evaluation, expected.forecast.test_evaluation, rel=rel)
    assert_equal(actual.grid_search.best_index_, expected.grid_search.best_index_, rel=rel)
    # checks CV search parameters
    assert_equal(actual.grid_search.cv.__dict__, expected.grid_search.cv.__dict__)
...
def mock_pipeline(
    ...
    """Creates and returns custom pipeline parameters"""
