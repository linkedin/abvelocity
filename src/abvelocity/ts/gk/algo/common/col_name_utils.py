...
def simplify_event(name):
    ...
    if cst.EVENT_PREFIX in name:
        result = re.search(rf"\('({cst.EVENT_PREFIX}.*)'\),", name)
...
def simplify_pred_cols(pred_cols):
    ...
    new_pred_cols : `list` [ `str` ]
        ...
        `~blah.greykite.algo.common.col_name_utils.simplify_name`
    """
...
def add_category_cols(coef_summary, pred_category):
    ...
    coef_summary : `pandas.DataFrame`
        ...
        `~blah.greykite.algo.common.model_summary_utils.add_model_coef_df`
        or
        `~blah.greykite.algo.common.model_summary_utils.add_model_coef_df`.
    pred_category : `dict`
        The predictor category dictionary by
        `~blah.greykite.algo.common.col_name_utils.create_pred_category`.
    ...
    coef_summary_with_new_columns : `pandas.DataFrame
        New df with the following columns added:
            ...
            "is_time_feature" : 0 or 1
                ...
                Time features belong to `~blah.greykite.common.constants.TimeFeaturesEnum`.
            "is_event" : 0 or 1
                Event features or not.
                Event features have `~blah.greykite.common.constants.EVENT_PREFIX`.
            "is_trend" : 0 or 1
                Trend features or not.
                Trend features have `~blah.greykite.common.constants.CHANGEPOINT_COL_PREFIX` or "cp\\d".
            "is_seasonality" : 0 or 1
                Seasonality feature or not.
                Seasonality features have `~blah.greykite.common.constants.SEASONALITY_REGEX`.
            "is_lag" : 0 or 1
    ...
    coef_summary["is_intercept"] = [1 if col in pred_category["intercept"] else 0 for col in pred_cols]
    coef_summary["is_time_feature"] = [1 if col in pred_category["time_features"] else 0 for col in pred_cols]
    coef_summary["is_event"] = [1 if col in pred_category["event_features"] else 0 for col in pred_cols]
    coef_summary["is_trend"] = [1 if col in pred_category["trend_features"] else 0 for col in pred_cols]
    coef_summary["is_seasonality"] = [1 if col in pred_category["seasonality_features"] else 0 for col in pred_cols]
    coef_summary["is_lag"] = [1 if col in pred_category["lag_features"] else 0 for col in pred_cols]
    coef_summary["is_regressor"] = [1 if col in pred_category["regressor_features"] else 0 for col in pred_cols]
    coef_summary["is_interaction"] = [1 if col in pred_category["interaction_features"] else 0 for col in pred_cols]
    return coef_summary
...
def create_pred_category(pred_cols, extra_pred_cols, df_cols):
    ...
    regressor_cols = [col for col in df_cols if col in [c for term in extra_pred_cols for c in term.split(":")]]
    pred_category = {
        ...
        "time_features": [
            ...
            if (
                (
                    re.search(":", col) is None
                    and re.search("|".join(cst.TimeFeaturesEnum.__dict__["_member_names_"]), col)
                    and re.search(cst.SEASONALITY_REGEX, col) is None
                )
                or (
                    re.search(":", col)
                    and any(
                        [
                            re.search("|".join(cst.TimeFeaturesEnum.__dict__["_member_names_"]), subcol) and re.search(cst.SEASONALITY_REGEX, subcol) is None
        ...
        "trend_features": [
            ...
            for col in pred_cols
            if (
                (re.search(":", col) is None and re.search(cst.TREND_REGEX, col) and re.search(cst.SEASONALITY_REGEX, col) is None)
                or (
                    re.search(":", col)
                    and any([re.search(cst.TREND_REGEX, subcol) and re.search(cst.SEASONALITY_REGEX, subcol) is None for subcol in col.split(":")])
                )
            )
            and col not in regressor_cols
        ],
        ...
        "regressor_features": [col for col in pred_cols if col in regressor_cols or any([x in regressor_cols for x in col.split(":")])],
...
def filter_coef_summary(
    ...
    is_trend : `bool` or `None`, default `None`
        Trend features or not.
        Trend features have `~blah.greykite.common.constants.CHANGEPOINT_COL_PREFIX`.
