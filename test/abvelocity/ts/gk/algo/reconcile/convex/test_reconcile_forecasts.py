...
import pandas as pd
import pytest
from abvelocity.ts.gk.algo.reconcile.convex.reconcile_forecasts import (
...
from abvelocity.ts.gk.algo.reconcile.hierarchical_relationship import HierarchicalRelationship
from abvelocity.ts.common.constants import TIME_COL
from abvelocity.ts.common.data_loader import DataLoader
from abvelocity.ts.common.evaluation import mean_absolute_percent_error, median_absolute_percent_error, root_mean_squared_error
from abvelocity.ts.common.python_utils import assert_equal, reorder_columns
...
def data():
    ...
    order_dict = {"root": 0, "c1": 1, "c2": 2, "c3": 2}  # hierarchy order
    return {
        "forecasts": forecasts,
        "actuals": actuals,
        "tree": tree,
...
def test_get_weight_matrix():
    ...
    # weights is None
    wmat = get_weight_matrix(weights=None, n_forecasts=3, name="weight_bias", default_weights={})
    ...
    wmat = get_weight_matrix(weights="custom", n_forecasts=3, name="weight_bias", default_weights=default_weights)
    ...
    # weights is a list
    wmat = get_weight_matrix(weights=[1, 2, 3], n_forecasts=3, name="weight_bias", default_weights={})
    expected_wmat = np.diag([1, 2, 3]) * target_norm / np.linalg.norm(np.diag([1, 2, 3]))
    assert_equal(expected_wmat, wmat)

    # weights is an array
    wmat = get_weight_matrix(weights=np.array([1, 2, 3]), n_forecasts=3, name="weight_bias", default_weights={})
    assert_equal(expected_wmat, wmat)

    # exception
    with pytest.raises(ValueError, match="Expected square matrix with size 10, but `weight_bias` " "has weight matrix with shape \\(3, 3\\)"):
        get_weight_matrix(weights=np.array([1, 2, 3]), n_forecasts=10, name="weight_bias", default_weights={})  # doesn't match len(weights)

    # weights is string, no default
    with pytest.raises(ValueError, match="The requested weight 'dummy' for `weight_bias` is not found. " r"Must be one of \['custom'\]"):
        wmat = get_weight_matrix(weights="dummy", n_forecasts=3, name="weight_bias", default_weights={"custom": np.ones((3, 3))})
        assert_equal(np.eye(3), wmat)
...
def test_get_fit_params():
    ...
    with pytest.raises(ValueError, match="`method` 'unknown' is not recognized. " "Must be one of 'bottom_up', 'ols', 'mint_sample', 'custom'"):
...
def test_apply_method_defaults():
    ...
    def dummy_fit_func(self, forecasts, actuals, **params):
        return {
            "self": self,
            "forecasts": forecasts,
            "actuals": actuals,
    ...
    result = dummy_fit_func(self="self", forecasts="forecasts", actuals="actuals", method="mint_sample")
    assert_equal(
        result,
        {
            "self": "self",
    ...
    params = {"method": "mint_sample", "covariance": "identity", "verbose": True}
    result = new_fit_func(self="self", forecasts="forecasts", actuals="actuals", **params)
    ...
    # `method` == "custom"
    new_fit_func = apply_method_defaults(dummy_fit_func)
    params = {"method": "custom", "covariance": "identity", "verbose": True}
    result = new_fit_func(self="self", forecasts="forecasts", actuals="actuals", **params)
    assert_equal(
        result,
        {
...
def test_trace_info(data):
    ...
    trace = TraceInfo(df=data["forecasts"], color="blue", name="forecast", legendgroup=None)
...
def test_evaluation_plot(data):
    ...
    with pytest.raises(ValueError, match="Column names must be identical in all traces."):
        f = forecasts.copy()
        f.columns = list("abcd")
        traces = [TraceInfo(df=f), TraceInfo(df=actuals)]
        evaluation_plot(x=x, traces=traces)

    traces = [TraceInfo(df=forecasts), TraceInfo(df=actuals, color="blue", name="actuals", legendgroup="a")]  # uses default (None) for other attributes
    ...
    with pytest.raises(ValueError) as record:
        ...
        assert "`num_cols` should be between 1 and 4 (the number of columns), found 0." in record[0].message.args[0]

    with pytest.raises(ValueError) as record:
        evaluation_plot(x=x, traces=traces, num_cols=num_timeseries + 1)
        assert "`num_cols` should be between 1 and 4 (the number of columns), found 5." in record[0].message.args[0]
...
def test_raf_form_constraints():
    ...
    constraints = raf._form_constraints(transform_variable=transform_variable, Ya=None)
    ...
    raf.lower_bound = None
    raf.upper_bound = None
    constraints = raf._form_constraints(transform_variable=transform_variable, Ya=Ya)
    ...
    with pytest.warns(UserWarning, match="Actuals do not satisfy the constraints!"):
        Ya[0] += 1.0
        raf._form_constraints(transform_variable=transform_variable, Ya=Ya)

    with pytest.raises(ValueError, match="`Ya` must be provided if `unbiased` and `tree` is None."):
        raf.tree = None
        raf._form_constraints(transform_variable=transform_variable, Ya=None)
...
def test_raf_form_objective():
    ...
    obj = raf._form_objective(Yf=Yf, Ya=Ya, transform_variable=transform_variable, covariance=None)
    assert_equal(
        raf.objective_weights,
        {"weight_adj": np.eye(m), "weight_bias": weight_inverr, "weight_train": weight_err, "weight_var": weight_inverr, "covariance": None},
    )
    ...
    obj = raf._form_objective(Yf=Yf, Ya=Ya, transform_variable=transform_variable, covariance="identity")
    assert_equal(
        raf.objective_weights, {"weight_adj": weight_err, "weight_bias": np.eye(m), "weight_train": np.eye(m), "weight_var": np.eye(m), "covariance": np.eye(m)}
    )
    assert raf.objective_fn(transform_matrix) == {"adj": 0.0, "bias": 0.0, "train": 0.0, "var": 0.0, "total": 0.0}  # all the self.lam_* are 0.0
    ...
    raf.lam_train = 3.3
    raf.lam_var = 4.4
    obj = raf._form_objective(Yf=Yf, Ya=Ya, transform_variable=transform_variable, covariance="sample")
    assert obj.is_dcp()
    ...
    assert_equal(
        raf.objective_weights, {"weight_adj": np.eye(m), "weight_bias": np.eye(m), "weight_train": np.eye(m), "weight_var": np.eye(m), "covariance": cov}
    )
    ...
    assert_equal(
        raf.objective_fn(transform_matrix),
        {
            "adj": expected_adj,
            "bias": expected_bias,
    ...
    assert_equal(
        raf.objective_fn(transform_matrix, forecast_matrix=Yf * 2, actual_matrix=Ya * 2),
        {
            "adj": 4 * expected_adj,
    ...
    constraints = [transform_variable >= -1.0, transform_variable <= 1.0, tree.constraint_matrix @ transform_variable == 0]
    prob = cp.Problem(obj, constraints=constraints)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=UserWarning)  # sometimes there is a warning about solver accuracy
        prob.solve(tol_gap_abs=1e-5, tol_gap_rel=1e-8, tol_feas=1e-5, verbose=True)
    ...
    with pytest.raises(ValueError, match="`covariance` not recognized. Provide a valid string in \\['identity', 'sample'\\] or a matrix."):
        raf = ReconcileAdditiveForecasts()
        raf._form_objective(Yf=Yf, Ya=Ya, transform_variable=transform_variable, covariance="unknown-covariance")
...
def test_raf_fit(data):
    ...
    with pytest.raises(ValueError, match="Either `constraint_matrix` or `levels` must be provided."):
        raf.fit(forecasts=forecasts, actuals=actuals, method="bottom_up")

    with pytest.raises(ValueError, match="Must provide `levels` if `method='bottom_up'`."):
        raf.fit(forecasts=forecasts, actuals=actuals, constraint_matrix=tree.constraint_matrix, method="bottom_up")

    with pytest.raises(ValueError, match="Only one of `constraint_matrix` and `levels` can be provided."):
        raf.fit(forecasts=forecasts, actuals=actuals, levels=levels, constraint_matrix=tree.constraint_matrix, method="bottom_up")

    with pytest.raises(ValueError, match="`lower_bound` 1.5 should not be greater than `upper_bound` 1.0"):
        raf.fit(forecasts=forecasts, actuals=actuals, levels=levels, method="bottom_up", lower_bound=1.5, upper_bound=1.0)

    with pytest.raises(ValueError, match="Forecast shape \\(20, 4\\) does not match actuals shape \\(150, 4\\)"):
        raf.fit(forecasts=forecasts[:20], actuals=actuals, levels=levels, method="bottom_up")
    ...
    with pytest.raises(
        ...
        raf.fit(forecasts=forecasts, actuals=actuals, constraint_matrix=np.eye(3))
    ...
    with pytest.raises(
        ...
        raf.fit(forecasts=forecasts, actuals=actuals, levels=[[2], [3, 2]])

    # Tests order_dict, scaling, objective function, method
    raf = ReconcileAdditiveForecasts()
    raf.fit(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, method="mint_sample", lower_bound=-1.5, upper_bound=1.5)
    ...
    with pytest.warns(UserWarning, match="Variance of residuals is underestimated if the estimator is biased."):
        raf = ReconcileAdditiveForecasts()
        raf.fit(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, unbiased=False, covariance="sample", lam_adj=3.0, lam_var=2.0)
        ...
        assert set(raf.objective_fn_val.keys()) == {"adj", "bias", "train", "var", "total"}
        assert all(v >= 0 for v in raf.objective_fn_val.values())
        assert pytest.approx(raf.objective_fn_val["total"]) == (
            raf.objective_fn_val["adj"] + raf.objective_fn_val["bias"] + raf.objective_fn_val["train"] + raf.objective_fn_val["var"]
        )
    ...
    raf.fit(
        ...
        tol_gap_rel=1e-7,  # solver params
        tol_gap_abs=1e-7,
        tol_feas=1e-7,
    )
    ...
    raf = ReconcileAdditiveForecasts()
    raf.fit(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, method="bottom_up")
    ...
    with pytest.warns(UserWarning, match="Failed to find a solution. Falling back to bottom-up method"):
        raf = ReconcileAdditiveForecasts()
        raf.fit(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, lower_bound=3.0, upper_bound=3.0)
        assert_equal(raf.transform_matrix, raf.tree.bottom_up_transform)
    ...
    with pytest.warns(
        UserWarning, match="Failed to find a solution. Try setting CVXPY solver parameters, changing the " "constraints, or changing the objective weights"
    ):
        raf = ReconcileAdditiveForecasts()
        raf.fit(forecasts=forecasts, actuals=actuals, constraint_matrix=constraint_matrix, order_dict=order_dict, lower_bound=3.0, upper_bound=3.0)
...
def test_raf_transform(data):
    ...
    raf.fit(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, method="mint_sample")
    ...
    raf2.fit_transform(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, method="ols")
...
def test_raf_evaluate(data):
    ...
    order_dict = data["order_dict"]
    raf = ReconcileAdditiveForecasts()
    raf.fit_transform(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, method="mint_sample")
    ...
    assert_equal(raf.evaluation_df["RMSE % change"], 100 * (raf.evaluation_df["Adjusted RMSE"] / raf.evaluation_df["Base RMSE"] - 1.0), check_names=False)
    assert_equal(raf.evaluation_df["MAPE pp change"], raf.evaluation_df["Adjusted MAPE"] - raf.evaluation_df["Base MAPE"], check_names=False)
    assert_equal(raf.evaluation_df["MedAPE pp change"], raf.evaluation_df["Adjusted MedAPE"] - raf.evaluation_df["Base MedAPE"], check_names=False)
    ...
    assert_equal(mean_absolute_percent_error(raf.actuals["root"], raf.adjusted_forecasts["root"]), raf.evaluation_df.loc["root", "Adjusted MAPE"])
    assert_equal(mean_absolute_percent_error(raf.actuals["c1"], raf.forecasts["c1"]), raf.evaluation_df.loc["c1", "Base MAPE"])
    assert_equal(median_absolute_percent_error(raf.actuals["c2"], raf.adjusted_forecasts["c2"]), raf.evaluation_df.loc["c2", "Adjusted MedAPE"])
    assert_equal(median_absolute_percent_error(raf.actuals["c3"], raf.forecasts["c3"]), raf.evaluation_df.loc["c3", "Base MedAPE"])
    assert_equal(root_mean_squared_error(raf.actuals["root"], raf.adjusted_forecasts["root"]), raf.evaluation_df.loc["root", "Adjusted RMSE"])
    assert_equal(root_mean_squared_error(raf.actuals["c1"], raf.forecasts["c1"]), raf.evaluation_df.loc["c1", "Base RMSE"])
    ...
    assert_equal(root_mean_squared_error(raf.actuals_test["c1"], raf.forecasts_test["c1"]), raf.evaluation_df_test.loc["c1", "Base RMSE"])

    # Tests `fit_transform_evaluate`
    # (and that plot/ipython_display run without error)
    raf = ReconcileAdditiveForecasts()
    raf.fit_transform(forecasts=forecasts, actuals=actuals, levels=levels, order_dict=order_dict, method="bottom_up")
    raf.evaluate(is_train=True)
    ...
    raf2.fit_transform_evaluate(
        forecasts=forecasts,
        actuals=actuals,
        fit_kwargs=dict(levels=levels, order_dict=order_dict, method="bottom_up"),
        evaluate_kwargs=dict(ipython_display=True, plot=False, plot_num_cols=2),
    )
    ...
    with np.errstate(divide="ignore"):
        raf2.transform_evaluate(forecasts_test=forecasts_test, actuals_test=actuals_test, plot=False, plot_num_cols=2, ipython_display=True)
        assert len(raf2.figures_test) == 3
        assert raf2.figures != raf2.figures_test

    assert_equal(root_mean_squared_error(raf2.actuals_test["c1"], raf2.forecasts_test["c1"]), raf2.evaluation_df_test.loc["c1", "Base RMSE"])
...
def test_raf_ols_mint():
    ...
    raf = ReconcileAdditiveForecasts()
    raf.fit_transform(forecasts=forecasts, actuals=actuals, levels=levels, method="ols")
    assert_equal(raf.adjusted_forecasts, expected_ols, rel=1e-4)
    raf.fit_transform(forecasts=forecasts, actuals=actuals, levels=levels, method="mint_sample")
