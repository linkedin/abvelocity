...
def test_get_auto_seasonality_daily(df_daily):
    ...
    seasonality = get_auto_seasonality(df=df_daily, time_col=TIME_COL, value_col=VALUE_COL)
...
def test_get_auto_seasonality_hourly(df_hourly):
    """Tests get automatic seasonality."""
    seasonality = get_auto_seasonality(df=df_hourly, time_col=TIME_COL, value_col=VALUE_COL)
...
def test_get_auto_seasonality_override(df_daily):
    ...
    seasonality = get_auto_seasonality(df=df_daily, time_col=TIME_COL, value_col=VALUE_COL, yearly_seasonality=False)
...
def test_get_auto_holiday(df_daily):
    ...
    pre_num = 2
    post_num = 2
    pre_post_num_dict = {"New Years": (1, 3)}
    holiday_lookup_countries = ["US"]

    # Constructs `daily_event_df_dict` through directly calling `HolidayGrouper`.
    # Constructs `holiday_df`.
    holiday_df_dict = get_holidays(countries=holiday_lookup_countries, year_start=start_year - 1, year_end=end_year + 1)
    ...
    holiday_df = holiday_df[~holiday_df[EVENT_DF_LABEL_COL].str.endswith("_observed")]
    ...
    daily_event_df_dict_from_input_holiday_df = get_auto_holidays(
        ...
        holidays_to_model_separately=None,
        daily_event_df_dict=None,
        auto_holiday_params=dict(holiday_df=holiday_df),
    )
...
def test_get_auto_holiday_with_holidays_to_model_separately(df_daily):
    ...
    auto_holiday_params = dict(min_abs_avg_score=0, min_same_sign_ratio=0, get_suffix_func=None, min_n_days=1)
    ...
    # Constructs `daily_event_df_dict` through `get_auto_holidays`, here we model "New Years"
    ...
    # Checks that "New Years" and its neighboring days have its own groups in `daily_event_df_dict_with_separate`.
    # The number of neighboring days depend on `pre_post_num_dict`.
    assert "New Years" in daily_event_df_dict_with_separate.keys()
    assert "New Years_minus_1" in daily_event_df_dict_with_separate.keys()
    assert "New Years_minus_2" not in daily_event_df_dict_with_separate.keys()
    assert "New Years_plus_3" in daily_event_df_dict_with_separate.keys()
...
def test_get_auto_holiday_non_daily_df(df_hourly):
    ...
    df_daily_reconstructed = df_tmp.drop(columns=TIME_COL).reset_index() if TIME_COL in df_tmp.columns else df_tmp.reset_index()
    ...
    df_weekly_reconstructed = df_tmp.drop(columns=TIME_COL).reset_index() if TIME_COL in df_tmp.columns else df_tmp.reset_index()
...
def test_get_auto_growth_daily(df_daily):
    ...
    growth = get_auto_growth(
        ...
        value_col=VALUE_COL,
        forecast_horizon=7,
        changepoints_dict_override=dict(method="auto", dates=["2010-01-01"], combine_changepoint_min_distance="D", keep_detected=True),
    )
