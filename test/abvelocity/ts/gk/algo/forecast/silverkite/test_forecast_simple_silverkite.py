...
from abvelocity.ts.gk.algo.forecast.silverkite.constants.silverkite_column import SilverkiteColumn
from abvelocity.ts.gk.algo.forecast.silverkite.constants.silverkite_holiday import SilverkiteHoliday
from abvelocity.ts.gk.algo.forecast.silverkite.constants.silverkite_seasonality import SilverkiteSeasonality, SilverkiteSeasonalityEnum
from abvelocity.ts.gk.algo.forecast.silverkite.forecast_simple_silverkite import SimpleSilverkiteForecast
from abvelocity.ts.gk.algo.forecast.silverkite.forecast_simple_silverkite_helper import generate_holiday_events
from abvelocity.ts.common.constants import CHANGEPOINT_COL_PREFIX, EVENT_DF_DATE_COL, EVENT_DF_LABEL_COL, LOGGER_NAME, TIME_COL, VALUE_COL
...
from abvelocity.ts.common.features.timeseries_features import get_available_holidays_across_countries, get_holidays
from abvelocity.ts.common.python_utils import assert_equal, update_dictionary
...
def daily_data_reg():
    ...
    """
    df = generate_df_with_reg_for_tests(freq="D", periods=50, train_start_date=datetime.datetime(2018, 11, 30), remove_extra_cols=False)["df"]
    ...
    df = df.rename({cst.TIME_COL: "custom_time_col", cst.VALUE_COL: "custom_value_col"}, axis=1)
...
def weekly_data():
    ...
    return generate_df_for_tests(freq="W", periods=20, train_start_date=datetime.datetime(2018, 11, 30))["df"]


@pytest.fixture
def hourly_data():
    """Generates 500 days of hourly data for tests"""
    return generate_df_for_tests(freq="H", periods=24 * 500, train_start_date=datetime.datetime(2018, 7, 1))["df"]
...
def test_origin_for_time_vars(hourly_data):
    ...
    parameters = silverkite.convert_params(
        df=hourly_data, time_col=cst.TIME_COL, value_col=cst.VALUE_COL, origin_for_time_vars=2017.12, time_properties=time_properties
    )
    assert parameters["origin_for_time_vars"] == 2017.12

    # then default to ``time_properties``
    parameters = silverkite.convert_params(df=hourly_data, time_col=cst.TIME_COL, value_col=cst.VALUE_COL, time_properties=time_properties)
    assert parameters["origin_for_time_vars"] == 2018.12

    # then default to the first point in ``df``
    parameters = silverkite.convert_params(df=hourly_data, time_col=cst.TIME_COL, value_col=cst.VALUE_COL)
...
def test_daily_event_df_dict(hourly_data):
    ...
    holiday_pre_post_num_dict = {"New Years": (7, 3), "Christmas": (3, 3)}
    ...
    event_df_dict = get_holidays(countries, year_start=2015, year_end=2030)
    parameters = silverkite.convert_params(
        df=hourly_data,
        time_col=cst.TIME_COL,
        value_col=cst.VALUE_COL,
...
def test_fs_components_df(hourly_data):
    ...
    expected = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=time_properties["simple_freq"].name, num_days=time_properties["num_training_days"], seasonality=seasonality
    )
...
def test_passthrough(daily_data_reg):
    ...
    passthrough_params = dict(
        ...
        changepoints_dict={"method": "uniform", "n_changepoints": 20},
...
def test_extra_pred_cols(hourly_data, daily_data_reg, weekly_data):
    ...
    example_holiday_cols = [
        "C(Q('events_Lunar New Year'), levels=['', 'event'])",
        "C(Q('events_Lunar New Year_minus_1'), levels=['', 'event'])",
        "C(Q('events_Lunar New Year_minus_2'), levels=['', 'event'])",
        "C(Q('events_Lunar New Year_plus_1'), levels=['', 'event'])",
        "C(Q('events_Lunar New Year_plus_2'), levels=['', 'event'])",
        "C(Q('events_Independence'), levels=['', 'event'])",
    ]
    excluded_holiday_cols = [  # should not appear
        "C(Q('events_Lunar New Year_minus_3'), levels=['', 'event'])" "C(Q('events_Independence_plus_3'), levels=['', 'event'])"
    ]
    ...
    expected_cols = set([cst.GrowthColEnum[growth_term].value] + example_holiday_cols + feature_set_cols + extra_pred_cols)
    ...
    event_df_dict = get_holidays(["UnitedStates", "China"], year_start=2015, year_end=2030)
    more_example_holiday_cols = []  # country-level event col levels order is non-deterministic; checked via startswith below
    ...
    for feature_set_name, use_set in feature_sets_used.items():
        if use_set:
            ...
            excluded_cols += example_cols[feature_set_name]
    expected_cols = set(
        [cst.GrowthColEnum[growth_term].value] + regressor_cols + example_holiday_cols + more_example_holiday_cols + feature_set_cols + extra_pred_cols
    )
    assert len(result_cols) == 65  # desired length
    assert len(result_cols) == len(set(result_cols))  # no duplicates
    assert set(expected_cols).issubset(set(result_cols))  # contains expected cols
    assert len(set(excluded_cols).intersection(set(result_cols))) == 0  # does not contain excluded cols
    assert any(col.startswith("C(Q('events_China')") for col in result_cols)  # China events column present
    assert any(col.startswith("C(Q('events_UnitedStates')") for col in result_cols)  # US events column present
...
def test_convert_simple_silverkite_params_hourly(hourly_data):
    ...
    assert {
        ...
        "C(Q('events_Christmas'), levels=['', 'event']):sin4_tod_daily",
        "ct1:cos4_tow_weekly",
...
def test_forecast_simple_silverkite_freq():
    ...
    test_settings = [
        ...
        ("H", 60 * 24, 100 * 24),  # long-range training to make sure all features are present
    ]
    with warnings.catch_warnings():
        ...
        for freq, periods, forecast_horizon in test_settings:
            ...
            # trains a model
            silverkite = SimpleSilverkiteForecast()
            trained_model = silverkite.forecast_simple(df=data, time_col=cst.TIME_COL, value_col=cst.VALUE_COL, forecast_horizon=forecast_horizon)

            # checks the parameters
            parameters = silverkite.convert_params(df=data, time_col=cst.TIME_COL, value_col=cst.VALUE_COL, forecast_horizon=forecast_horizon)
            keys_to_check = ["time_col", "value_col", "origin_for_time_vars", "uncertainty_dict"]
            ...
            result = silverkite.predict_n_no_sim(fut_time_num=forecast_horizon, trained_model=trained_model, freq=freq)["fut_df"]
...
def test_forecast_simple_silverkite_changepoints():
    ...
    trained_model = silverkite.forecast_simple(df=df, time_col="ts", value_col="y", changepoints_dict=None)
    changepoint_values = trained_model["changepoint_values"]
...
def test_forecast_simple_silverkite_seasonality_changepoints():
    ...
    trained_model = silverkite.forecast_simple(
        ...
        time_col="ts",
        value_col="y",
        changepoints_dict={"method": "auto", "no_changepoint_proportion_from_end": no_change_prop},
        seasonality_changepoints_dict={"no_changepoint_proportion_from_end": no_change_prop, "regularization_strength": 0.4},
    )
...
def test_get_requested_seasonality_order():
    ...
    silverkite = SimpleSilverkiteForecast()
    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality="auto", default_order=5, is_enabled_auto=True) == 5
    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality="auto", default_order=5, is_enabled_auto=False) == 0

    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=True, default_order=6, is_enabled_auto=True) == 6
    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=True, default_order=6, is_enabled_auto=False) == 6

    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=False, default_order=5, is_enabled_auto=True) == 0
    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=False, default_order=5, is_enabled_auto=False) == 0

    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=None, default_order=5, is_enabled_auto=True) == 0
    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=None, default_order=5, is_enabled_auto=False) == 0

    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=1, default_order=5, is_enabled_auto=True) == 1
    assert silverkite._SimpleSilverkiteForecast__get_requested_seasonality_order(requested_seasonality=2, default_order=5, is_enabled_auto=False) == 2
...
def test_get_silverkite_seasonality():
    ...
    silverkite = SimpleSilverkiteForecast()
    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(simple_freq=SimpleTimeFrequencyEnum.DAY.name, num_days=10)
    assert fs is None

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(simple_freq=SimpleTimeFrequencyEnum.DAY.name, num_days=100)
    cols = ["name", "period", "order", "seas_names"]
    expected = pd.DataFrame([["tow", 7.0, 4, "weekly"]], columns=cols)
    assert fs.equals(expected)

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(simple_freq=SimpleTimeFrequencyEnum.DAY.name, num_days=365)
    expected = pd.DataFrame([["tow", 7.0, 4, "weekly"], ["toq", 1.0, 5, "quarterly"]], columns=cols)
    assert fs.equals(expected)

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=SimpleTimeFrequencyEnum.DAY.name, num_days=SilverkiteSeasonalityEnum.YEARLY_SEASONALITY.value.default_min_days
    )
    expected = pd.DataFrame([["tow", 7.0, 4, "weekly"], ["toq", 1.0, 5, "quarterly"], ["ct1", 1.0, 15, "yearly"]], columns=cols)
    assert fs.equals(expected)
    ...
    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=SilverkiteSeasonalityEnum.YEARLY_SEASONALITY.value.default_min_days
    )
    expected = pd.DataFrame([["tod", 24.0, 12, "daily"], ["tow", 7.0, 4, "weekly"], ["toq", 1.0, 5, "quarterly"], ["ct1", 1.0, 15, "yearly"]], columns=cols)
    assert fs.equals(expected)
    ...
    expected = pd.DataFrame([["tod", 24.0, 12, "daily"], ["tow", 7.0, 6, "weekly"], ["ct1", 1.0, 15, "yearly"]], columns=cols)
    assert fs.equals(expected)

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=SimpleTimeFrequencyEnum.WEEK.name, num_days=TimeEnum.ONE_YEAR_IN_DAYS.value
    )
    expected = pd.DataFrame([["tom", 1.0, 2, "monthly"], ["toq", 1.0, 5, "quarterly"]], columns=cols)
    assert fs.equals(expected)

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=SimpleTimeFrequencyEnum.MONTH.name, num_days=TimeEnum.ONE_YEAR_IN_DAYS.value
    )
    assert fs is None

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=SimpleTimeFrequencyEnum.QUARTER.name, num_days=TimeEnum.ONE_YEAR_IN_DAYS.value
    )
    assert fs is None

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=SimpleTimeFrequencyEnum.YEAR.name, num_days=TimeEnum.ONE_YEAR_IN_DAYS.value
    )
    assert fs is None

    fs = silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
        simple_freq=SimpleTimeFrequencyEnum.MULTIYEAR.name, num_days=TimeEnum.ONE_YEAR_IN_DAYS.value
    )
    assert fs is None

    with pytest.raises(
        ValueError,
        match=r"unknown must be one of dict_keys\(\['daily_seasonality',"
        r" 'weekly_seasonality', 'monthly_seasonality', 'quarterly_seasonality',"
        r" 'yearly_seasonality'\]\)",
    ):
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        silverkite._SimpleSilverkiteForecast__get_silverkite_seasonality(
            simple_freq=SimpleTimeFrequencyEnum.MONTH.name, num_days=TimeEnum.ONE_YEAR_IN_DAYS.value, seasonality={"weekly_seasonality": 3}
        )
        log_capture.check(
            (
                LOGGER_NAME,
...
def test_get_seasonality_order_from_dataframe():
    ...
    assert (
        silverkite._SimpleSilverkiteForecast__get_seasonality_order_from_dataframe(seasonality=SilverkiteSeasonalityEnum.WEEKLY_SEASONALITY.value, fs=fs)
        == SilverkiteSeasonalityEnum.WEEKLY_SEASONALITY.value.order
    )
    assert (
        silverkite._SimpleSilverkiteForecast__get_seasonality_order_from_dataframe(seasonality=SilverkiteSeasonalityEnum.QUARTERLY_SEASONALITY.value, fs=fs)
        == SilverkiteSeasonalityEnum.QUARTERLY_SEASONALITY.value.order
    )
    assert (
        silverkite._SimpleSilverkiteForecast__get_seasonality_order_from_dataframe(seasonality=SilverkiteSeasonalityEnum.YEARLY_SEASONALITY.value, fs=fs)
        == SilverkiteSeasonalityEnum.YEARLY_SEASONALITY.value.order
    )
    # max_order
    assert (
        silverkite._SimpleSilverkiteForecast__get_seasonality_order_from_dataframe(
            seasonality=SilverkiteSeasonalityEnum.WEEKLY_SEASONALITY.value, fs=fs, max_order=1
        )
        == 1
    )

    # returns 0 if seasonality is not found in the dataframe
    assert silverkite._SimpleSilverkiteForecast__get_seasonality_order_from_dataframe(seasonality=SilverkiteSeasonalityEnum.DAILY_SEASONALITY.value, fs=fs) == 0

    seas = SilverkiteSeasonality(name="ct1", period=1.0, order=2, seas_names="another_one", default_min_days=10)
    assert silverkite._SimpleSilverkiteForecast__get_seasonality_order_from_dataframe(seasonality=seas, fs=fs) == 0

    # returns 0 if there is no dataframe
    assert (
        silverkite._SimpleSilverkiteForecast__get_seasonality_order_from_dataframe(seasonality=SilverkiteSeasonalityEnum.WEEKLY_SEASONALITY.value, fs=None) == 0
    )
...
def test_get_feature_sets_enabled():
    ...
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.DAY.name, num_days=1000, feature_sets_enabled="auto"
    )
    expected = {
        SilverkiteColumn.COLS_HOUR_OF_WEEK: False,
        SilverkiteColumn.COLS_WEEKEND_SEAS: False,
    ...
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.DAY.name, num_days=40, feature_sets_enabled="auto"
    )
    expected = {
        SilverkiteColumn.COLS_HOUR_OF_WEEK: False,
        SilverkiteColumn.COLS_WEEKEND_SEAS: False,
    ...
    # hourly data, 3+ years
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=2000, feature_sets_enabled="auto"
    )
    expected = {
        SilverkiteColumn.COLS_HOUR_OF_WEEK: True,
        SilverkiteColumn.COLS_WEEKEND_SEAS: True,
    ...
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=101, feature_sets_enabled="auto"
    )
    expected = {
        SilverkiteColumn.COLS_HOUR_OF_WEEK: True,
        SilverkiteColumn.COLS_WEEKEND_SEAS: True,
    ...
    # weekly data
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.WEEK.name, num_days=60, feature_sets_enabled="auto"
    )
    expected = {
        SilverkiteColumn.COLS_HOUR_OF_WEEK: False,
        SilverkiteColumn.COLS_WEEKEND_SEAS: False,
    ...
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=101, feature_sets_enabled=False
    )
    assert feature_sets_enabled == expected
    # feature_sets_enabled=None
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=101, feature_sets_enabled=None
    )
    assert feature_sets_enabled == expected
    # feature_sets_enabled=True
    feature_sets_enabled = silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
        simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=101, feature_sets_enabled=True
    )
    ...
    with pytest.raises(ValueError, match="Unrecognized feature set: 'unknown_feature_set'"):
        silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
            simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=101, feature_sets_enabled={"unknown_feature_set": True}
        )
    with pytest.raises(
        ValueError,
        match=f"Unrecognized `feature_sets_enabled` dictionary value for key {SilverkiteColumn.COLS_TREND_WEEKLY_SEAS}: "
        f"expected bool or 'auto' or None. Found: off",
    ):
        silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(
            simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=101, feature_sets_enabled={SilverkiteColumn.COLS_TREND_WEEKLY_SEAS: "off"}
        )
    with pytest.raises(ValueError, match="Unrecognized type for `feature_sets_enabled`: expected bool, dict, 'auto', or None. Found: on"):
        silverkite._SimpleSilverkiteForecast__get_feature_sets_enabled(simple_freq=SimpleTimeFrequencyEnum.HOUR.name, num_days=101, feature_sets_enabled="on")
...
def test_get_feature_sets_terms():
    ...
    holidays = silverkite._SimpleSilverkiteForecast__get_silverkite_holidays(
        holiday_lookup_countries="auto", holidays_to_model_separately="auto", start_year=2017, end_year=2025, pre_num=2, post_num=2
    )
    ...
    expected = {
        ...
        SilverkiteColumn.COLS_EVENT_SEAS: (2 * daily_seas_interaction_order * num_interaction_holidays, "C(Q('events_Christmas'), level"),
    ...
    feature_sets_terms = silverkite._SimpleSilverkiteForecast__get_feature_sets_terms(
        daily_event_df_dict=None, daily_seas_interaction_order=0, weekly_seas_interaction_order=0, growth_term=None, changepoint_cols=None
    )
    expected = {
        # number of terms, substring of first term
        SilverkiteColumn.COLS_HOUR_OF_WEEK: (1, "C(Q('dow_hr'), levels=['1_00',"),
...
def test_get_silverkite_holidays():
    ...
    holidays_to_model_separately = get_available_holidays_across_countries(countries=holiday_lookup_countries, year_start=2016, year_end=2026)
    expected = generate_holiday_events(
        countries=holiday_lookup_countries, holidays_to_model_separately=holidays_to_model_separately, year_start=2016, year_end=2026, pre_num=1, post_num=3
    )
    assert_equal(holidays, expected)

    # custom values
    holiday_lookup_countries = ["US"]
    holidays_to_model_separately = ["New Years", "Christmas"]
    pre_post_num_dict = {"New Years": (7, 3), "Christmas": (3, 3)}
    holidays = silverkite._SimpleSilverkiteForecast__get_silverkite_holidays(
        holiday_lookup_countries=holiday_lookup_countries,
        holidays_to_model_separately=holidays_to_model_separately,
        start_year=2021,
    ...
    holidays_to_model_separately = "New Years"
    with pytest.raises(ValueError, match="`holiday_lookup_countries` should be a list, found India"):
        silverkite._SimpleSilverkiteForecast__get_silverkite_holidays(
            holiday_lookup_countries=holiday_lookup_countries, holidays_to_model_separately="auto", start_year=2021, end_year=2022, pre_num=2, post_num=2
        )
    with pytest.raises(ValueError, match="`holidays_to_model_separately` should be a list, found New Years"):
        silverkite._SimpleSilverkiteForecast__get_silverkite_holidays(
            holiday_lookup_countries="auto", holidays_to_model_separately=holidays_to_model_separately, start_year=2021, end_year=2022, pre_num=2, post_num=2
        )
...
def test_auto_config_params(daily_data_reg):
    ...
    params = silverkite.convert_params(
        ...
        daily_event_df_dict=dict(
            custom_event=pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2010-03-03", "2011-03-03", "2012-03-03"]), EVENT_DF_LABEL_COL: "threethree"})
        ),
        auto_holiday_params=dict(n_clusters=5),
        auto_growth=True,
        growth_term="quadratic",
        changepoints_dict=dict(method="uniform", n_changepoints=2),
    ...
    assert params["fs_components_df"].equals(
        pd.DataFrame({"name": ["tow", "toq", "ct1"], "period": [7.0, 1.0, 1.0], "order": [3, 1, 6], "seas_names": ["weekly", "quarterly", "yearly"]})
    )
...
def test_auto_config_run(daily_data_reg):
    ...
    silverkite.forecast_simple(
        ...
        holiday_post_num_days=2,
        daily_event_df_dict=dict(
            custom_event=pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2010-03-03", "2011-03-03", "2012-03-03"]), EVENT_DF_LABEL_COL: "event"})
        ),
        auto_holiday_params=None,
        auto_growth=True,
        growth_term="quadratic",
...
def test_quantile_regression():
    ...
    trained_model = silverkite.forecast_simple(
        ...
        fit_algorithm_params={"quantile": 0.9, "alpha": 0},
    )
    pred = silverkite.predict(df, trained_model=trained_model)["fut_df"]
    assert round(sum(pred[VALUE_COL] > df[VALUE_COL]) / len(pred), 1) >= 0.8
...
def test_regression_weight_col_passed_through():
    """Tests that regression_weight_col is propagated through forecast_simple so the weight column
    is preserved in fit_df and reaches the ML fitting step without KeyError.
    Regression test for: weight_col not passed to get_forecast_time_properties."""
    import numpy as np
    ...
    # built-in greykite weight columns (ct1/ct2/ct3) should also work
    trained_model_ct3 = silverkite.forecast_simple(
        df=df,
        time_col=TIME_COL,
        value_col=VALUE_COL,
        forecast_horizon=7,
        ...
        feature_sets_enabled=False,
        fit_algorithm="ridge",
        regression_weight_col="ct3",
    )
    assert trained_model_ct3 is not None
