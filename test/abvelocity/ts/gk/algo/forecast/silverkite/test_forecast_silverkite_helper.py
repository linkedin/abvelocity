...
import pandas as pd
import pytest
from abvelocity.ts.gk.algo.forecast.silverkite.forecast_silverkite_helper import (
    get_default_changepoints_dict,
    get_fourier_feature_col_names,
    get_silverkite_uncertainty_dict,
    get_similar_lag,
)
from abvelocity.ts.common.features.timeseries_features import fourier_series_multi_fcn
...
def test_get_silverkite_uncertainty_dict():
    ...
    obtained_dict = get_silverkite_uncertainty_dict(uncertainty=None, coverage=0.95)
    ...
    obtained_dict = get_silverkite_uncertainty_dict(uncertainty="auto", coverage=0.95)
    assert_equal(obtained_dict, expected_uncertainty)

    # coverage provided, uncertainty missing quantiles
    coverage = 0.80
    uncertainty_dict = {"uncertainty_method": "simple_conditional_residuals"}
    obtained_dict = get_silverkite_uncertainty_dict(uncertainty=uncertainty_dict, coverage=coverage)
    ...
    assert get_silverkite_uncertainty_dict(uncertainty=None, coverage=None) is None
...
def test_get_silverkite_uncertainty_dict_exception():
    ...
    with pytest.raises(ValueError, match=expected_match_str):
        get_silverkite_uncertainty_dict(uncertainty=uncertainty_dict, coverage=0.95)
    ...
    expected_match_str = "`quantiles` is expected to be an increasing sequence" " of at least two elements."
    ...
    expected_match_str = "`quantiles` are not specified in `uncertainty`" " and `coverage` is not provided to infer them"

    with pytest.raises(ValueError, match=expected_match_str):
        get_silverkite_uncertainty_dict(uncertainty=uncertainty_dict, coverage=None)
...
def test_get_silverkite_uncertainty_dict_warning2():
    ...
    with pytest.warns(Warning):
        get_silverkite_uncertainty_dict(uncertainty=uncertainty_dict, coverage=0.6)
...
def test_get_default_changepoints_dict():
    ...
    change_points_dict = get_default_changepoints_dict(changepoints_method="uniform", num_days=365, forecast_horizon_in_days=7)

    for key in ["method", "n_changepoints", "continuous_time_col"]:
        assert change_points_dict[key] == {"method": "uniform", "n_changepoints": 12, "continuous_time_col": "ct1"}[key]

    change_points_dict = get_default_changepoints_dict(changepoints_method="auto", num_days=365, forecast_horizon_in_days=7)
    ...
    change_points_dict = get_default_changepoints_dict(changepoints_method=None, num_days=365, forecast_horizon_in_days=7)

    assert change_points_dict is None

    change_points_dict = get_default_changepoints_dict(changepoints_method="auto", num_days=10, forecast_horizon_in_days=7)

    assert change_points_dict is None

    change_points_dict = get_default_changepoints_dict(changepoints_method="uniform", num_days=10, forecast_horizon_in_days=7)
...
def test_get_fourier_feature_col_names():
    ...
    df = pd.DataFrame({"ts": pd.date_range("2020-01-01", freq="D", periods=14)})
    fs_components_df = pd.DataFrame(
        {"name": ["tod", "tow", "toy"], "period": [24.0, 7.0, 1.0], "order": [1, 2, 3], "seas_names": ["daily", "weekly", "yearly"]}
    )
    ...
    fs_cols = get_fourier_feature_col_names(df=df, time_col="ts", fs_func=fs_func)
    assert fs_cols == [
        "sin1_tod_daily",
        "cos1_tod_daily",
        "sin1_tow_weekly",
