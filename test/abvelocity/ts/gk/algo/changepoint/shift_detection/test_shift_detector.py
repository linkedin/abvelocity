...
from abvelocity.ts.common.constants import LEVELSHIFT_COL_PREFIX_SHORT
...
def test_detect_daily(time_col: str, value_col: str):
    ...
    output_regressor_col, output_df = detector.detect(input_df, time_col=time_col, value_col=value_col, forecast_horizon=5, freq="D", z_score_cutoff=3)

    # unit test
    pd.testing.assert_frame_equal(output_df, expected_df)
    assert output_regressor_col == [f"{LEVELSHIFT_COL_PREFIX_SHORT}_2020_01_11_00_00", f"{LEVELSHIFT_COL_PREFIX_SHORT}_2020_01_21_00_00"]
...
def test_detect_weekly(time_col: str, value_col: str):
    ...
    # call the function
    detector = ShiftDetection()
    output_regressor_col, output_df = detector.detect(input_df, time_col=time_col, value_col=value_col, forecast_horizon=5, freq="W", z_score_cutoff=3)

    # unit test
    pd.testing.assert_frame_equal(output_df, expected_df)
    assert output_regressor_col == [f"{LEVELSHIFT_COL_PREFIX_SHORT}_2020_03_15_00_00", f"{LEVELSHIFT_COL_PREFIX_SHORT}_2020_05_24_00_00"]
...
def test_invalid_freq(time_col="ts", value_col="actual"):
    ...
    with pytest.raises(ValueError):
        output_regressor_col, output_df = detector.detect(input_df, time_col=time_col, value_col=value_col, forecast_horizon=5, freq="S", z_score_cutoff=3)
...
def test_find_shifts():
    ...
    expected_shift_dates = [(datetime(2020, 1, 11, 0, 0, 0), datetime(2020, 1, 11, 0, 0, 0)), (datetime(2020, 1, 21, 0, 0, 0), datetime(2020, 1, 21, 0, 0, 0))]
    ...
    # call the function
    detector = ShiftDetection()
    output_df_find_shifts, output_shift_dates = detector.find_shifts(input_df, time_col="ts", value_col="actual", z_score_cutoff=3)
...
def test_create_df_with_regressor():
    # create inputs
    input_shiftsm = [(datetime(2020, 1, 11, 0, 0, 0), datetime(2020, 1, 11, 0, 0, 0)), (datetime(2020, 1, 21, 0, 0, 0), datetime(2020, 1, 21, 0, 0, 0))]
    input_val_ls = [100] * 10 + [200] * 10 + [300] * 10
    input_ts_ls = pd.date_range(datetime(2020, 1, 1), freq="D", periods=30)
    input_df = pd.DataFrame({"ts": input_ts_ls, "actual": input_val_ls})
