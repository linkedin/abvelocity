...
import pandas as pd
import pytest
from abvelocity.ts.gk.algo.changepoint.adalasso.changepoint_detector import ChangepointDetector, get_changepoints_dict, get_seasonality_changepoints
from abvelocity.ts.gk.algo.changepoint.shift_detection.shift_detector import ShiftDetection
from abvelocity.ts.common.data_loader import DataLoader
from abvelocity.ts.common.logging import LOGGER_NAME
from abvelocity.ts.common.testing_utils import generate_df_for_tests, generate_df_with_arbitrary_trends_and_shifts, generate_test_changepoint_df
...
@pytest.fixture
def hourly_data():
    """Generate 200 days of hourly data for tests"""
    return generate_df_for_tests(freq="H", periods=24 * 200)
...
def test_find_trend_changepoints(hourly_data):
    ...
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y")
    ...
    model = ChangepointDetector()
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y", regularization_strength=1.0)
    assert isinstance(model.trend_model, RegressorMixin)
    assert model.trend_model.coef_.shape[0] == 100 + 1 + 8 * 2
    assert model.trend_coef.shape[0] == 100 + 1 + 8 * 2
    assert model.trend_intercept is not None
    ...
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y", regularization_strength=0.1)
    # ``regularization_strength`` between 0 and 1 indicates at least one change point
    assert len(model.trend_changepoints) > 0
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y", actual_changepoint_min_distance="D", regularization_strength=0.0)
    ...
    model = ChangepointDetector()
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y", potential_changepoint_distance="100D")
    # test override `potential_changepoint_n`
    # df has length 200 days, with distance "100D", only 1 change point is placed.
    assert model.trend_potential_changepoint_n == 1
    with pytest.raises(
        ValueError,
        ...
        model.find_trend_changepoints(df=df, time_col="ts", value_col="y", potential_changepoint_distance="2M")
    # test `no_changepoint_distance_from_end` with the Peyton Manning data
    model = ChangepointDetector()
    res = model.find_trend_changepoints(
        df=df_pt, time_col="ts", value_col="y", no_changepoint_distance_from_begin="730D", no_changepoint_distance_from_end="730D", regularization_strength=0
    )
    ...
    first_date_to_have_changepoint = pd.to_datetime(df_pt["ts"].iloc[int(df_pt.shape[0] * no_changepoint_proportion_from_end)])
    ...
    with pytest.raises(
        ...
        model.find_trend_changepoints(df=df_pt, time_col="ts", value_col="y", no_changepoint_distance_from_end="2M")
    ...
    model = ChangepointDetector()
    with pytest.raises(ValueError, match="potential_changepoint_n can not be negative. " "A large number such as 100 is recommended"):
        model.find_trend_changepoints(df=df, time_col="ts", value_col="y", potential_changepoint_n=-1)
    # negative year_seasonality_order
    model = ChangepointDetector()
    with pytest.raises(ValueError, match="year_seasonality_order can not be negative. " "A number less than or equal to 10 is recommended"):
        model.find_trend_changepoints(df=df, time_col="ts", value_col="y", yearly_seasonality_order=-1)
    # negative regularization_strength
    with pytest.raises(ValueError, match="regularization_strength must be between 0.0 and 1.0."):
        model.find_trend_changepoints(df=df, time_col="ts", value_col="y", regularization_strength=-1)
    # estimator parameter combination not valid warning
    with pytest.warns(UserWarning, match="trend_estimator not in"):
        model = ChangepointDetector()
        model.find_trend_changepoints(df=df, time_col="ts", value_col="y", trend_estimator="something")
    with pytest.warns(UserWarning, match="trend_estimator"):
        model = ChangepointDetector()
        model.find_trend_changepoints(df=df, time_col="ts", value_col="y", trend_estimator="ols", yearly_seasonality_order=8)
    with pytest.warns(UserWarning, match="adaptive_lasso_initial_estimator"):
        model = ChangepointDetector()
        model.find_trend_changepoints(df=df, time_col="ts", value_col="y", adaptive_lasso_initial_estimator="something")
    # df sample size too small
    df = pd.DataFrame(data={"ts": pd.date_range(start="2020-1-1", end="2020-1-3", freq="D"), "y": [1, 2, 3]})
    model = ChangepointDetector()
    with pytest.raises(ValueError, match="Change point detector does not work for less than " "5 observations. Please increase sample size."):
        model.find_trend_changepoints(
            df=df,
            time_col="ts",
            value_col="y",
    ...
    df = pd.DataFrame(data={"ts": pd.date_range(start="2020-1-1", end="2020-1-9", freq="2D"), "y": [1, 2, 3, 4, 5]})
    model = ChangepointDetector()
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y")
    ...
    model = ChangepointDetector()
    model.find_trend_changepoints(df=df_pt, time_col="ts", value_col="y", yearly_seasonality_change_freq="365D")
...
def test_find_trend_changepoints_with_shift_detector(hourly_data):
    df = generate_df_with_arbitrary_trends_and_shifts(
        ...
        freq="D",
        seed=10,
        trend_slopes=[-1.0, 1.0],
        trend_intervals=[0.5, 1.0],
        level_shifts=[(0.05, 0.15), (0.25, 0.35), (0.65, 0.75), (0.85, 0.95)],
    ...
    assert model.adaptive_lasso_coef is None
    # model training with default values
    model.find_trend_changepoints(df=df, time_col="timestamp", value_col="y")
    ...
    model.find_trend_changepoints(df=df, time_col="timestamp", value_col="y", shift_detector=ShiftDetection())
    ...
    # test a given ``regularization_strength``
    model = ChangepointDetector()
    model.find_trend_changepoints(df=df, time_col="timestamp", value_col="y", regularization_strength=1.0, shift_detector=ShiftDetection())
    assert isinstance(model.trend_model, RegressorMixin)
    assert model.trend_model.coef_.shape[0] >= 100 + 1 + 8 * 2
    assert model.trend_coef.shape[0] >= 100 + 1 + 8 * 2
    assert model.trend_intercept is not None
    ...
    assert model.y.index[0] not in model.trend_changepoints
    assert model.trend_changepoints == []
    model.find_trend_changepoints(df=df, time_col="timestamp", value_col="y", regularization_strength=0.5, shift_detector=ShiftDetection())
    # ``regularization_strength`` between 0 and 1 indicates at least one change point
    assert len(model.trend_changepoints) > 0
    model.find_trend_changepoints(
        df=df, time_col="timestamp", value_col="y", actual_changepoint_min_distance="D", regularization_strength=0.0, shift_detector=ShiftDetection()
    )
    # ``regularization_strength`` == 0.0 indicates all potential change points are present
    assert len(model.trend_changepoints) == 100
    with pytest.raises(
        ...
        model.find_trend_changepoints(df=df, time_col="timestamp", value_col="y", potential_changepoint_distance="2M", shift_detector=ShiftDetection())
...
def test_find_trend_changepoints_slow(hourly_data):
    ...
    model = ChangepointDetector()
    model.find_trend_changepoints(df=df_pt, time_col="ts", value_col="y", fast_trend_estimation=False)
    assert isinstance(model.trend_model, RegressorMixin)
    assert model.trend_model.coef_.shape[0] == 100 + 1 + 8 * 2
    assert model.trend_coef.shape[0] == 100 + 1 + 8 * 2
    assert model.trend_intercept is not None
...
def test_find_seasonality_changepoints(hourly_data):
    ...
    model.find_seasonality_changepoints(
        ...
        seasonality_components_df=pd.DataFrame(
            {"name": ["tod", "tow", "conti_year"], "period": [24.0, 7.0, 1.0], "order": [3, 4, 5], "seas_names": ["daily", "weekly", "yearly"]}
        ),
    )
    ...
    # test a given ``regularization_strength``
    model = ChangepointDetector()
    model.find_seasonality_changepoints(df=df, time_col="ts", value_col="y", regularization_strength=1.0)
    # ``regularization_strength`` == 1.0 indicates no change point
    assert all([model.seasonality_changepoints[key] == [] for key in model.seasonality_changepoints.keys()])
    model.find_seasonality_changepoints(df=df_pt, time_col="ts", value_col="y", regularization_strength=0.0)
    ...
    # test `no_changepoint_distance_from_end` with the Peyton Manning data
    model = ChangepointDetector()
    res = model.find_seasonality_changepoints(df=df_pt, time_col="ts", value_col="y", no_changepoint_distance_from_end="730D", regularization_strength=0.1)
    ...
    res = cd.find_seasonality_changepoints(df=df_pt, time_col="ts", value_col="y")
    ...
    cd = ChangepointDetector()
    cd.find_seasonality_changepoints(df=df_pt, time_col="ts", value_col="y")
    with pytest.warns(UserWarning, match="Trend changepoints are already identified, using past trend estimation."):
        cd.find_seasonality_changepoints(df=df2, time_col="ts2", value_col="y2")
    assert cd.time_col == "ts"
    assert cd.value_col == "y"
    # negative potential_changepoint_n
    model = ChangepointDetector()
    with pytest.raises(ValueError, match="potential_changepoint_n can not be negative. " "A large number such as 50 is recommended"):
        model.find_seasonality_changepoints(df=df, time_col="ts", value_col="y", potential_changepoint_n=-1)
    # negative regularization_strength
    with pytest.raises(ValueError, match="regularization_strength must be between 0.0 and 1.0."):
        model.find_seasonality_changepoints(df=df, time_col="ts", value_col="y", regularization_strength=-1)
    # test regularization_strength == None warning
    with pytest.warns(UserWarning, match="regularization_strength is set to None. This will"):
        model.find_seasonality_changepoints(df=df, time_col="ts", value_col="y", regularization_strength=None)
    # test existing trend estimation warning
    model = ChangepointDetector()
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y")
    with pytest.warns(UserWarning, match="Trend changepoints are already identified, using past trend estimation."):
        model.find_seasonality_changepoints(df=df, time_col="ts", value_col="y")
    # df sample size too small
    df_small = pd.DataFrame(data={"ts": pd.date_range(start="2020-1-1", end="2020-1-3", freq="D"), "y": [1, 2, 3]})
    model = ChangepointDetector()
    with pytest.raises(ValueError, match="Change point detector does not work for less than " "5 observations. Please increase sample size."):
        model.find_seasonality_changepoints(
            df=df_small,
    ...
    cd = ChangepointDetector()
    cd.find_seasonality_changepoints(df=df_pt, time_col="ts", value_col="y", trend_changepoints=list(pd.to_datetime(["2016-01-01", "2017-02-05"])))
...
def test_plot(hourly_data):
    ...
    model.find_trend_changepoints(df=df, time_col="ts", value_col="y")
    # test empty plot
    with pytest.warns(UserWarning, match="Figure is empty, at least one component"):
        model.plot(
            observation=False,
    ...
    with pytest.warns(UserWarning, match="You haven't run trend change point detection "):
        model = ChangepointDetector()
        model.plot(
            observation=False,
            observation_original=False,
    ...
    with pytest.warns(UserWarning, match="You haven't run seasonality change point detection"):
        model = ChangepointDetector()
        model.plot(
            observation=False,
            observation_original=False,
...
def test_get_changepoints_dict():
    ...
    new_changepoints_dict, changepoint_detector = get_changepoints_dict(df=df_pt, time_col="ts", value_col="y", changepoints_dict=changepoints_dict)
    ...
    assert (changepoint_dates[-1] - pd.to_datetime(df_pt["ts"].iloc[0])) / (pd.to_datetime(df_pt["ts"].iloc[-1]) - pd.to_datetime(df_pt["ts"].iloc[0])) <= 0.7
    ...
    # tests for None
    new_changepoints_dict, changepoint_detector = get_changepoints_dict(df=df_pt, time_col="ts", value_col="y", changepoints_dict=None)
    assert new_changepoints_dict is None
    assert changepoint_detector is None
    # tests for "custom"
    changepoints_dict = {"method": "custom", "dates": ["2020-01-01"]}
    new_changepoints_dict, changepoint_detector = get_changepoints_dict(df=df_pt, time_col="ts", value_col="y", changepoints_dict=changepoints_dict)
    assert new_changepoints_dict == changepoints_dict
    assert changepoint_detector is None
    # tests for uniform
    changepoints_dict = {"method": "uniform", "n_changepoints": 100}
    new_changepoints_dict, changepoint_detector = get_changepoints_dict(df=df_pt, time_col="ts", value_col="y", changepoints_dict=changepoints_dict)
    assert new_changepoints_dict == changepoints_dict
    assert changepoint_detector is None
    # tests unused keys
    changepoints_dict = {"method": "auto", "unused_key": "value"}
    with pytest.warns(UserWarning, match="The following keys in"):
        get_changepoints_dict(df=df_pt, time_col="ts", value_col="y", changepoints_dict=changepoints_dict)


def test_get_seasonality_changepoints():
    # tests the functionality under cases that were not tested elsewhere
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-01", end="2020-01-30", freq="D"), "y": np.random.randn(30)})
    # tests uniform trend change point dictionary
    seasonality_changepoints_result = get_seasonality_changepoints(
        df=df,
        time_col="ts",
        value_col="y",
        trend_changepoints_dict={"method": "uniform", "n_changepoints": 5},
    ...
    seasonality_changepoints_result = get_seasonality_changepoints(
        ...
        time_col="ts",
        value_col="y",
        trend_changepoints_dict={"method": "custom", "dates": ["2020-01-10"]},
        trend_changepoint_dates=None,
        seasonality_changepoints_dict={
            "seasonality_components_df": pd.DataFrame({"name": ["conti_year"], "period": [1.0], "order": [1], "seas_names": ["yearly"]}),
            "resample_freq": "D",
...
def test_nan():
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-01", periods=366, freq="D"), "y": np.random.randn(366)})
    df.iloc[5:10, 1] = np.nan
    cd = ChangepointDetector()
    cd.find_trend_changepoints(df=df, time_col="ts", value_col="y")
    cd.find_seasonality_changepoints(df=df, time_col="ts", value_col="y")
...
def test_capping_potential_changepoints():
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        cd = ChangepointDetector()
        cd.find_trend_changepoints(df=df, time_col="ts", value_col="y", resample_freq="D", potential_changepoint_distance="D", potential_changepoint_n_max=100)
        log_capture.check_present(
            (
                LOGGER_NAME,
                "INFO",
                "Number of potential changepoints is capped by 'potential_changepoint_n_max' "
                "as 100. The 'potential_changepoint_distance' D is ignored. "
                "The original number of changepoints was 365.",
            )
        )
