...
from abvelocity.ts.gk.algo.changepoint.adalasso.changepoints_utils import (
    ...
    build_seasonality_feature_df_with_changes,
    build_trend_feature_df_with_changes,
    check_freq_unit_at_most_day,
    combine_detected_and_custom_trend_changepoints,
    ...
    get_seasonality_changes_from_adaptive_lasso,
    get_trend_changepoint_dates_from_cols,
    get_trend_changes_from_adaptive_lasso,
...
def test_build_trend_feature_df_with_changes(hourly_data):
    ...
    df = hourly_data["df"]
    df_trend = build_trend_feature_df_with_changes(df=df, time_col="ts")
    assert df_trend.shape[0] == df.shape[0]
    assert df_trend.shape[1] == 101  # default value
    # test given parameters
    df_trend = build_trend_feature_df_with_changes(df=df, time_col="ts", changepoints_dict={"method": "uniform", "n_changepoints": 50})
    assert df_trend.shape[0] == df.shape[0]
    assert df_trend.shape[1] == 51
    # test no change point
    df_trend = build_trend_feature_df_with_changes(df=df, time_col="ts", changepoints_dict={"method": "uniform", "n_changepoints": 0})
    assert df_trend.shape[0] == df.shape[0]
    assert df_trend.shape[1] == 1
    # test result values
    df = pd.DataFrame({"ts": [dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5), dt(2020, 1, 6)], "y": [1, 2, 3, 4, 5, 6]})
    df_trend = build_trend_feature_df_with_changes(df=df, time_col="ts", changepoints_dict={"method": "uniform", "n_changepoints": 2})
    expected_df_trend = pd.DataFrame(
        ...
        index=[dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5), dt(2020, 1, 6)],
    )
...
def test_build_seasonality_feature_df_with_changes(hourly_data):
    # test default parameters
    df = hourly_data["df"]
    df_seasonality = build_seasonality_feature_df_with_changes(df=df, time_col="ts")
    assert df_seasonality.shape[0] == df.shape[0]
    assert df_seasonality.shape[1] == 22  # default value
    # test given parameters
    df_seasonality = build_seasonality_feature_df_with_changes(df=df, time_col="ts", changepoints_dict={"method": "uniform", "n_changepoints": 10})
    assert df_seasonality.shape[0] == df.shape[0]
    ...
    df_seasonality = build_seasonality_feature_df_with_changes(
        df=df,
        time_col="ts",
        changepoints_dict={"method": "uniform", "n_changepoints": 10},
        fs_components_df=pd.DataFrame({"name": ["conti_year"], "period": [1.0], "order": [8], "seas_names": ["yearly"]}),
    )
    assert df_seasonality.shape[0] == df.shape[0]
    assert df_seasonality.shape[1] == 16 * 11
    # test no change point
    df_seasonality = build_seasonality_feature_df_with_changes(df=df, time_col="ts", changepoints_dict={"method": "uniform", "n_changepoints": 0})
    assert df_seasonality.shape[0] == df.shape[0]
    assert df_seasonality.shape[1] == 22
    # test result values
    df = pd.DataFrame({"ts": [dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5)], "y": [1, 2, 3, 4, 5]})
    df_seasonality = build_seasonality_feature_df_with_changes(
        df=df,
        time_col="ts",
        changepoints_dict={"method": "uniform", "n_changepoints": 1},
        fs_components_df=pd.DataFrame({"name": ["conti_year"], "period": [1.0], "order": [1], "seas_names": ["yearly"]}),
    )
    freq = 2 * np.pi / 366
    expected_df_seasonality = pd.DataFrame(
        ...
        index=[dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5)],
    )
...
def test_build_seasonality_feature_df_with_detection_result():
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-06", end="2020-01-12", freq="D"), "y": list(range(7))})
    seasonality_changepoints = {"weekly": list(pd.to_datetime(["2020-01-08", "2020-01-11"])), "yearly": list(pd.to_datetime(["2020-01-09"]))}
    seasonality_components_df = pd.DataFrame({"name": ["tow", "conti_year"], "period": [7.0, 1.0], "order": [1, 2], "seas_names": ["weekly", "yearly"]})
    ...
    expected_df = pd.DataFrame(
        {
            ...
            "sin1_conti_year_yearly_2020_01_09_00": np.sin([2 * np.pi * i / 366 if i > year_cp1 else 0 for i in range(5, 12)]),
            "cos1_conti_year_yearly_2020_01_09_00": np.cos([2 * np.pi * i / 366 if i > year_cp1 else np.pi / 2 for i in range(5, 12)]),
            "sin2_conti_year_yearly_2020_01_09_00": np.sin([2 * np.pi * i / 366 * 2 if i > year_cp1 else 0 for i in range(5, 12)]),
            "cos2_conti_year_yearly_2020_01_09_00": np.cos([2 * np.pi * i / 366 * 2 if i > year_cp1 else np.pi / 2 for i in range(5, 12)]),
        },
        index=df["ts"],
    )
    ...
    with pytest.warns(UserWarning) as record:
        ...
        assert (
            f"The following seasonality components have detected seasonality changepoints"
            f" but these changepoints are not included in the model,"
            f" because the seasonality component is not included in the model. {['yearly']}"
        ) in record[0].message.args[0]
    expected_df = expected_df[
        ["sin1_tow_weekly_2020_01_08_00", "cos1_tow_weekly_2020_01_08_00", "sin1_tow_weekly_2020_01_11_00", "cos1_tow_weekly_2020_01_11_00"]
    ]
    pd.testing.assert_frame_equal(seasonality_df, expected_df, check_names=False)
...
def test_compute_fitted_components():
    ...
    intercept = 1
    trend = compute_fitted_components(x=df, coef=coef, regex="^changepoint", include_intercept=True, intercept=intercept)
    expected_trend = pd.Series([1 * 1 + 2 * 0 + 1] + [1 * 1 + 2 * 2 + 1] * 3)
    ...
    seasonality = compute_fitted_components(x=df, coef=coef, regex="^.*yearly.*$", include_intercept=False)
    expected_seasonality = pd.Series(
        [np.sin(1) + np.cos(1), np.sin(2) + np.cos(2), np.sin(3) + np.cos(3) + np.sin(3) + np.cos(3), np.sin(4) + np.cos(4) + np.sin(4) + np.cos(4)]
    )
    assert seasonality.equals(expected_seasonality)
    # tests ValueError
    with pytest.raises(ValueError, match="``intercept`` must be provided when ``include_intercept`` is True."):
        compute_fitted_components(x=df, coef=coef, regex="^.*yearly.*$", include_intercept=True)


def test_plot_change():
    observations = pd.Series({"y": [1, 2, 3, 4, 5]}, index=[dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5)])
    trend_estimate = pd.Series([2, 3, 4, 5, 6], index=[dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5)])
    trend_change = [dt(2020, 1, 2), dt(2020, 1, 4)]
    year_seasonality_estimate = pd.Series([1, 2, 1, 2, 1], index=[dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5)])
    adaptive_lasso_estimate = pd.Series([3, 4, 5, 6, 7], index=[dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5)])
    seasonality_estimate = pd.Series([1, 2, 3, 1, 2], index=[dt(2020, 1, 1), dt(2020, 1, 2), dt(2020, 1, 3), dt(2020, 1, 4), dt(2020, 1, 5)])
    seasonality_change_dict = {"weekly": [dt(2020, 1, 1)], "yearly": [dt(2020, 1, 3)]}
    ...
    fig1 = plot_change(
        observation=None, trend_estimate=None, trend_change=trend_change, year_seasonality_estimate=None, adaptive_lasso_estimate=adaptive_lasso_estimate
    )
    ...
    with pytest.warns(UserWarning) as record:
        ...
        assert ("Only the first 8 components with detected change points" "are plotted.") in record[0].message.args[0]
    with pytest.warns(UserWarning) as record:
        plot_change(
            observation=None,
            trend_estimate=None,
    ...
    assert (
        "trend_change is not shown. Must provide observations, trend_estimate, " "adaptive_lasso_estimate or seasonality_estimate to plot trend_change."
    ) in record[0].message.args[0]
    with pytest.warns(UserWarning) as record:
        plot_change(
            observation=None,
        ...
        assert ("seasonality_change is not shown. Must provide observations or" " seasonality_estimate to plot seasonality_change.") in record[0].message.args[
            0
        ]
    with pytest.raises(ValueError, match="seasonality_change must be either list or dict."):
        plot_change(
            observation=None,
            trend_estimate=trend_estimate,
            trend_change=trend_change,
...
def test_adaptive_lasso_cv():
    ...
    intercept, coef = adaptive_lasso_cv(x=x, y=y, initial_coef="ridge")
    assert coef.shape[0] == 5
    assert intercept is not None
    # test "lasso" as initial estimator
    intercept, coef = adaptive_lasso_cv(x=x, y=y, initial_coef="lasso")
    assert coef.shape[0] == 5
    assert intercept is not None
    # test "ols" as initial estimator
    intercept, coef = adaptive_lasso_cv(x=x, y=y, initial_coef="ols")
    assert coef.shape[0] == 5
    assert intercept is not None
    # test `numpy.array` as initial estimator
    initial_coef = np.array([1, 2, 3, 4, 5])
    intercept, coef = adaptive_lasso_cv(x=x, y=y, initial_coef=initial_coef)
    assert coef.shape[0] == 5
    assert intercept is not None
    # test given lambda for adaptive lasso
    intercept, coef = adaptive_lasso_cv(x=x, y=y, initial_coef="ridge", regularization_strength=1.0)
    assert coef.shape[0] == 5
    assert intercept is not None
    # tests no change point when ``regularization_strength`` == 1.0
    assert (coef != 0).sum() == 0
    _, coef = adaptive_lasso_cv(x=x, y=y, initial_coef="ridge", regularization_strength=0.0)
    ...
    _, coef1 = adaptive_lasso_cv(x=x, y=y, initial_coef="ridge", regularization_strength=0.1)
    _, coef2 = adaptive_lasso_cv(x=x, y=y, initial_coef="ridge", regularization_strength=0.1, max_min_ratio=1e100)
    ...
    with pytest.raises(ValueError, match="regularization_strength must be between 0.0 and 1.0."):
        adaptive_lasso_cv(x=x, y=y, initial_coef="ridge", regularization_strength=-1)


def test_find_neighbor_changepoints():
    cp_idx = [1, 2, 5, 7, 8, 9, 10, 18, 22]
    neighbor_cps = find_neighbor_changepoints(cp_idx=cp_idx)
    assert neighbor_cps == [[1, 2], [5], [7, 8, 9, 10], [18], [22]]
    # test with greater neighbor distance
    cp_idx = [1, 2, 4, 7, 9, 12, 15, 20]
    neighbor_cps = find_neighbor_changepoints(cp_idx=cp_idx, min_index_distance=3)
    ...
    with pytest.raises(ValueError, match="`min_index_distance` must be positive."):
        find_neighbor_changepoints(cp_idx=cp_idx, min_index_distance=-1)
...
def test_get_trend_changes_from_adaptive_lasso():
    ...
    y = np.random.randn(20)
    intercept, coef = adaptive_lasso_cv(x=x, y=y, initial_coef="ridge")
    changepoipnt_dates = pd.Series([dt(2020, 1, i) for i in range(1, 21)])
    changepoints, coefs = get_trend_changes_from_adaptive_lasso(x=x, y=y, changepoint_dates=changepoipnt_dates, initial_coef="ridge")
    ...
    changepoints, coefs = get_trend_changes_from_adaptive_lasso(
        x=x, y=y, changepoint_dates=changepoipnt_dates, initial_coef="ridge", regularization_strength=1.0
    )
    assert intercept == coefs[0]
    assert np.array_equal(coef, coefs[1])
    assert changepoints is not None
...
def test_compute_min_changepoint_index_distance():
    # test functionality
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-01", end="2020-01-30"), "y": list(range(30))})
    min_index_distance = compute_min_changepoint_index_distance(df=df, time_col="ts", n_changepoints=10, min_distance_between_changepoints="5D")
    ...
    min_index_distance = compute_min_changepoint_index_distance(df=df, time_col="ts", n_changepoints=0, min_distance_between_changepoints="5D")
    assert min_index_distance == df.shape[0]
    # test non-daily
    min_index_distance = compute_min_changepoint_index_distance(df=df, time_col="ts", n_changepoints=10, min_distance_between_changepoints="120H")
    ...
    with pytest.raises(
        ...
        compute_min_changepoint_index_distance(df=df, time_col="ts", n_changepoints=10, min_distance_between_changepoints="W")
...
def test_filter_changepoints():
    ...
    selected_changepoints = filter_changepoints(cp_blocks=cp_blocks, coef=coef, min_index_distance=min_index_distance)
    ...
    with pytest.raises(
        ValueError, match="`min_index_distance` is the minimum distance between change point" "indices to consider them separate, and must be positive."
    ):
        filter_changepoints(cp_blocks=cp_blocks, coef=coef, min_index_distance=0)
...
def test_get_changes_from_beta():
    ...
    expected_changes = [np.array([np.sqrt(5), np.sqrt(52) - np.sqrt(5)]), np.array([np.sqrt(38), np.sqrt(264) - np.sqrt(38)])]
...
def test_estimate_trend_with_detected_changepoints():
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-01", end="2020-01-05", freq="D"), "y": [1, 2, 3, 4, 5]})
    changepoints = [dt(2020, 1, 3)]
    trend_estimate = estimate_trend_with_detected_changepoints(df=df, time_col="ts", value_col="y", changepoints=changepoints)
    ...
    # tests ValueError
    with pytest.raises(ValueError, match="estimator can only be either 'ridge' or 'ols'."):
        estimate_trend_with_detected_changepoints(df=df, time_col="ts", value_col="y", changepoints=changepoints, estimator="lasso")


def test_estimate_seasonality_with_detected_changepoints():
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-01", end="2020-01-05", freq="D"), "y": [1, 2, 3, 4, 5]})
    seasonality_changepoints = {"weekly": [dt(2020, 1, 2)], "yearly": [dt(2020, 1, 4)]}
...
def test_get_seasonality_changepoint_df_cols():
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-01", end="2020-01-10", freq="D"), "y": list(range(10))})
    seasonality_changepoints = {"weekly": list(pd.to_datetime(["2020-01-05", "2020-01-07"])), "yearly": list(pd.to_datetime(["2020-01-08"]))}
    seasonality_components_df = pd.DataFrame({"name": ["tow", "conti_year"], "period": [7.0, 1.0], "order": [1, 2], "seas_names": ["weekly", "yearly"]})
    ...
    assert cols == ["sin1_tow_weekly_2020_01_05_00", "cos1_tow_weekly_2020_01_05_00", "sin1_tow_weekly_2020_01_07_00", "cos1_tow_weekly_2020_01_07_00"]
...
def test_get_yearly_seasonality_changepoint_dates_from_freq():
    df = pd.DataFrame({"ts": pd.date_range(start="2020-01-01", end="2025-01-01", freq="D")})
    # tests None
    result = get_yearly_seasonality_changepoint_dates_from_freq(df=df, time_col="ts", yearly_seasonality_change_freq=None)
    assert result == []
    # tests regular input
    result = get_yearly_seasonality_changepoint_dates_from_freq(df=df, time_col="ts", yearly_seasonality_change_freq="365D")
    assert result == list(pd.to_datetime(["2020-12-31", "2021-12-31", "2022-12-31", "2023-12-31"]))
    # tests least_training_length
    result = get_yearly_seasonality_changepoint_dates_from_freq(df=df, time_col="ts", yearly_seasonality_change_freq="365D", min_training_length="400D")
    assert result == list(pd.to_datetime(["2020-12-31", "2021-12-31", "2022-12-31"]))
    # tests warnings
    with pytest.warns(UserWarning) as record:
        get_yearly_seasonality_changepoint_dates_from_freq(df=df, time_col="ts", yearly_seasonality_change_freq="180D")
        assert "yearly_seasonality_change_freq is less than a year. It might be too short " "to fit accurate yearly seasonality." in record[0].message.args[0]
    with pytest.warns(UserWarning) as record:
        get_yearly_seasonality_changepoint_dates_from_freq(df=df, time_col="ts", yearly_seasonality_change_freq="10000D")
        assert (
            "No yearly seasonality changepoint added. Either data length is too short "
...
def test_combine_detected_and_custom_trend_changepoints():
    ...
    combined_changepoints = combine_detected_and_custom_trend_changepoints(
        detected_changepoint_dates=detected_changepoints, custom_changepoint_dates=custom_changepoints, min_distance=None, keep_detected=False
    )
    assert all(combined_changepoints == pd.to_datetime(["2020-01-01", "2020-01-05", "2020-02-05", "2020-04-13", "2020-07-03"]))
    # tests ``min_distance`` with ``keep_detected=False``
    combined_changepoints = combine_detected_and_custom_trend_changepoints(
        detected_changepoint_dates=detected_changepoints, custom_changepoint_dates=custom_changepoints, min_distance="5D", keep_detected=False
    )
    assert all(combined_changepoints == pd.to_datetime(["2020-01-05", "2020-02-05", "2020-04-13", "2020-07-03"]))
    # tests ``min_distance`` with ``keep_detected=True``
    combined_changepoints = combine_detected_and_custom_trend_changepoints(
        detected_changepoint_dates=detected_changepoints, custom_changepoint_dates=custom_changepoints, min_distance="5D", keep_detected=True
    )
    assert all(combined_changepoints == pd.to_datetime(["2020-01-01", "2020-02-05", "2020-04-13", "2020-07-03"]))
    # tests empty input
    combined_changepoints = combine_detected_and_custom_trend_changepoints(
        detected_changepoint_dates=detected_changepoints, custom_changepoint_dates=[], min_distance="5D", keep_detected=False
    )
    assert all(combined_changepoints == pd.to_datetime(["2020-01-01", "2020-02-05", "2020-07-03"]))
    combined_changepoints = combine_detected_and_custom_trend_changepoints(
        detected_changepoint_dates=[], custom_changepoint_dates=custom_changepoints, min_distance="5D", keep_detected=False
    )
    assert all(combined_changepoints == pd.to_datetime(["2020-01-05", "2020-04-13"]))
