...
def test_estimate_empirical_distribution():
    ...
    df = pd.DataFrame({"x": ["a"] * 100 + ["b"] * 100, "y": list(range(100, 200)) + list(range(200, 300))})
    ...
    expected_ecdf_df_overall = pd.DataFrame(
        {"y_ecdf_quantile_summary": [(-49.75, 0.0, 49.75)], "y_min": [100], "y_mean": [199.5], "y_max": [299], "y_std": [57.87915], "y_count": [200]}
    )

    # Tests for the case with specified quantile_grid_size
    model = estimate_empirical_distribution(df=df, distribution_col="y", quantile_grid_size=0.25, quantiles=None, conditional_cols=["x"])
    ...
    model = estimate_empirical_distribution(
        df=df, distribution_col="y", quantile_grid_size=None, quantiles=[0.25, 0.50, 0.75], conditional_cols=["x"], remove_conditional_mean=True
    )
    assert_equal(model["ecdf_df"], expected_ecdf_df)
    assert_equal(model["ecdf_df_overall"], expected_ecdf_df_overall)
    ...
    for conditional_cols in [None, []]:
        model = estimate_empirical_distribution(
            df=df, distribution_col="y", quantile_grid_size=None, quantiles=[0.25, 0.50, 0.75], conditional_cols=conditional_cols, remove_conditional_mean=False
        )
