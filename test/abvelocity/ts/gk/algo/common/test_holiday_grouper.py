...
from abvelocity.ts.gk.algo.common.holiday_utils import HOLIDAY_NAME_COL, get_weekday_weekend_suffix
...
def holiday_df():
    ...
    # Exclude observed-date variants (marked with "_observed" suffix) so tests work with
    # a single canonical entry per holiday per year, matching the original test intent.
...
HOLIDAY_IMPACT_DICT = {
    "Christmas": (4, 3),  # Always 12/25.
    "Halloween": (1, 1),  # Always 10/31.
    "Independence": (4, 4),  # Always 7/4.
    "Labor": (3, 1),  # Monday.
    "MLK": (3, 1),  # Monday.
    "Memorial": (3, 1),  # Monday.
    "New Years": (3, 4),  # Always 1/1.
...
def test_expand_holiday_df_with_suffix(holiday_df):
    ...
    expanded_holiday_df = (
        ...
        .sort_values(by=[EVENT_DF_DATE_COL, EVENT_DF_LABEL_COL])
        .reset_index(drop=True)
    )

    pd.testing.assert_frame_equal(expanded_holiday_df, holiday_df, check_dtype=False)
    ...
    assert "Christmas_minus_1" in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    assert "New Years_plus_2" in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    ...
    # Spot checks a few events are being correctly added.
    assert "Christmas_minus_4" in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    assert "New Years_plus_4" in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    ...
    assert "Veterans_minus_8" in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    assert "Veterans_plus_1" not in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    assert "New Years_plus_4" in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    assert "New Years_minus_8" not in expanded_holiday_df[EVENT_DF_LABEL_COL].tolist()
    ...
    # Checks an instance where New Years falls on Friday.
    idx = expanded_holiday_df["event_name"].str.contains("New Years_WD_plus_1_WE")
    ...
    # Checks an instance where Christmas falls on Sunday and is observed on Monday.
    idx = expanded_holiday_df["event_name"].str.contains("Christmas_WE_plus_1_WD")
    ...
    # Checks an instance where Labor always fall on Monday for all years.
    idx = expanded_holiday_df["event_name"].str.contains("Labor_WD_minus_1_WE")
    assert idx.sum() == 7
...
def test_group_holidays(daily_df, holiday_df):
    ...
    for key in ["score_result", "score_result_avg"]:
        assert new_result_dict[key] == result_dict[key]

    # Grouping results changed.
    for key in ["daily_event_df_dict_with_score", "daily_event_df_dict", "kde_cutoffs", "kde_res", "kde_plot"]:
