...
import numpy as np
import pytest
from abvelocity.ts.gk.algo.common.l1_quantile_regression import QuantileRegression, l1_quantile_regression, ordinary_quantile_regression
...
def test_ordinary_quantile_regression(data):
    ...
    coef = ordinary_quantile_regression(
        x=np.concatenate([data["x"], np.ones([len(data["x"]), 1])], axis=1), y=data["y"], q=0.1, sample_weight=np.ones(len(data["y"])), max_iter=200, tol=1e-3
    )
    pred = np.concatenate([data["x"], np.ones([len(data["x"]), 1])], axis=1) @ coef
    coef1 = ordinary_quantile_regression(
        x=np.concatenate([data["x"], np.ones([len(data["x"]), 1])], axis=1), y=data["y"], q=0.9, sample_weight=np.ones(len(data["y"])), max_iter=200, tol=1e-3
    )
...
def test_l1_quantile_regression(data):
    ...
    coef = l1_quantile_regression(
        x=data["x"], y=data["y"], q=0.1, alpha=1, sample_weight=np.ones(len(data["y"])), feature_weight=np.ones(data["x"].shape[1]), include_intercept=True
    )
    pred = data["x"] @ coef["coef"] + coef["intercept"]
    coef1 = l1_quantile_regression(
        x=data["x"], y=data["y"], q=0.9, alpha=1, sample_weight=np.ones(len(data["y"])), feature_weight=np.ones(data["x"].shape[1]), include_intercept=True
    )
...
def test_quantile_regression_fit_predict_mape(data):
    ...
    qr = QuantileRegression(quantile=0.8, alpha=0, optimize_mape=True)
...
def test_errors(data):
    ...
    with pytest.raises(ValueError, match="The shapes of x and y do not match. "):
        qr = QuantileRegression()
    ...
    with pytest.raises(ValueError, match="The length of sample weight must match the number of observations"):
        qr = QuantileRegression(sample_weight=[1, 2])
        qr.fit(data["x"], data["y"])
    # Feature weight length wrong.
    with pytest.raises(ValueError, match="The length of feature weight must match the number of features"):
    ...
    with pytest.raises(ValueError, match="tol must be a positive number, found"):
