import pandas as pd
import pytest
from abvelocity.ts.gk.algo.common.holiday_utils import (
    add_shifted_events,
    get_autoreg_holiday_interactions,
    get_dow_grouped_suffix,
    get_weekday_weekend_suffix,
)
...
def daily_event_df_dict():
    ...
    daily_event_df_dict = {
        "New Years Day": pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2020-01-01", "2021-01-01", "2022-01-01"]), EVENT_DF_LABEL_COL: "event"}),
        "Christmas": pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2020-12-25", "2021-12-25", "2022-12-25"]), EVENT_DF_LABEL_COL: "event"}),
    }
...
def test_get_autoreg_holiday_interactions(daily_event_df_dict):
    ...
    interaction_terms = get_autoreg_holiday_interactions(daily_event_df_dict=daily_event_df_dict, lag_names=["y_lag1", "y_avglag_7_14_21"])
    assert interaction_terms == [
        "C(Q('events_Christmas'), levels=['', 'event']):y_lag1",
        "C(Q('events_New Years Day'), levels=['', 'event']):y_lag1",
        "C(Q('events_Christmas'), levels=['', 'event']):y_avglag_7_14_21",
...
def test_add_shifted_events(daily_event_df_dict):
    ...
    assert sorted(shifted_events_cols) == [
        "events_Christmas_1D_after",
        "events_Christmas_1D_before",
        "events_New Years Day_1D_after",
        "events_New Years Day_1D_before",
    ]
    assert sorted(drop_pred_cols) == [
        "C(Q('events_Christmas_1D_after'), levels=['', 'Christmas_1D_after'])",
        "C(Q('events_Christmas_1D_before'), levels=['', 'Christmas_1D_before'])",
