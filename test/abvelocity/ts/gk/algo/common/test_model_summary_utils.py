...
import pandas as pd
import pytest
from abvelocity.ts.gk.algo.common.model_summary_utils import (
    Bootstrapper,
    format_summary_df,
    get_info_dict_lm,
    print_summary,
...
def test_process_intercept():
    ...
    x_new, beta_new, pred_cols_new = process_intercept(x=x, beta=beta, intercept=intercept, pred_cols=pred_cols)
    ...
    x_new, beta_new, pred_cols_new = process_intercept(x=x_no_intercept, beta=beta, intercept=intercept, pred_cols=pred_cols)
    assert np.array_equal(x_new, x)
    assert np.array_equal(beta_new, np.array([1, 1]))
    assert pred_cols_new == pred_cols
    # beta does not have intercept term either
    beta_no_intercept = np.array([1])
    x_new, beta_new, pred_cols_new = process_intercept(x=x_no_intercept, beta=beta_no_intercept, intercept=intercept, pred_cols=pred_cols)
    assert np.array_equal(x_new, x)
    assert np.array_equal(beta_new, np.array([1, 1]))
    assert pred_cols_new == pred_cols
    # pred_col does not have intercept term
    pred_cols_no_intercept = ["x"]
    x_new, beta_new, pred_cols_new = process_intercept(x=x_no_intercept, beta=beta_no_intercept, intercept=intercept, pred_cols=pred_cols_no_intercept)
    assert np.array_equal(x_new, x)
    assert np.array_equal(beta_new, np.array([1, 1]))
    assert pred_cols_new == pred_cols
    # tests error
    ...
    pred_cols = ["Intercept", "x"]
    with pytest.raises(
        ValueError, match="The shape of x and beta do not match. " "x has shape \\(3, 2\\) with the intercept column, " "but beta has length 3."
    ):
        process_intercept(x=x, beta=beta, intercept=intercept, pred_cols=pred_cols)
    # tests second error
    x = np.array([[1, 2], [1, 3], [1, 4]])
    beta = np.array([0, 1])
    intercept = 1
    ...
    with pytest.raises(
        ValueError,
        match="The length of pred_cols does not match the shape of x. " "x has shape \\(3, 2\\) with the intercept column, " "but pred_cols has length 3.",
    ):
        process_intercept(x=x, beta=beta, intercept=intercept, pred_cols=pred_cols)
...
def test_get_bootstrapper():
    bootstrap_idx = Bootstrapper(sample_size=100, bootstrap_size=100, num_bootstrap=5)
    idx = []
...
def test_degenerate_columns():
    ...
    info_dict = get_info_dict_lm(x=x, y=y, beta=beta, ml_model=ml_model, fit_algorithm=fit_algorithm, pred_cols=pred_cols)
