import pandas as pd
from abvelocity.ts.gk.algo.common.col_name_utils import (
    add_category_cols,
...
def test_simplify_pred_cols():
    ...
    assert new_pred_cols == ["events_Chinese New Year-1", "str_dow_2-Tue", "cp1_2018_09_02_00", "cos1_ct1_yearly_2019_02_05_00", "no_prefix"]
...
def test_create_pred_category():
    pred_cols = ["Intercept", "ct1", "sin1_toy_yearly", "y_lag7", "ct1:sin1_toy_yearly", "C(Q('events_Chinese New Year'), levels=['', 'event'])[T.event]", "x"]
    extra_pred_cols = ["x"]
    pred_category = create_pred_category(pred_cols, extra_pred_cols, df_cols=["ts", "y", "x"])
...
def test_create_pred_category_regressor():
    ...
    pred_category = create_pred_category(
        pred_cols, extra_pred_cols, df_cols=["ts", "y", "x", "weather_wghtd_avg_cld_cvr_tot_pct_max_mid", "media_total_spend_lag4"]
    )
    expected_pred_category = {
        "intercept": ["Intercept"],
        "time_features": ["ct1", "ct1:sin1_toy_yearly"],
...
def test_filter_coef_summary():
    ...
    x = filter_coef_summary(
        coef_summary=coef_summary, pred_category=pred_category, is_intercept=True, is_time_feature=False, is_trend=True, is_interaction=False
    )
    pd.testing.assert_frame_equal(
        x,
        pd.DataFrame(
            {
                "Pred_col": ["Intercept", "cp1_2018_09_02_00"],
                "is_intercept": [1, 0],
                "is_time_feature": [0, 0],
                "is_event": [0, 0],
                "is_trend": [0, 1],
