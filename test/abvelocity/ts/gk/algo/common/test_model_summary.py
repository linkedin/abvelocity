...
def test_model_summary():
    ...
    pred_cols = ["Intercept", "ct1", "sin1_toy_yearly", "y_lag7", "ct1:sin1_toy_yearly", "C(Q('events_Chinese New Year'), levels=['', 'event'])[T.event]"]
    pred_category = {
        "intercept": ["Intercept"],
    ...
    for fit_algorithm in fit_algorithm_dict:
        ...
        if "statsmodels" in fit_algorithm or fit_algorithm == "linear":
            ...
            ml_model.fit(x, y)
        summary = ModelSummary(x=x, y=y, pred_cols=pred_cols, pred_category=pred_category, fit_algorithm=fit_algorithm, ml_model=ml_model)
        summary.__str__()
