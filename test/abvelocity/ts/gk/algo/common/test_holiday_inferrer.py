...
import pandas as pd
import pytest
from abvelocity.ts.gk.algo.common.holiday_inferrer import HolidayInferrer
from abvelocity.ts.common.constants import EVENT_DF_DATE_COL, EVENT_DF_LABEL_COL, TIME_COL, VALUE_COL
...
def test_inferrer_super_daily_data():
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        log_capture.check_present((LOGGER_NAME, "INFO", "Data frequency is greater than daily, " "holiday inferring is skipped."))
...
def test_infer_daily_data(daily_df):
    """Tests on daily data."""
    hi = HolidayInferrer()
    result = hi.infer_holidays(df=daily_df, plot=True)
    # Checks result.
    ...
    assert sorted(result["independent_holidays"]) == sorted(
        [
            ...
            ("US", "Veterans_-1"),
            ("US", "Veterans_-2"),
            ("US", "Washingtons Birthday_+1"),
            ("US", "Washingtons Birthday_+2"),
            ("US", "Washingtons Birthday_-1"),
            ("US", "Washingtons Birthday_-2"),
        ]
    )
...
def test_daily_data_diff_params(daily_df):
    ...
    # Checks result.
    assert len(result["scores"]) == 180
    assert "US_Christmas_+0" in result["scores"]
    assert "US_New Years_-3" in result["scores"]
    assert "India_Independence_-3" in result["scores"]
    assert len(result["fig"].data) == 5
    assert len(result["independent_holidays"]) == 49
    assert len(result["together_holidays_positive"]) == 9
    assert len(result["together_holidays_negative"]) == 26
    # Checks attributes.
    assert hi.df is not None
    assert hi.time_col == TIME_COL
    assert hi.value_col == VALUE_COL
    ...
    assert len(hi.ts) == len(hi.df)
    assert hi.country_holiday_df is not None
    assert len(hi.holidays) == 45
    assert len(hi.score_result) == 180
    assert len(hi.score_result_avg) == 180
    assert hi.result == result
...
def test_sub_daily_data():
    ...
    np.random.seed(123)
    df = pd.DataFrame({TIME_COL: pd.date_range("2020-01-01", freq="h", periods=24 * 365), VALUE_COL: list(range(24 * 365)) + np.random.randn(24 * 365)})
    hi = HolidayInferrer()
    result = hi.infer_holidays(df=df, plot=True)
    # Checks result.
...
def test_errors(daily_df):
    """Tests errors."""
    hi = HolidayInferrer()
    with pytest.raises(ValueError, match="Both 'post_search_days' and 'pre_search_days' must be non-negative integers."):
        hi.infer_holidays(df=daily_df, post_search_days=-1)
    with pytest.raises(ValueError, match="Both 'independent_holiday_thres' and 'together_holiday_thres' must be between " "0 and 1 \\(inclusive\\)."):
        hi.infer_holidays(df=daily_df, independent_holiday_thres=80)


def test_transform_holiday_country():
    """Tests the transformations between country-holiday strings and country-holiday tuples."""
    hi = HolidayInferrer()
    holiday_country_strs = ["US_Christmas", "UK_New Years", "France_Thanksgiving"]
    holiday_country_tuples = [("US", "Christmas"), ("UK", "New Years"), ("France", "Thanksgiving")]
...
def test_get_scores():
    ...
    df = pd.DataFrame({"ts": pd.date_range("2020-12-01", freq="D", periods=40), "y": [1] * 22 + [2, 3, 4, 3, 2, 1, 1, 2, 3, 4, 3, 2] + [1] * 6})
    result = hi.infer_holidays(df=df, use_relative_score=False)
    assert result["scores"]["US_Christmas_-2"] == [0.5]
    assert result["scores"]["US_Christmas_-1"] == [1.0]
    assert result["scores"]["US_Christmas_+0"] == [1.5]
    assert result["scores"]["US_Christmas_+1"] == [1.0]
    assert result["scores"]["US_Christmas_+2"] == [0.5]
    assert result["scores"]["US_New Years_-2"] == [0.5]
    assert result["scores"]["US_New Years_-1"] == [1.0]
    assert result["scores"]["US_New Years_+0"] == [1.5]
    assert result["scores"]["US_New Years_+1"] == [1.0]
    assert result["scores"]["US_New Years_+2"] == [0.0]

    # Tests the relative scores
    result = hi.infer_holidays(df=df, use_relative_score=True)
    assert result["scores"]["US_Christmas_-2"] == [1 / 3]
    assert result["scores"]["US_Christmas_-1"] == [0.5]
    assert result["scores"]["US_Christmas_+0"] == [0.6]
    assert result["scores"]["US_Christmas_+1"] == [0.5]
    assert result["scores"]["US_Christmas_+2"] == [1 / 3]
    assert result["scores"]["US_New Years_-2"] == [1 / 3]
    assert result["scores"]["US_New Years_-1"] == [0.5]
    assert result["scores"]["US_New Years_+0"] == [0.6]
    assert result["scores"]["US_New Years_+1"] == [0.5]
    assert result["scores"]["US_New Years_+2"] == [0.0]
...
def test_remove_observed():
    ...
    df = pd.DataFrame({"ts": pd.date_range("2020-01-01", freq="D", periods=731), "y": 1})
    ...
    # 2020-07-03 is the observed Independence for 2022-07-04.
    assert hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2020-07-03")]["holiday"].values == "Independence"
    assert len(hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2020-07-04")]["holiday"]) == 0
    # 2021-07-05 is the observed Independence for 2022-07-04.
    assert hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2021-07-05")]["holiday"].values == "Independence"
    assert len(hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2021-07-04")]["holiday"]) == 0
    # 2021-12-24 is the observed Christmas for 2021-12-25.
    assert hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2021-12-24")]["holiday"].values == "Christmas"
    assert len(hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2021-12-25")]["holiday"]) == 0
    # 2021-12-31 is the observed New Years for 2022-01-01.
    assert hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2021-12-31")]["holiday"].values == "New Years"
    assert len(hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2022-01-01")]["holiday"]) == 0
    # 2022-12-26 is the observed Christmas for 2022-12-25.
    assert hi.country_holiday_df[hi.country_holiday_df["ts"] == pd.Timestamp("2022-12-26")]["holiday"].values == "Christmas"
...
def test_cum_effect(daily_df):
    ...
    # After adding the last one, the effect should exceed the threshold.
    tog_effects = [
        hi.score_result_avg[hi._transform_country_holidays([h])[0]] for h in result["together_holidays_positive"] + result["together_holidays_negative"]
    ]
...
def test_get_single_event_df():
    ...
    holiday1 = ("US", "Christmas_-2")
    holiday2 = ("US", "Christmas_+0")
    holiday3 = ("US", "Christmas_+1")
    hi = HolidayInferrer()
    hi.infer_holidays(df=pd.DataFrame({TIME_COL: pd.date_range("2020-01-01", freq="D", periods=731), VALUE_COL: 1}))
    df1 = hi._get_event_df_for_single_event(holiday=holiday1, country_holiday_df=hi.country_holiday_df)
    df2 = hi._get_event_df_for_single_event(holiday=holiday2, country_holiday_df=hi.country_holiday_df)
    df3 = hi._get_event_df_for_single_event(holiday=holiday3, country_holiday_df=hi.country_holiday_df)
    assert df1.equals(pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2020-12-23", "2021-12-22", "2022-12-24"]), EVENT_DF_LABEL_COL: "US_Christmas_minus_2"}))
    assert df2.equals(pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2020-12-25", "2021-12-24", "2022-12-26"]), EVENT_DF_LABEL_COL: "US_Christmas"}))
    assert df3.equals(pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2020-12-26", "2021-12-25", "2022-12-27"]), EVENT_DF_LABEL_COL: "US_Christmas_plus_1"}))
...
def test_get_daily_event_dict(daily_df):
    ...
    assert len(daily_event_dict) == 30
    ...
    country_holiday_df = get_holiday_df(country_list=["US"], years=[2015, 2016])
    holidays = {
        "independent_holidays": [("US", "Christmas_+0"), ("US", "New Years_-1")],
        "together_holidays_negative": [("US", "Memorial_+2"), ("US", "Labor_+0")],
        "together_holidays_positive": [],
    }
    daily_event_dict = hi.generate_daily_event_dict(country_holiday_df=country_holiday_df, holiday_result=holidays)
    assert len(daily_event_dict) == 3
    assert "US_Christmas" in daily_event_dict
    assert "US_New Years_minus_1" in daily_event_dict
    assert "Holiday_negative_group" in daily_event_dict
    assert "Holiday_positive_group" not in daily_event_dict
    pd.testing.assert_frame_equal(
        daily_event_dict["US_Christmas"],
        pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2016-12-25", "2015-12-25"]), EVENT_DF_LABEL_COL: "US_Christmas"}),
        check_dtype=False,
    )
    assert daily_event_dict["US_New Years_minus_1"].equals(
        pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2015-12-31", "2014-12-31"]), EVENT_DF_LABEL_COL: "US_New Years_minus_1"})
    )
    assert daily_event_dict["Holiday_negative_group"].equals(
        pd.DataFrame({EVENT_DF_DATE_COL: pd.to_datetime(["2016-06-01", "2015-05-27", "2016-09-05", "2015-09-07"]), EVENT_DF_LABEL_COL: "event"})
    )
