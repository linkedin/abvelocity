import numpy as np
import pandas as pd
from abvelocity.ts.gk.algo.common.partial_regularize_regression import PartialRegularizeRegression, PartialRegularizeRegressionCV, constant_col_finder
...
def test_partial_regularize_regression():
    ...
    model = PartialRegularizeRegression(l1_index=[0, 1, 2])
    model.fit(xc, y)
    model.predict(xc)
    ...
    model = PartialRegularizeRegression(l2_index=[0, 1, 2, 3, 4, 5, 6], l2_alpha=0.1)
    model.fit(x, y)
    coef = model.coef_
    ...
    model = PartialRegularizeRegressionCV(l1_index=[1], l1_alphas=[10000])
    model.fit(x, y)
