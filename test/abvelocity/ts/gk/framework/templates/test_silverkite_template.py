...
from abvelocity.ts.gk.algo.forecast.silverkite.forecast_simple_silverkite_helper import generate_holiday_events, get_event_pred_cols
...
from abvelocity.ts.gk.framework.templates.silverkite_template import SilverkiteTemplate, apply_default_model_components, get_extra_pred_cols
...
def model_components_param(silverkite):
    return ModelComponentsParam(
        seasonality={"fs_components_df": None},
        events=None,
        changepoints={
            "changepoints_dict": {
                "method": "uniform",
...
def test_get_extra_pred_cols():
    extra_pred_cols = get_extra_pred_cols(model_components=None)
    assert extra_pred_cols is None

    extra_pred_cols = get_extra_pred_cols(model_components=ModelComponentsParam(custom={}))
    assert extra_pred_cols is None

    extra_pred_cols = get_extra_pred_cols(model_components=ModelComponentsParam(custom={"extra_pred_cols": ["p1", "p2", "p3"]}))
    assert set(extra_pred_cols) == {"p1", "p2", "p3"}

    extra_pred_cols = get_extra_pred_cols(model_components=ModelComponentsParam(custom={"extra_pred_cols": [["p1"], ["p2", "p3"], None, []]}))
    assert set(extra_pred_cols) == {"p1", "p2", "p3"}
...
def test_apply_default_model_components(model_components_param, silverkite):
    ...
    assert model_components.events == {"daily_event_df_dict": [None], "daily_event_neighbor_impact": [None], "daily_event_shifted_effect": [None]}
    assert model_components.changepoints == {
        "changepoints_dict": [None],
        "seasonality_changepoints_dict": [None],
    }
    assert model_components.autoregression == {"autoreg_dict": [None], "simulation_num": [10], "fast_simulation": [False]}
    ...
    time_properties = {"origin_for_time_vars": 2020}
    original_components = dataclasses.replace(model_components_param)  # creates a copy
    updated_components = apply_default_model_components(model_components=model_components_param, time_properties=time_properties)
    ...
    assert updated_components.events == {"daily_event_df_dict": [None], "daily_event_neighbor_impact": [None], "daily_event_shifted_effect": [None]}
    ...
    assert updated_components.autoregression == {"autoreg_dict": [None], "simulation_num": [10], "fast_simulation": [False]}
    ...
    updated_components = apply_default_model_components(model_components=model_components_param, time_properties={})
...
def test_get_regressor_cols():
    ...
    template.config.model_components_param = ModelComponentsParam(custom={})
    regressor_cols = template.get_regressor_cols()
    assert regressor_cols is None

    template.config.model_components_param = ModelComponentsParam(custom={"extra_pred_cols": ["p1", "p2", "p3", template.config.metadata_param.time_col]})
    regressor_cols = template.get_regressor_cols()
    assert set(regressor_cols) == {"p1", "p2"}

    template.config.model_components_param = ModelComponentsParam(custom={"extra_pred_cols": [["p1"], ["p2", "p3"], None, []]})
    regressor_cols = template.get_regressor_cols()
    assert set(regressor_cols) == {"p1", "p2"}
...
def test_get_silverkite_hyperparameter_grid(model_components_param, silverkite):
    ...
    assert_equal(hyperparameter_grid, expected_grid, ignore_keys={"estimator__silverkite": None})
    ...
    expected_grid = {
        ...
        "estimator__regression_weight_col": [None],
        "estimator__remove_intercept": [False],
        "estimator__uncertainty_dict": [{"uncertainty_method": "simple_conditional_residuals"}],
    }
    ...
    expected_grid = [updated_grid1, expected_grid, updated_grid2, expected_grid]
    assert_equal(hyperparameter_grid, expected_grid)
...
def test_apply_template_decorator():
    ...
    df = data["df"]
    template = SilverkiteTemplate()
    with pytest.raises(ValueError, match="SilverkiteTemplate only supports config.model_template='SK', found 'PROPHET'"):
        template.apply_template_for_pipeline_params(df=df, config=ForecastConfig(model_template="PROPHET"))
...
def test_silverkite_template_custom(model_components_param):
    ...
    metadata = MetadataParam(
        time_col=time_col, value_col=value_col, freq="H", date_format="%Y-%m-%d-%H", train_end_date=datetime.datetime(2019, 7, 1), anomaly_info=anomaly_info
    )
...
def test_run_template_2():
    ...
    data = generate_df_with_reg_for_tests(freq="D", periods=400, remove_extra_cols=True, mask_test_actuals=True)
    ...
    daily_event_df_dict = generate_holiday_events(
        countries=["UnitedStates"], holidays_to_model_separately=["New Years"], year_start=2017, year_end=2022, pre_num=2, post_num=2
    )
    ...
    model_components = ModelComponentsParam(
        ...
        custom={
            ...
            "fit_algorithm_dict": {"fit_algorithm": "ridge", "fit_algorithm_params": {"cv": 2}},
...
def test_run_template_3():
    ...
    model_components = ModelComponentsParam(seasonality=None, events=None, changepoints=None, autoregression=None, uncertainty={"uncertainty_dict": None})
    config = ForecastConfig(
        model_template=ModelTemplateEnum.SK.name,
        forecast_horizon=forecast_horizon,
        coverage=0.9,
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(3.37, rel=1e-1)
        check_forecast_pipeline_result(
            result, coverage=0.9, strategy=None, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=False
        )
...
def test_run_template_4():
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(3.13, rel=1e-1)
        check_forecast_pipeline_result(
            result, coverage=0.9, strategy=None, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=False
        )
...
def test_run_template_5():
    ...
    model_components = ModelComponentsParam(
        custom=dict(fit_algorithm_dict=dict(fit_algorithm="ridge"), extra_pred_cols=["ct2"]),
        autoregression=dict(autoreg_dict=dict(lag_dict=dict(orders=[1])), fast_simulation=True),
        uncertainty=dict(uncertainty_dict=None),
    )
    config = ForecastConfig(
        model_template=ModelTemplateEnum.SK.name,
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(2.79, rel=1e-1)
        check_forecast_pipeline_result(
            result, coverage=0.9, strategy=None, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=False
        )
...
def test_run_template_6():
    ...
    model_components = ModelComponentsParam(
        custom=dict(fit_algorithm_dict=dict(fit_algorithm="ridge"), extra_pred_cols=reg_cols),
        autoregression=dict(autoreg_dict=dict(lag_dict=dict(orders=[1]))),
        lagged_regressors={
            "lagged_regressor_dict": [{"regressor_categ": {"lag_dict": {"orders": [1, 5]}}}, {"regressor_categ": {"lag_dict": {"orders": [5]}}}]
        },
        uncertainty=dict(uncertainty_dict=None),
    )
    config = ForecastConfig(
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(3.26, rel=1e-1)
        check_forecast_pipeline_result(
            result, coverage=0.9, strategy=None, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=False
        )
        ...
        expected_pred_cols = {"regressor1", "y_lag1", "regressor_categ_lag5"}
        expected_x_mat_cols = {"regressor1", "y_lag1", "regressor_categ_lag5[T.c2]", "regressor_categ_lag5[T.c2]"}
