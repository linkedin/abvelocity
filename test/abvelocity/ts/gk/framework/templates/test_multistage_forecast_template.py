...
import numpy as np
import pytest
from abvelocity.ts.common.constants import LOGGER_NAME, PREDICTED_COL, PREDICTED_LOWER_COL, PREDICTED_UPPER_COL, TIME_COL, VALUE_COL
...
from abvelocity.ts.gk.framework.templates.multistage_forecast_template import MultistageForecastTemplate
from abvelocity.ts.gk.framework.templates.multistage_forecast_template_config import SILVERKITE_TWO_STAGE, MultistageForecastTemplateConfig
...
from abvelocity.ts.gk.sklearn.uncertainty.uncertainty_methods import UncertaintyMethodEnum
...
@pytest.fixture
def df():
    df = generate_df_for_tests(freq="H", periods=24 * 7 * 8, train_start_date=datetime.datetime(2018, 1, 1), conti_year_origin=2018)["df"]
...
@pytest.fixture
def df_daily():
    df = generate_df_for_tests(freq="D", periods=1000, train_start_date=datetime.datetime(2018, 1, 1), conti_year_origin=2018)["df"]
    return df
    ...
    configs = [
        MultistageForecastTemplateConfig(
            ...
            model_components=ModelComponentsParam(
                ...
                changepoints={"changepoints_dict": None, "seasonality_changepoints_dict": None},
...
def forecast_config(multistage_forecast_configs):
    forecast_config = ForecastConfig(
        ...
        forecast_horizon=12,
        metadata_param=MetadataParam(time_col=TIME_COL, value_col=VALUE_COL, freq="H"),
        model_components_param=ModelComponentsParam(custom=dict(multistage_forecast_configs=multistage_forecast_configs)),
...
def test_get_regressor_cols(df, forecast_config):
    ...
    forecast_config.model_components_param.custom["multistage_forecast_configs"][0].model_components.regressors["regressor_cols"] = ["reg1"]
    forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_components.regressors["regressor_cols"] = ["reg2"]
...
def test_get_lagged_regressor_info(df, forecast_config):
    ...
    template.df = df
    template.config = forecast_config
    forecast_config.model_components_param.custom["multistage_forecast_configs"][0].model_components.lagged_regressors["lagged_regressor_dict"] = [
        {"reg1": {"lag_dict": {"orders": [12]}, "series_na_fill_func": lambda s: s.bfill().ffill()}}
    ]
    forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_components.lagged_regressors["lagged_regressor_dict"] = [
        {"reg2": {"lag_dict": {"orders": [12]}, "series_na_fill_func": lambda s: s.bfill().ffill()}}
    ]
    lagged_regressor_info = template.get_lagged_regressor_info()
    assert lagged_regressor_info == dict(lagged_regressor_cols=["reg1", "reg2"], overall_min_lag_order=12.0, overall_max_lag_order=12.0)
...
def test_get_hyperparameter_grid(df, forecast_config):
    ...
    with pytest.raises(ValueError, match="Forecast config must be provided"):
    ...
    forecast_config.model_components_param.custom["multistage_forecast_configs"][0].model_components.seasonality["weekly_seasonality"] = [1, 2]
    forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_components.seasonality["daily_seasonality"] = [10, 12]
    template.config = forecast_config
...
def test_get_hyperparameter_grid_same_template(df, forecast_config):
    ...
    forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_components.seasonality["weekly_seasonality"] = 5
    # Removes the daily seasonality specification.
    del forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_components.seasonality["daily_seasonality"]
    template.config = forecast_config
    hyperparameter_grid = template.get_hyperparameter_grid()
...
def test_get_hyperparameter_grid_different_template(df, forecast_config):
    ...
    forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_template = "SILVERKITE_EMPTY"
    # Sets weekly seasonality to 5.
    forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_components.seasonality["weekly_seasonality"] = 5
    # Removes the daily seasonality specification.
    del forecast_config.model_components_param.custom["multistage_forecast_configs"][1].model_components.seasonality["daily_seasonality"]
...
def test_get_multistage_forecast_configs_override(df, forecast_config):
    ...
    default_model_components = template._MultistageForecastTemplate__get_default_model_components(forecast_config.model_template)
...
def test_get_estimators_and_params_from_template_configs(df, forecast_config):
    ...
    estimator_list, estimator_params_list = template._MultistageForecastTemplate__get_estimators_and_params_from_template_configs(new_configs=new_configs)
...
def test_flatten_estimator_params_list():
    template = MultistageForecastTemplate()
    x = [{"estimator__a": [1], "estimator__b": [2, 3]}, {"estimator__c": [4, 5]}]
    flattened_params = template._MultistageForecastTemplate__flatten_estimator_params_list(estimator_params_list=x)
    assert flattened_params == [[{"a": 1, "b": 2}, {"c": 4}], [{"a": 1, "b": 2}, {"c": 5}], [{"a": 1, "b": 3}, {"c": 4}], [{"a": 1, "b": 3}, {"c": 5}]]


def test_multistage_forecast_model_template(df, forecast_config):
    forecaster = Forecaster()
    forecast_result = forecaster.run_forecast_config(df=df, config=forecast_config)
...
def test_multistage_forecast_model_template_with_regressor(df, forecast_config):
    forecaster = Forecaster()
    forecast_config.model_components_param.custom["multistage_forecast_configs"][0].model_components.regressors["regressor_cols"] = ["regressor"]
    df.iloc[-12:, 1] = np.nan
    forecast_result = forecaster.run_forecast_config(df=df, config=forecast_config)
    assert forecast_result.backtest is not None
    assert forecast_result.grid_search is not None
...
def test_multistage_forecast_model_template_with_lagged_regressor(df, forecast_config):
    forecaster = Forecaster()
    forecast_config.model_components_param.custom["multistage_forecast_configs"][0].model_components.lagged_regressors["lagged_regressor_dict"] = [
        {"regressor": {"lag_dict": {"orders": [12]}, "series_na_fill_func": lambda s: s.bfill().ffill()}}
    ]
    forecast_result = forecaster.run_forecast_config(df=df, config=forecast_config)
    assert forecast_result.backtest is not None
    assert forecast_result.grid_search is not None
...
def test_errors(df, forecast_config):
    ...
    forecast_config.model_template = "MULTISTAGE_EMPTY"
    template.config = forecast_config
    with pytest.raises(ValueError, match="``MULTISTAGE_EMPTY`` can not be used without over"):
        template.get_hyperparameter_grid()
    ...
    template.config = forecast_config
    with pytest.raises(ValueError, match="The ``multistage_forecast_configs`` parameter must be a list of"):
        template.get_hyperparameter_grid()


def test_get_default_model_components():
    template = MultistageForecastTemplate()
    assert template._MultistageForecastTemplate__get_default_model_components("SILVERKITE_TWO_STAGE") == SILVERKITE_TWO_STAGE
    with pytest.raises(ValueError, match="The template name "):
        template._MultistageForecastTemplate__get_default_model_components("some_template")


def test_get_template_class():
    template = MultistageForecastTemplate()
    assert template._MultistageForecastTemplate__get_template_class(ForecastConfig(model_template="SILVERKITE")) == SimpleSilverkiteTemplate
    with pytest.raises(ValueError, match="Currently Multistage Forecast only supports"):
        template._MultistageForecastTemplate__get_template_class(ForecastConfig(model_template="DAILY_CP_NONE"))
...
def test_uncertainty(df, forecast_config):
    ...
    assert forecast_result.model[-1].uncertainty_model.UNCERTAINTY_METHOD == UncertaintyMethodEnum.simple_conditional_residuals.name
    last_interval_width_99 = forecast_result.forecast.df[PREDICTED_UPPER_COL].iloc[-1] - forecast_result.forecast.df[PREDICTED_LOWER_COL].iloc[-1]

    # Tests coverage and uncertainty, there is uncertainty.
    forecast_config.model_components_param.uncertainty = dict(
        uncertainty_dict=dict(uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(conditional_cols=["dow"]))
    )
    forecaster = Forecaster()
    forecast_result = forecaster.run_forecast_config(df=df, config=forecast_config)
    assert PREDICTED_LOWER_COL in forecast_result.backtest.df_test
    ...
    last_day_interval_width_99 = forecast_result.forecast.df[PREDICTED_UPPER_COL].iloc[-1] - forecast_result.forecast.df[PREDICTED_LOWER_COL].iloc[-1]
    second_last_day_interval_width_99 = forecast_result.forecast.df[PREDICTED_UPPER_COL].iloc[-25] - forecast_result.forecast.df[PREDICTED_LOWER_COL].iloc[-25]
    ...
    forecast_config.model_components_param.uncertainty = dict(
        uncertainty_dict=dict(uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict())
    )
    forecaster = Forecaster()
    forecast_result = forecaster.run_forecast_config(df=df, config=forecast_config)
    assert PREDICTED_LOWER_COL in forecast_result.backtest.df_test
    ...
    last_interval_width_95 = forecast_result.forecast.df[PREDICTED_UPPER_COL].iloc[-1] - forecast_result.forecast.df[PREDICTED_LOWER_COL].iloc[-1]
...
def test_uncertainty_fail(df, forecast_config):
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        forecast_config.coverage = 0.95
        forecast_config.model_components_param.uncertainty = dict(
            uncertainty_dict=dict(uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(conditional_cols=["dowww"]))
        )
        forecaster = Forecaster()
        forecast_result = forecaster.run_forecast_config(df=df, config=forecast_config)
        ...
        assert (
            ...
            "The following conditional columns are not found in `train_df`: ['dowww'].",
...
def test_silverkite_wow(df_daily):
    ...
    config = ForecastConfig(
        ...
        forecast_horizon=7,
        metadata_param=MetadataParam(freq="D"),
            ...
            custom=dict(
                multistage_forecast_configs=[
                    MultistageForecastTemplateConfig(
                        ...
                        model_components=ModelComponentsParam(seasonality=dict(quarterly_seasonality=False)),
                    ),
                    MultistageForecastTemplateConfig(
                        train_length="500D", model_template="LAG_BASED", model_components=ModelComponentsParam(custom=dict(lags=[1, 2, 3]))
                    ),
                ]
            )
        ),
        evaluation_period_param=EvaluationPeriodParam(cv_max_splits=1, test_horizon=7),
    )
    forecast_result = forecaster.run_forecast_config(df=df_daily, config=config)
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        log_capture.check_present(
            (LOGGER_NAME, "INFO", "This is a lag based forecast model that uses lags '[1, 2, 3]', " "with unit 'week' and aggregation function 'mean'.")
        )
