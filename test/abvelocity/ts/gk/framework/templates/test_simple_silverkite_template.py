...
import pandas as pd
import pytest
from abvelocity.ts.gk.algo.forecast.silverkite.constants.silverkite_constant import SilverkiteConstant
from abvelocity.ts.gk.algo.forecast.silverkite.constants.silverkite_holiday import SilverkiteHoliday
from abvelocity.ts.gk.algo.forecast.silverkite.forecast_simple_silverkite import SimpleSilverkiteForecast
from abvelocity.ts.common.constants import ADJUSTMENT_DELTA_COL, END_TIME_COL, METRIC_ID_COL, PREDICTED_COL, START_TIME_COL, TIME_COL, VALUE_COL
from abvelocity.ts.common.data_loader import DataLoader
from abvelocity.ts.common.evaluation import EvaluationMetricEnum
from abvelocity.ts.common.logging import LOGGER_NAME
...
from abvelocity.ts.common.time_properties_forecast import get_forecast_time_properties
from abvelocity.ts.gk.framework.constants import COMPUTATION_N_JOBS, CV_REPORT_METRICS_ALL
from abvelocity.ts.gk.framework.templates.autogen.forecast_config import (
    ComputationParam,
...
from abvelocity.ts.gk.framework.templates.forecaster import Forecaster
from abvelocity.ts.gk.framework.templates.model_templates import ModelTemplateEnum
from abvelocity.ts.gk.framework.templates.simple_silverkite_template import SimpleSilverkiteTemplate
from abvelocity.ts.gk.framework.templates.simple_silverkite_template_config import SILVERKITE_COMPONENT_KEYWORDS, SimpleSilverkiteTemplateOptions
from abvelocity.ts.gk.framework.utils.framework_testing_utils import assert_basic_pipeline_equal, check_forecast_pipeline_result
from abvelocity.ts.gk.framework.utils.result_summary import summarize_grid_search_results
...
def test_template_name_from_dataclass():
    ...
    assert name == "DAILY_SEAS_LT_GR_LINEAR_CP_NONE_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO"
    name = SimpleSilverkiteTemplateOptions(freq=SILVERKITE_COMPONENT_KEYWORDS.FREQ.value.HOURLY, cp=SILVERKITE_COMPONENT_KEYWORDS.CP.value.LT)
    name = sst._SimpleSilverkiteTemplate__template_name_from_dataclass(name)
    assert name == "HOURLY_SEAS_LT_GR_LINEAR_CP_LT_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO"
...
def test_decode_single_template():
    ...
    assert (
        sst._SimpleSilverkiteTemplate__decode_single_template("HOURLY_AR_AUTO_CP_LT_FEASET_AUTO_HOL_NONE_GR_LINEAR_ALGO_RIDGE_SEAS_NM")
        == "HOURLY_SEAS_NM_GR_LINEAR_CP_LT_HOL_NONE_FEASET_AUTO_ALGO_RIDGE_AR_AUTO_DSI_AUTO_WSI_AUTO"
    )
    # Recognizes all keywords 1.
    assert (
        sst._SimpleSilverkiteTemplate__decode_single_template("HOURLY_SEAS_NM_GR_LINEAR_CP_LT_HOL_NONE_FEASET_AUTO_ALGO_RIDGE_AR_AUTO_DSI_AUTO_WSI_AUTO")
        == "HOURLY_SEAS_NM_GR_LINEAR_CP_LT_HOL_NONE_FEASET_AUTO_ALGO_RIDGE_AR_AUTO_DSI_AUTO_WSI_AUTO"
    )
    # Recognizes all keywords 2.
    assert (
        sst._SimpleSilverkiteTemplate__decode_single_template("HOURLY_SEAS_NM_GR_LINEAR_CP_LT_HOL_NONE_FEASET_AUTO_ALGO_RIDGE_AR_AUTO_DSI_OFF_WSI_OFF")
    ...
    assert (
        sst._SimpleSilverkiteTemplate__decode_single_template("DAILY_AR_AUTO_CP_LT_FEASET_AUTO_HOL_NONE_GR_LINEAR")
        == "DAILY_SEAS_LT_GR_LINEAR_CP_LT_HOL_NONE_FEASET_AUTO_ALGO_RIDGE_AR_AUTO_DSI_AUTO_WSI_AUTO"
    )
    # Provides frequency only.
    assert (
        sst._SimpleSilverkiteTemplate__decode_single_template("DAILY")
        == "DAILY_SEAS_LT_GR_LINEAR_CP_NONE_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO"
    )
    # Error: not recognized as a single template.
    with pytest.raises(ValueError, match="Template not recognized. Please make sure the frequency belongs to \\['HOURLY', 'DAILY', 'WEEKLY'\\]."):
    ...
    # ERROR: no keyword.
    with pytest.raises(ValueError, match="No keyword found in the template name. Please make sure your template name consists of keyword \\+ value pairs."):
        sst._SimpleSilverkiteTemplate__decode_single_template("WEEKLY_LT")
    # Error: wrong value without keyword.
    with pytest.raises(ValueError, match="ONN is not recognized for AR. The valid values are \\['OFF', 'AUTO', 'DEFAULT'\\]."):
        sst._SimpleSilverkiteTemplate__decode_single_template("WEEKLY_AR_ONN")
    # Error: wrong format1.
    with pytest.raises(
        ValueError, match="CP is not recognized for SEAS. The valid values are \\['LT', 'NM', 'HV', 'LTQM', 'NMQM', 'HVQM', 'NONE', 'DEFAULT'\\]."
    ):
        sst._SimpleSilverkiteTemplate__decode_single_template("WEEKLY_SEAS_CP")
    # Error: wrong format2.
    with pytest.raises(ValueError, match="Template name is not valid. It must be either keyword \\+ value or purely values."):
    ...
    with pytest.warns(UserWarning) as record:
        ...
        assert (
            "The following words are not used because they are neither a component keyword"
    ...
    with pytest.warns(UserWarning) as record:
        ...
        assert (
            "The following component keywords are not found in the template name, "
...
def test_check_template_type():
    ...
    with pytest.raises(
        ValueError,
        match="The template name SILVERKITE_WEEKLY_100 is not recognized. It must be 'SILVERKITE', "
        "'SILVERKITE_DAILY_1_CONFIG_1', 'SILVERKITE_DAILY_1_CONFIG_2', 'SILVERKITE_DAILY_1_CONFIG_3', 'SILVERKITE_EMPTY', "
        "a `SimpleSilverkiteTemplateOptions` data class, of the type"
        " '\\{FREQ\\}_SEAS_\\{VAL\\}_GR_\\{VAL\\}_CP_\\{VAL\\}_HOL_\\{VAL\\}_FEASET_\\{VAL\\}_ALGO_\\{VAL\\}_AR_\\{VAL\\}' or"
        " belong to \\['SILVERKITE_DAILY_1', 'SILVERKITE_DAILY_90', 'SILVERKITE_WEEKLY', 'SILVERKITE_HOURLY_1', 'SILVERKITE_HOURLY_24', "
...
def test_get_name_string_from_model_template():
    ...
    assert name_string == ["DAILY_SEAS_NONE_GR_NONE_CP_NONE_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_OFF_WSI_OFF"]
    name_string = sst._SimpleSilverkiteTemplate__get_name_string_from_model_template("DAILY_SEAS_HV_CP_HV")
    assert name_string == ["DAILY_SEAS_HV_GR_LINEAR_CP_HV_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO"]
    name_string = sst._SimpleSilverkiteTemplate__get_name_string_from_model_template(
        SimpleSilverkiteTemplateOptions(
            freq=SILVERKITE_COMPONENT_KEYWORDS.FREQ.value.DAILY, seas=SILVERKITE_COMPONENT_KEYWORDS.SEAS.value.HV, cp=SILVERKITE_COMPONENT_KEYWORDS.CP.value.HV
        )
    )
    assert name_string == ["DAILY_SEAS_HV_GR_LINEAR_CP_HV_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO"]
    ...
    assert name_strings == [
        ...
        "SILVERKITE_DAILY_1_CONFIG_2",
        "SILVERKITE_DAILY_1_CONFIG_3",
        "DAILY_SEAS_NONE_GR_NONE_CP_NONE_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_OFF_WSI_OFF",
        "WEEKLY_SEAS_NM_GR_LINEAR_CP_NONE_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO",
        "WEEKLY_SEAS_NM_GR_LINEAR_CP_LT_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO",
        ...
        "DAILY_SEAS_NONE_GR_NONE_CP_NONE_HOL_NONE_FEASET_OFF_ALGO_LINEAR_AR_OFF_DSI_OFF_WSI_OFF",
        "DAILY_SEAS_HV_GR_LINEAR_CP_HV_HOL_NONE_FEASET_OFF_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO",
    ]
...
def test_get_single_model_components_param_from_template():
    ...
    assert model_components == ModelComponentsParam(
        ...
        lagged_regressors={"lagged_regressor_dict": None},
    ...
    assert model_components == ModelComponentsParam(
        ...
        autoregression={"autoreg_dict": "auto", "simulation_num": 10, "fast_simulation": False},
        regressors={"regressor_cols": []},
        lagged_regressors={"lagged_regressor_dict": None},
        uncertainty={"uncertainty_dict": None},
        custom={
    ...
    assert model_components == ModelComponentsParam(
        ...
        changepoints={"auto_growth": False, "changepoints_dict": None, "seasonality_changepoints_dict": None},
        autoregression={"autoreg_dict": None, "simulation_num": 10, "fast_simulation": False},
        regressors={"regressor_cols": []},
        lagged_regressors={"lagged_regressor_dict": None},
        uncertainty={"uncertainty_dict": None},
        custom={
...
def test_get_model_components_from_model_template(silverkite):
    ...
    assert model_components.growth == {"growth_term": "linear"}
    ...
    assert model_components.autoregression == {"autoreg_dict": "auto", "simulation_num": 10, "fast_simulation": False}
    assert model_components.regressors == {"regressor_cols": []}
    assert model_components.lagged_regressors == {"lagged_regressor_dict": None}
    assert model_components.uncertainty == {"uncertainty_dict": None}
    assert model_components.custom == {
        "fit_algorithm_dict": {"fit_algorithm": "ridge", "fit_algorithm_params": None},
    ...
    model_components = ModelComponentsParam(
        seasonality={"auto_seasonality": False, "yearly_seasonality": True, "weekly_seasonality": False},
        ...
        events={
            ...
            "holiday_pre_post_num_dict": {"New Years": (7, 3)},
        ...
        changepoints={
            ...
            "seasonality_changepoints_dict": {"regularization_strength": 0.5},
        },
        autoregression={"autoreg_dict": {"dummy_key": "test_value"}, "simulation_num": 10, "fast_simulation": False},
        regressors={"regressor_cols": ["r1", "r2"]},
        lagged_regressors={"lagged_regressor_dict": {"dummy_key": "test_value"}},
        uncertainty={
    ...
    updated_components = sst._SimpleSilverkiteTemplate__get_model_components_and_override_from_model_template(
        template="SILVERKITE", model_components=model_components
    )[0]
    assert original_components == model_components  # not mutated by the function
    assert updated_components.growth == {"growth_term": "quadratic"}
    ...
    assert updated_components.autoregression == {"autoreg_dict": {"dummy_key": "test_value"}, "simulation_num": 10, "fast_simulation": False}
    assert updated_components.regressors == {"regressor_cols": ["r1", "r2"]}
    assert updated_components.lagged_regressors == {"lagged_regressor_dict": {"dummy_key": "test_value"}}
    assert updated_components.uncertainty == {"uncertainty_dict": "auto"}
    ...
    model_components = ModelComponentsParam(
        changepoints={"changepoints_dict": {"method": "auto", "yearly_seasonality_order": 10, "no_changepoint_proportion_from_end": 0.2}}
    )
    updated_components = sst._SimpleSilverkiteTemplate__get_model_components_and_override_from_model_template(
        template="SILVERKITE", model_components=model_components
    )[0]
    assert updated_components.changepoints == {
        "auto_growth": False,
        "changepoints_dict": {"method": "auto", "yearly_seasonality_order": 10, "no_changepoint_proportion_from_end": 0.2},
        "seasonality_changepoints_dict": None,
    }
...
def test_override_model_components(silverkite):
    ...
    model_components = ModelComponentsParam(
        ...
        hyperparameter_override={"estimator__silverkite": silverkite, "estimator__daily_seasonality": 10},
    )
    new_model_components = sst._SimpleSilverkiteTemplate__override_model_components(
        default_model_components=default_model_components, model_components=model_components
    )
...
def test_get_model_components_and_override_from_model_template_single():
    ...
    model_components = sst._SimpleSilverkiteTemplate__get_model_components_and_override_from_model_template(
        ...
        model_components=ModelComponentsParam(seasonality={"daily_seasonality": 12}, regressors={"regressor_cols": ["x"]}),
    )
...
def test_get_model_components_and_override_from_model_template_multi():
    ...
    sst = SimpleSilverkiteTemplate()
    model_components = sst._SimpleSilverkiteTemplate__get_model_components_and_override_from_model_template(
        template="SILVERKITE_DAILY_90", model_components=ModelComponentsParam(seasonality={"daily_seasonality": 12}, regressors={"regressor_cols": ["x"]})
    )
    # The model_components is used to override all single templates in `SILVERKITE_DAILY_90`.
    assert all([single_model_components.seasonality["daily_seasonality"] == 12 for single_model_components in model_components])
    assert all([single_model_components.regressors == {"regressor_cols": ["x"]} for single_model_components in model_components])
...
def test_get_regressor_cols():
    ...
    # single list
    model_components = ModelComponentsParam(regressors={"regressor_cols": None})
    template.config.model_components_param = model_components
    regressor_cols = template.get_regressor_cols()
    assert regressor_cols is None

    # single list
    model_components = ModelComponentsParam(regressors={"regressor_cols": ["c1", "c2", "c3"]})
    template.config.model_components_param = model_components
    regressor_cols = template.get_regressor_cols()
...
def test_get_lagged_regressor_info():
    ...
    template = SimpleSilverkiteTemplate()
    template.config = template.apply_forecast_config_defaults()
    expected_lagged_regressor_info = {"lagged_regressor_cols": None, "overall_min_lag_order": None, "overall_max_lag_order": None}
        ...
        lagged_regressors={
            "lagged_regressor_dict": [
                ...
                },
                {
                    "regressor_bool": {
                        "lag_dict": {"orders": [1]},
                        "agg_lag_dict": {"orders_list": [[7, 7 * 2]], "interval_list": [(8, 7 * 2)]},
                        "series_na_fill_func": lambda s: s.bfill().ffill(),
                    }
                },
            ]
...
def test_get_simple_silverkite_hyperparameter_grid(silverkite):
    ...
    model_components = ModelComponentsParam(
        ...
        regressors={"regressor_cols": ["reg1", "reg2"]},
        custom={"extra_pred_cols": ["some_column"]},
        hyperparameter_override={
            "input__response__null__max_frac": 0.1,
            "estimator__silverkite": silverkite,
    ...
    updated_grid["estimator__holiday_pre_post_num_dict"] = [{"New Years": (7, 3)}]
    ...
    expected_grid = [updated_grid1, expected_grid, updated_grid2]
    assert_equal(hyperparameter_grid, expected_grid)

    # Tests list of ``ModelComponentsParam``.
    model_components = [
        ModelComponentsParam(
            seasonality={"yearly_seasonality": 5}, hyperparameter_override=[{"estimator__daily_seasonality": 2}, {"estimator__daily_seasonality": 3}]
        ),
        ModelComponentsParam(seasonality={"yearly_seasonality": 6}),
    ]
    template = SimpleSilverkiteTemplate()
    template.config = template.apply_forecast_config_defaults()
...
def test_simple_silverkite_template_custom():
    ...
    data = generate_df_with_reg_for_tests(freq="H", periods=300 * 24, remove_extra_cols=True, mask_test_actuals=True)
    df = data["df"]
    ...
    anomaly_df = pd.DataFrame(
        {START_TIME_COL: [df[time_col].min()], END_TIME_COL: [df[time_col].max()], ADJUSTMENT_DELTA_COL: [adjustment_size], METRIC_ID_COL: [value_col]}
    )
    anomaly_info = {
        "value_col": VALUE_COL,
        ...
        "filter_by_dict": {METRIC_ID_COL: VALUE_COL},
    ...
    evaluation_metric = EvaluationMetricParam(
        ...
        agg_periods=24,
        agg_func=np.max,
        null_model_params={"strategy": "quantile", "constant": None, "quantile": 0.8},
    ...
    model_components = ModelComponentsParam(
        ...
        autoregression={"autoreg_dict": {"dummy_key": "test_value"}},
        regressors={"regressor_cols": ["regressor1", "regressor2", "regressor3", "regressor_bool", "regressor_categ"]},
        lagged_regressors={"lagged_regressor_dict": {"dummy_key": "test_value"}},
        uncertainty={
            "uncertainty_dict": "auto",
        },
    ...
    computation = ComputationParam(hyperparameter_budget=10, n_jobs=None, verbose=1)
    forecast_horizon = 20
    coverage = 0.7
    silverkite_constants = MySilverkiteConstant()
    silverkite = SimpleSilverkiteForecast(constants=silverkite_constants)
    ...
    expected_params = dict(
        ...
        weight_col=template.weight_col,
        # model
...
def test_run_template_1():
    ...
    with warnings.catch_warnings():
        ...
        assert result.backtest.test_evaluation[rmse] == pytest.approx(2.120, rel=0.05)
        assert result.backtest.test_evaluation[q80] == pytest.approx(0.863, rel=0.05)
        assert result.forecast.train_evaluation[rmse] == pytest.approx(1.975, rel=0.05)
        assert result.forecast.train_evaluation[q80] == pytest.approx(0.786, rel=0.05)
        check_forecast_pipeline_result(
            result, coverage=None, strategy=None, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=False
        )
...
def test_run_template_2():
    ...
    model_components = ModelComponentsParam(custom={"fit_algorithm_dict": {"fit_algorithm": "sgd", "fit_algorithm_params": {"random_state": 1234}}})

    data = generate_df_for_tests(freq="D", periods=90)
    df = data["train_df"]
    forecast_horizon = data["test_df"].shape[0]
    coverage = 0.90
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        q80 = EvaluationMetricEnum.Quantile80.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(1.968, rel=0.15)
        assert result.backtest.test_evaluation[q80] == pytest.approx(0.633, rel=0.15)
        assert result.forecast.train_evaluation[rmse] == pytest.approx(1.953, rel=0.15)
        assert result.forecast.train_evaluation[q80] == pytest.approx(0.784, rel=0.15)
        check_forecast_pipeline_result(
            result, coverage=coverage, strategy=None, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=False
        )
...
def test_run_template_3():
    ...
    - weekly data
    """
    data = generate_df_with_reg_for_tests(freq="W-MON", periods=140, remove_extra_cols=True, mask_test_actuals=True)
    reg_cols = ["regressor1", "regressor2", "regressor_categ"]
    keep_cols = [TIME_COL, VALUE_COL] + reg_cols
    df = data["df"][keep_cols]
    metric = EvaluationMetricEnum.MeanAbsoluteError
    evaluation_metric = EvaluationMetricParam(
        cv_selection_metric=metric.name, agg_periods=7, agg_func=np.max, null_model_params={"strategy": "quantile", "constant": None, "quantile": 0.5}
    )
    evaluation_period = EvaluationPeriodParam(
        test_horizon=10,
    ...
    model_components = ModelComponentsParam(
        regressors={"regressor_cols": reg_cols}, custom={"fit_algorithm_dict": {"fit_algorithm": "ridge", "fit_algorithm_params": {"cv": 2}}}
    )
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        q80 = EvaluationMetricEnum.Quantile80.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(3.299, rel=0.15)
        assert result.backtest.test_evaluation[q80] == pytest.approx(1.155, rel=0.15)
        assert result.forecast.train_evaluation[rmse] == pytest.approx(1.782, rel=0.15)
        assert result.forecast.train_evaluation[q80] == pytest.approx(0.746, rel=0.15)
        check_forecast_pipeline_result(result, coverage=coverage, strategy=None, score_func=metric.name, greater_is_better=False)
    ...
    with pytest.raises((ValueError, KeyError)) as exception_info:
        model_components = ModelComponentsParam(regressors={"regressor_cols": ["missing_regressor"]})
...
def test_run_template_4():
    ...
    - monthly data.
    """
    data = generate_df_for_tests(freq="MS", periods=90)
    df = data["df"]
    forecast_horizon = 30
    ...
    model_components = ModelComponentsParam(
        ...
        hyperparameter_override=[{"estimator__growth_term": ["linear", "quadratic"]}, {"estimator__yearly_seasonality": [2]}],
    )
    config = ForecastConfig(
        model_template=ModelTemplateEnum.SILVERKITE.name,
        forecast_horizon=forecast_horizon,
    ...
    grid_results = summarize_grid_search_results(result.grid_search, score_func=score_func, score_func_greater_is_better=False)
    ...
    assert result.backtest.test_evaluation[rmse] == pytest.approx(5.425, rel=0.15)
    assert result.backtest.test_evaluation[q80] == pytest.approx(1.036, rel=0.15)
    assert result.forecast.train_evaluation[rmse] == pytest.approx(2.728, rel=0.15)
    assert result.forecast.train_evaluation[q80] == pytest.approx(0.991, rel=0.15)
    check_forecast_pipeline_result(result, coverage=0.99, strategy=None, score_func=score_func, greater_is_better=False)
...
def test_run_template_5():
    ...
    data = generate_df_for_tests(freq="W", periods=230, growth_coef=4.0, growth_pow=3.0, intercept=100.0)
    df = data["train_df"]  # training dataset with 185 points
    forecast_horizon = 30
    # Configures 3 CV splits with train start indices: 0, 30, 60.
    evaluation_period = EvaluationPeriodParam(
        cv_expanding_window=False, cv_horizon=30, cv_min_train_periods=55, cv_periods_between_train_test=10, test_horizon=30  # rolling start date
    )
    coverage = None
    # Parameters that match synthetic dataset. Should fit well
    # because origin for growth term is the start of the dataset.
    model_components = ModelComponentsParam(
        growth={"growth_term": "cubic"},
        events={
    ...
    time_properties = get_forecast_time_properties(df=df, forecast_horizon=forecast_horizon)
    ...
    with warnings.catch_warnings():
        ...
        assert cv_results[f"mean_train_{metric_name}"][0] == pytest.approx(1.911, rel=0.1)
        assert cv_results[f"mean_test_{metric_name}"][0] == pytest.approx(10.27, rel=0.1)
        ...
        assert cv_results[f"mean_train_{metric_name}"][0] == pytest.approx(1.913, rel=0.1)
        assert cv_results[f"mean_test_{metric_name}"][0] == pytest.approx(9.965, rel=0.1)
...
def test_run_template_6():
    ...
    model_components = ModelComponentsParam(
        seasonality={"weekly_seasonality": 0},  # No weekly seasonality
        custom={"fit_algorithm_dict": {"fit_algorithm": "ridge"}},
        changepoints={
            "changepoints_dict": {
                "method": "auto",
            ...
            "seasonality_changepoints_dict": {"no_changepoint_proportion_from_end": 0.3, "regularization_strength": 0.1},
        },
    )
    evaluation_period = EvaluationPeriodParam(
        cv_expanding_window=False, cv_horizon=0, cv_min_train_periods=55, cv_periods_between_train_test=10, test_horizon=30  # rolling start date
    )
    config = ForecastConfig(
        model_template=ModelTemplateEnum.SILVERKITE.name,
        forecast_horizon=30,
...
def test_run_template_8():
    ...
    data = generate_df_with_reg_for_tests(freq="MS", periods=48, remove_extra_cols=True, mask_test_actuals=True)
    reg_cols = ["regressor1", "regressor2", "regressor_categ"]
    keep_cols = [TIME_COL, VALUE_COL] + reg_cols
    df = data["df"][keep_cols]
    forecast_horizon = data["test_df"].shape[0]
    ...
    model_components = ModelComponentsParam(
        ...
        seasonality=dict(
            yearly_seasonality=[False], quarterly_seasonality=[False], monthly_seasonality=[False], weekly_seasonality=[False], daily_seasonality=[False]
        ),
        custom=dict(fit_algorithm_dict=dict(fit_algorithm="ridge"), extra_pred_cols=["ct2"]),
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(4.0, rel=0.35)  # macOS ~4.844, Linux ~3.597
        check_forecast_pipeline_result(
            result, coverage=0.9, strategy=None, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=False
        )
...
def test_run_template_9():
    ...
    model_components = ModelComponentsParam(
        ...
        lagged_regressors={
            "lagged_regressor_dict": {
                "regressor2": {"lag_dict": {"orders": [5]}, "agg_lag_dict": {"orders_list": [[7, 7 * 2, 7 * 3]], "interval_list": [(8, 7 * 2)]}},
                "regressor_bool": "auto",
                "regressor_categ": {"lag_dict": {"orders": [5]}},
            }
        },
        custom={"fit_algorithm_dict": {"fit_algorithm": "ridge", "fit_algorithm_params": {"cv": 2}}},
    )
    computation = ComputationParam(verbose=2)
    forecast_horizon = 27
    coverage = 0.90
    ...
    with warnings.catch_warnings():
        ...
        rmse = EvaluationMetricEnum.RootMeanSquaredError.get_metric_name()
        q80 = EvaluationMetricEnum.Quantile80.get_metric_name()
        assert result.backtest.test_evaluation[rmse] == pytest.approx(3.360, rel=0.15)
        assert result.backtest.test_evaluation[q80] == pytest.approx(0.995, rel=0.15)
        assert result.forecast.train_evaluation[rmse] == pytest.approx(2.069, rel=0.15)
        assert result.forecast.train_evaluation[q80] == pytest.approx(0.771, rel=0.15)
        check_forecast_pipeline_result(result, coverage=coverage, strategy=None, score_func=metric.name, greater_is_better=False)
        ...
        expected_pred_cols = {"regressor1", "ct1", "regressor2_lag5", "regressor2_avglag_7_14_21", "regressor2_avglag_8_to_14", "regressor_categ_lag5"}
        expected_x_mat_cols = {
            "regressor1",
            "ct1",
            "regressor2_lag5",
    ...
    # Note that for newer scikit-learn (1.1+), we need to add a check for ValueError, matching "model is misconfigured"
    with pytest.raises((ValueError, KeyError)) as exception_info:
        model_components = ModelComponentsParam(lagged_regressors={"lagged_regressor_dict": {"missing_lagged_regressor": {"lag_dict": {"orders": [5]}}}})
        Forecaster().run_forecast_config(
            df=df,
            config=ForecastConfig(
                model_template=ModelTemplateEnum.SILVERKITE.name,
    ...
    with pytest.raises((ValueError, KeyError)) as exception_info:
        model_components = ModelComponentsParam(
            lagged_regressors={
                "lagged_regressor_dict": {"missing_lagged_regressor": {"lag_dict": {"orders": [5]}}, "regressor_bool": {"lag_dict": {"orders": [5]}}}
            }
        )
        Forecaster().run_forecast_config(
            df=df,
...
def test_run_template_with_event_indicators():
    ...
    data = generate_df_with_reg_for_tests(freq="D", periods=365 * 3, remove_extra_cols=True, mask_test_actuals=True)
    ...
    metadata = MetadataParam(
        time_col=time_col, value_col=value_col, freq="D", date_format="%Y-%m-%d", train_end_date=datetime.datetime(2020, 9, 7)  # 2020-09-07 is Labor (Monday)
    )
    evaluation_metric = EvaluationMetricParam(
        cv_selection_metric=EvaluationMetricEnum.MedianAbsolutePercentError.name,
        cv_report_metrics=None,
    )
    evaluation_period = EvaluationPeriodParam(test_horizon=1, periods_between_train_test=0, cv_max_splits=0)
    ...
    # 2020-09-07 is Labor and on Monday
...
def test_run_template_with_infinite_values():
    ...
    data = generate_df_for_tests(freq="D", periods=400)
    df = data["train_df"]
...
def test_run_template_daily_1():
    ...
    check_forecast_pipeline_result(result=result, coverage=None, expected_grid_size=3)


def test_run_template_daily_90():
    dl = DataLoader()
    data = dl.load_peyton_manning()
    evaluation_period = EvaluationPeriodParam(test_horizon=30, cv_max_splits=0)  # no CV needed for a single config; avoids 4x model fits on CI
    # Use a single ridge config from the SILVERKITE_DAILY_90 family instead of the
    # full 4-config grid to keep CI memory usage bounded.
    config = ForecastConfig(
        model_template="DAILY_SEAS_LTQM_GR_LINEAR_CP_LT_HOL_SP2_FEASET_AUTO_ALGO_RIDGE_AR_OFF_DSI_AUTO_WSI_AUTO",
        forecast_horizon=10,
        evaluation_period_param=evaluation_period,
    )
    result = Forecaster().run_forecast_config(
    ...
    # cv_max_splits=0 skips CV so cv_results_ doesn't exist; just verify the pipeline ran.
...
def test_run_template_weekly():
    ...
    new_df = data.iloc[:730].resample(rule="W-TUE", on="ts").mean(numeric_only=True)
    ...
    check_forecast_pipeline_result(result=result, coverage=None, expected_grid_size=4)
...
def test_run_template_forecast_one_by_one():
    ...
    config = ForecastConfig(
        ...
        forecast_horizon=7,
        evaluation_period_param=evaluation_period,
        model_components_param=ModelComponentsParam(autoregression=dict(autoreg_dict="auto")),
    ...
    assert_equal(result.model[-1].estimators[0].predict(future_df.iloc[:1])[[PREDICTED_COL]].values, result.forecast.df_test.iloc[:1][[PREDICTED_COL]].values)
    assert_equal(result.model[-1].estimators[1].predict(future_df.iloc[1:3])[[PREDICTED_COL]].values, result.forecast.df_test.iloc[1:3][[PREDICTED_COL]].values)
    assert_equal(result.model[-1].estimators[2].predict(future_df.iloc[3:7])[[PREDICTED_COL]].values, result.forecast.df_test.iloc[3:7][[PREDICTED_COL]].values)
...
def test_silverkite_ar_lag():
    ...
    data = dl.load_peyton_manning()
    evaluation_period = EvaluationPeriodParam(
        cv_expanding_window=False, cv_horizon=1, cv_min_train_periods=52, cv_periods_between_train_test=0, test_horizon=1, cv_max_splits=1  # rolling start date
    )
    config = ForecastConfig(
...
def test_silverkite_simulation_num():
    ...
    data = dl.load_peyton_manning()
    evaluation_period = EvaluationPeriodParam(
        cv_expanding_window=False, cv_horizon=0, cv_min_train_periods=52, cv_periods_between_train_test=0, test_horizon=1, cv_max_splits=1  # rolling start date
    )
    config = ForecastConfig(
        model_template="SILVERKITE_EMPTY",
        forecast_horizon=2,
        evaluation_period_param=evaluation_period,
        model_components_param=ModelComponentsParam(autoregression=dict(autoreg_dict={"lag_dict": {"orders": [1]}}, simulation_num=2)),
    )
    data["ts"] = pd.to_datetime(data["ts"])
    new_df = data.iloc[:365]
    result = Forecaster().run_forecast_config(
    ...
    config = ForecastConfig(
        ...
        forecast_horizon=2,
        evaluation_period_param=evaluation_period,
        model_components_param=ModelComponentsParam(autoregression=dict(autoreg_dict={"lag_dict": {"orders": [1]}}, simulation_num=2, fast_simulation=True)),
    )
    result = Forecaster().run_forecast_config(
        df=new_df,
        config=config,
...
def test_silverkite_float32():
    ...
    config = ForecastConfig(
        model_template="SILVERKITE", forecast_horizon=1, evaluation_period_param=evaluation_period, model_components_param=ModelComponentsParam()
    )
    new_df = data.iloc[:365]
    Forecaster().run_forecast_config(
        df=new_df,
...
def test_silverkite_monthly_template():
    ...
    evaluation_period = EvaluationPeriodParam(test_horizon=1, cv_max_splits=1)
    config = ForecastConfig(
        model_template="SILVERKITE_MONTHLY", forecast_horizon=1, evaluation_period_param=evaluation_period, model_components_param=ModelComponentsParam()
    )
    Forecaster().run_forecast_config(
        df=data,
        config=config,
...
def test_silverkite_monthly_template_potential_changepoint_cap():
    df = pd.DataFrame({TIME_COL: pd.date_range("2000-01-01", freq="MS", periods=12 * 200), VALUE_COL: 1})
    with LogCapture(LOGGER_NAME) as log_capture:
        evaluation_period = EvaluationPeriodParam(test_horizon=0, cv_max_splits=0)
        config = ForecastConfig(
        ...
        log_capture.check_present(
            (
                LOGGER_NAME,
                "INFO",
                "Number of potential changepoints is capped by 'potential_changepoint_n_max' "
                "as 100. The 'potential_changepoint_distance' "
                "180D is ignored. The original number of changepoints was 405.",
            )
        )
...
def test_silverkite_auto_config():
    ...
    assert (
        result.model[-1]
        .model_dict["fs_components_df"][["name", "period", "order", "seas_names"]]
        .equals(
            pd.DataFrame(
                {
...
def test_holiday_neighbor_impact():
    ...
    df = pd.DataFrame({"ts": pd.date_range("2020-01-01", freq="W-SUN", periods=100), "y": np.random.randn(100)})
    config = ForecastConfig(
        model_template="SILVERKITE",
        forecast_horizon=1,
        metadata_param=MetadataParam(freq="W-SUN"),
        evaluation_period_param=EvaluationPeriodParam(test_horizon=0, cv_horizon=0),
        model_components_param=ModelComponentsParam(
            events=dict(daily_event_neighbor_impact=lambda x: [x - timedelta(days=x.isocalendar()[2] - 1) + timedelta(days=i) for i in range(7)]),
        ),
    )
    result = Forecaster().run_forecast_config(
        df=df,
    ...
    # The first day is "2020-01-05" (Sun). With daily_event_neighbor_impact mapping
    # each week to its Mon-Sun range, this week covers New Years (Jan 1).
    # New Years has its own explicit column (not bucketed into Other).
    assert result.model[-1].model_dict["x_mat"]["C(Q('events_New Years'), levels=['', 'event'])[T.event]"].iloc[0] == 1
...
def test_various_models(forecast_data):
    ...
    def fit_forecast(df_label, fit_algorithm):
        ...
        model_components = ModelComponentsParam(custom=dict(fit_algorithm_dict=dict(fit_algorithm=fit_algorithm)))
    ...
    with warnings.catch_warnings():
        ...
        for df_label in forecast_data.keys():
            for fit_algorithm in ["ridge", "elastic_net"]:
                err_feature_num_dict[f"{df_label}_{fit_algorithm}"] = fit_forecast(df_label, fit_algorithm)

    expected_feature_nums = {
        "daily_pt_ridge": 131,
        "daily_pt_elastic_net": 131,
        "daily_bk_ridge": 124,
        "daily_bk_elastic_net": 124,
        "sim_ridge": 131,
        "sim_elastic_net": 131,
    }
    expected_mapes = {
        "daily_pt_ridge": 1.9,
        "daily_pt_elastic_net": 1.9,
        "daily_bk_ridge": 37.56,
        "daily_bk_elastic_net": 34.54,
        "sim_ridge": 1.59,
        "sim_elastic_net": 1.36,
    }
    assert set(err_feature_num_dict.keys()) == set(expected_feature_nums.keys())
    for key, (mape, feat_num) in err_feature_num_dict.items():
        assert feat_num == expected_feature_nums[key]
        assert mape == pytest.approx(expected_mapes[key], rel=0.15)


def test_regression_weight_col_end_to_end():
    """Tests that a custom regression_weight_col column (not a built-in greykite time feature)
    is preserved through the full pipeline and reaches the ML fitting step without KeyError.
    Regression test for: weight_col not propagated through get_forecast_time_properties."""
    from abvelocity.ts.gk.framework.templates.autogen.forecast_config import EvaluationPeriodParam, ForecastConfig, MetadataParam, ModelComponentsParam
    from abvelocity.ts.gk.framework.templates.forecaster import Forecaster
    ...
    config = ForecastConfig(
        metadata_param=MetadataParam(time_col=TIME_COL, value_col=VALUE_COL, freq="D", train_end_date=df[TIME_COL].iloc[350]),
        model_components_param=ModelComponentsParam(
            custom={
                "fit_algorithm_dict": {"fit_algorithm": "ridge"},
        ...
        evaluation_period_param=EvaluationPeriodParam(test_horizon=7, cv_horizon=7, cv_min_train_periods=100, cv_max_splits=2),
    )

    # should complete without KeyError: 'custom_weight'
    result = Forecaster().run_forecast_config(df=df, config=config)
