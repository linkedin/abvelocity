...
from abvelocity.ts.gk.framework.templates.pickle_utils import dump_obj, load_obj, recursive_rm_dir
...
@pytest.fixture
def df():
    data = generate_df_for_tests(freq="D", periods=130)
    return data["df"]
...
def result(df):
    ...
    result = forecaster.run_forecast_config(
        ...
        config=ForecastConfig(
            ...
            evaluation_period_param=EvaluationPeriodParam(cv_max_splits=1, cv_horizon=7, test_horizon=7, cv_min_train_periods=80),
            model_components_param=ModelComponentsParam(custom={"fit_algorithm_dict": {"fit_algorithm": "ridge"}}, autoregression={"autoreg_dict": "auto"}),
...
def test_forecast_result_silverkite(df, result):
    dump_obj(result, dir_name="silverkite", dump_design_info=True, overwrite_exist_dir=True)
    result_rec = load_obj(dir_name="silverkite", load_design_info=True)
    recursive_rm_dir("silverkite")

    # Tests loaded results
    # Grid search cv results
    assert_equal(summarize_grid_search_results(result.grid_search), summarize_grid_search_results(result_rec.grid_search))
    # Grid search attributes
    for key in result.grid_search.__dict__.keys():
        if key not in ["scoring", "estimator", "refit", "cv", "error_score", "cv_results_", "scorer_", "best_estimator_"]:
            assert_equal(result.grid_search.__dict__[key], result_rec.grid_search.__dict__[key])

    # Model
    assert_equal(result.model[-1].predict(df), result_rec.model[-1].predict(df))
    ...
    for key in result.model[-1].__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.model[-1].__dict__[key], result_rec.model[-1].__dict__[key])
    assert_equal(inspect.getsource(result.model[-1].__dict__["score_func"]), inspect.getsource(result_rec.model[-1].__dict__["score_func"]))
    # Model: estimator/model_dict
    for key in result.model[-1].model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.model[-1].model_dict[key], result_rec.model[-1].model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(inspect.getsource(result.model[-1].model_dict[key]), inspect.getsource(result_rec.model[-1].model_dict[key]))
    ...
    for key in result.model[-1].model_dict["autoreg_dict"].keys():
        if key not in ["series_na_fill_func"]:
            assert_equal(result.model[-1].model_dict["autoreg_dict"][key], result_rec.model[-1].model_dict["autoreg_dict"][key])
    assert_equal(
    ...
    # Forecast
    assert_equal(result.forecast.estimator.predict(df), result_rec.forecast.estimator.predict(df))
    ...
    for key in result.forecast.__dict__.keys():
        if key not in ["r2_loss_function", "estimator"]:
            assert_equal(result.forecast.__dict__[key], result_rec.forecast.__dict__[key])
    assert_equal(inspect.getsource(result.forecast.__dict__["r2_loss_function"]), inspect.getsource(result_rec.forecast.__dict__["r2_loss_function"]))
    # Forecast: estimator
    for key in result.forecast.estimator.__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.forecast.estimator.__dict__[key], result_rec.forecast.estimator.__dict__[key])
    assert_equal(inspect.getsource(result.forecast.estimator.__dict__["score_func"]), inspect.getsource(result_rec.forecast.estimator.__dict__["score_func"]))
    # Model: estimator/model_dict
    for key in result.forecast.estimator.model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.forecast.estimator.model_dict[key], result_rec.forecast.estimator.model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(inspect.getsource(result.forecast.estimator.model_dict[key]), inspect.getsource(result_rec.forecast.estimator.model_dict[key]))
    # Model: estimator/model_dict/autoreg_dict
    for key in result.forecast.estimator.model_dict["autoreg_dict"].keys():
        if key not in ["series_na_fill_func"]:
            assert_equal(result.forecast.estimator.model_dict["autoreg_dict"][key], result_rec.forecast.estimator.model_dict["autoreg_dict"][key])
    assert_equal(
    ...
    # Backtest
    assert_equal(result.backtest.estimator.predict(df), result_rec.backtest.estimator.predict(df))
    assert result.backtest.estimator.model_dict["x_design_info"] is not None
    # Backtest: attributes
    for key in result.backtest.__dict__.keys():
        if key not in ["r2_loss_function", "estimator"]:
            assert_equal(result.backtest.__dict__[key], result_rec.backtest.__dict__[key])
    assert_equal(inspect.getsource(result.backtest.__dict__["r2_loss_function"]), inspect.getsource(result_rec.backtest.__dict__["r2_loss_function"]))
    # Backtest: estimator
    for key in result.backtest.estimator.__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.backtest.estimator.__dict__[key], result_rec.backtest.estimator.__dict__[key])
    assert_equal(inspect.getsource(result.backtest.estimator.__dict__["score_func"]), inspect.getsource(result_rec.backtest.estimator.__dict__["score_func"]))
    # Model: estimator/model_dict
    for key in result.backtest.estimator.model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.backtest.estimator.model_dict[key], result_rec.backtest.estimator.model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(inspect.getsource(result.backtest.estimator.model_dict[key]), inspect.getsource(result_rec.backtest.estimator.model_dict[key]))
    # Model: estimator/model_dict/autoreg_dict
    for key in result.backtest.estimator.model_dict["autoreg_dict"].keys():
        if key not in ["series_na_fill_func"]:
            assert_equal(result.backtest.estimator.model_dict["autoreg_dict"][key], result_rec.backtest.estimator.model_dict["autoreg_dict"][key])
    assert_equal(
    ...
    for key in result.timeseries.__dict__.keys():
        assert_equal(result.timeseries.__dict__[key], result_rec.timeseries.__dict__[key])
...
def test_forecast_result_one_by_one(df):
    ...
    dump_obj(result, dir_name="onebyone", dump_design_info=True, overwrite_exist_dir=True)
    result_rec = load_obj(dir_name="onebyone", load_design_info=True)
    ...
    for key in result.model[-1].estimators[0].__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.model[-1].estimators[0].__dict__[key], result_rec.model[-1].estimators[0].__dict__[key])
    assert_equal(
        inspect.getsource(result.model[-1].estimators[0].__dict__["score_func"]), inspect.getsource(result_rec.model[-1].estimators[0].__dict__["score_func"])
    )
    # Model: estimator/model_dict
    for key in result.model[-1].estimators[0].model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.model[-1].estimators[0].model_dict[key], result_rec.model[-1].estimators[0].model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(
                inspect.getsource(result.model[-1].estimators[0].model_dict[key]), inspect.getsource(result_rec.model[-1].estimators[0].model_dict[key])
            )
    # Model: estimator/model_dict/autoreg_dict
    for key in result.model[-1].estimators[0].model_dict["autoreg_dict"].keys():
        if key not in ["series_na_fill_func"]:
            assert_equal(result.model[-1].estimators[0].model_dict["autoreg_dict"][key], result_rec.model[-1].estimators[0].model_dict["autoreg_dict"][key])
    assert_equal(
    ...
    for key in result.forecast.estimator.estimators[0].__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.forecast.estimator.estimators[0].__dict__[key], result_rec.forecast.estimator.estimators[0].__dict__[key])
    assert_equal(
    ...
    for key in result.forecast.estimator.estimators[0].model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.forecast.estimator.estimators[0].model_dict[key], result_rec.forecast.estimator.estimators[0].model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(
    ...
    for key in result.backtest.estimator.estimators[0].__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.backtest.estimator.estimators[0].__dict__[key], result_rec.backtest.estimator.estimators[0].__dict__[key])
    assert_equal(
    ...
    for key in result.backtest.estimator.estimators[0].model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.backtest.estimator.estimators[0].model_dict[key], result_rec.backtest.estimator.estimators[0].model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(
...
def test_no_design_info(result):
    ...
    dump_obj(result, dir_name="silverkite_dump_design_false_load_design_true", dump_design_info=False, overwrite_exist_dir=True)
    result_rec = load_obj(dir_name="silverkite_dump_design_false_load_design_true", load_design_info=True)
    ...
    dump_obj(result, dir_name="silverkite_dump_design_false_load_design_false", dump_design_info=False, overwrite_exist_dir=True)
    result_rec = load_obj(dir_name="silverkite_dump_design_false_load_design_false", load_design_info=False)
    ...
    dump_obj(result, dir_name="silverkite_dump_design_true_load_design_false", dump_design_info=True, overwrite_exist_dir=True)
    result_rec = load_obj(dir_name="silverkite_dump_design_true_load_design_false", load_design_info=False)
...
def test_overwrite():
    dump_obj([1, "a", {"b": 3}], dir_name="overwrite", dump_design_info=True, overwrite_exist_dir=True)
    loaded_obj = load_obj(dir_name="overwrite")
    assert loaded_obj == [1, "a", {"b": 3}]
    # Overwrites the same dir with a different object.
    dump_obj([2, "b", {"c": 4}], dir_name="overwrite", dump_design_info=True, overwrite_exist_dir=True)
    loaded_obj = load_obj(dir_name="overwrite")
...
def test_errors(result):
    ...
    dump_obj(result, dir_name="name", dump_design_info=False, overwrite_exist_dir=True)
    with pytest.raises(
        FileExistsError, match="The directory already exists. " "Please either specify a new directory or " "set overwrite_exist_dir to True to overwrite it."
    ):
        dump_obj(result, dir_name="name", dump_design_info=False, overwrite_exist_dir=False)
    recursive_rm_dir("name")

    # Empty dir load.
    os.mkdir("this_dir_is_empty")
    with pytest.raises(ValueError, match="dir is empty!"):
        load_obj(dir_name="this_dir_is_empty")
    ...
    with pytest.raises(ValueError, match="Multiple elements found in top level."):
        load_obj(dir_name="multiple_dir")
