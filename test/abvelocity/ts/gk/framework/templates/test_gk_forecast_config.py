...
from abvelocity.ts.gk.framework.templates.autogen.forecast_config import (
    ...
    assert_equal_forecast_config,
    forecast_config_from_dict,
    from_list_dict,
    from_list_dict_or_none,
)
from abvelocity.ts.gk.framework.templates.autogen.forecast_config_utils import FORECAST_CONFIG_JSON_COMPLETE, FORECAST_CONFIG_JSON_DEFAULT
...
def assert_forecast_config(config: Optional[ForecastConfig] = None):
    ...
    assert config.evaluation_metric_param.cv_report_metrics == [EvaluationMetricEnum.MeanAbsoluteError.name, EvaluationMetricEnum.MeanAbsolutePercentError.name]
    ...
    assert config.model_components_param.hyperparameter_override == [{"h1": 4}, {"h2": 5}, None] or config.model_components_param.hyperparameter_override == [
        {"h1": 4},
        {"h2": 5},
        {},
    ]
...
def test_forecast_config():
    ...
    config = ForecastConfig(
        ...
        metadata_param=MetadataParam(time_col="custom_time_col", anomaly_info=[{"key": "value"}, {"key2": "value2"}]),
        evaluation_period_param=EvaluationPeriodParam(test_horizon=10, periods_between_train_test=5, cv_min_train_periods=20),
        evaluation_metric_param=EvaluationMetricParam(
            cv_selection_metric=EvaluationMetricEnum.MeanSquaredError.name,
            cv_report_metrics=[EvaluationMetricEnum.MeanAbsoluteError.name, EvaluationMetricEnum.MeanAbsolutePercentError.name],
    ...
    assert ForecastConfig(
        evaluation_metric_param=EvaluationMetricParam(cv_report_metrics=CV_REPORT_METRICS_ALL),
    ).to_dict()
...
def assert_forecast_config_json_multiple_model_components_parameter(config: Optional[ForecastConfig] = None):
    """Asserts the forecast config values. This function expects a particular config and is not generic"""
    config = ForecastConfigDefaults().apply_forecast_config_defaults(config)
    assert config.model_template == [ModelTemplateEnum.SILVERKITE.name, ModelTemplateEnum.SILVERKITE_DAILY_90.name, ModelTemplateEnum.SILVERKITE_WEEKLY.name]
    assert config.evaluation_metric_param.relative_error_tolerance == 0.02
    ...
    assert model_components_param_1.hyperparameter_override is None or model_components_param_1.hyperparameter_override is None
    ...
    assert model_components_param_2.hyperparameter_override == [{"h1": 4}, {"h2": 5}, None] or model_components_param_2.hyperparameter_override == [
        {"h1": 4},
        {"h2": 5},
        {},
    ]
...
def test_assert_equal_forecast_config():
    ...
    with pytest.raises(AssertionError, match='\\(column name="start"\\) values are different'):
        anomaly_df = pd.DataFrame({"start": ["2020-01-01", "2020-02-02"], "end": ["2020-01-10", "2020-02-05"]})
        ...
        anomaly_df = pd.DataFrame({"start": ["2020-01-05", "2020-02-02"], "end": ["2020-01-10", "2020-02-10"]})
