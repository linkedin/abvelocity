...
from abvelocity.ts.gk.framework.templates.autogen.forecast_config import EvaluationPeriodParam, ForecastConfig
...
@pytest.fixture
def hourly_data():
    df = pd.DataFrame({"ts": pd.date_range("2020-01-01", periods=3000, freq="H"), "y": range(3000)})
    return df


@pytest.fixture
def daily_data():
    df = pd.DataFrame({"ts": pd.date_range("2020-01-01", periods=1000, freq="D"), "y": range(1000)})
    return df


@pytest.fixture
def weekly_data():
    df = pd.DataFrame({"ts": pd.date_range("2020-01-01", periods=100, freq="W"), "y": range(100)})
    return df


@pytest.fixture
def monthly_data():
    df = pd.DataFrame({"ts": pd.date_range("2020-01-01", periods=100, freq="MS"), "y": range(100)})
    return df


def test_exception(hourly_data):
    """Exception."""
    with pytest.raises(ValueError, match="The `default_model_template_name` in " "`get_auto_silverkite_model_template` cannot be 'AUTO'."):
        get_auto_silverkite_model_template(df=hourly_data, default_model_template_name=ModelTemplateEnum.AUTO.name, config=ForecastConfig(forecast_horizon=10))


@pytest.mark.parametrize(
    "forecast_horizon,cv_max_splits,expected_template",
    [
...
def test_get_model_template_hourly(forecast_horizon, cv_max_splits, expected_template, hourly_data):
    ...
    df = hourly_data
    config = ForecastConfig(forecast_horizon=forecast_horizon, evaluation_period_param=EvaluationPeriodParam(cv_max_splits=cv_max_splits))
    with LogCapture(LOGGER_NAME) as log_capture:
        model_template = get_auto_silverkite_model_template(df=df, default_model_template_name=DEFAULT_MODEL_TEMPLATE, config=config)
        assert model_template == expected_template
        log_capture.check_present(
            (LOGGER_NAME, "INFO", f"Model template was set to 'auto'. " f"Automatically found most appropriate model template '{model_template}'.")
        )
...
@pytest.mark.parametrize("forecast_horizon,cv_max_splits,expected_template", [(3, 1, SILVERKITE_WEEKLY[0]), (3, 10, ModelTemplateEnum.SILVERKITE_WEEKLY.name)])
...
def test_get_model_template_monthly(monthly_data):
    ...
    df = monthly_data
    config = ForecastConfig(forecast_horizon=1, evaluation_period_param=EvaluationPeriodParam(cv_max_splits=1))
    with LogCapture(LOGGER_NAME) as log_capture:
        model_template = get_auto_silverkite_model_template(df=df, default_model_template_name=DEFAULT_MODEL_TEMPLATE, config=config)
...
def test_no_horizon(daily_data):
    ...
    df = daily_data
    config = ForecastConfig(evaluation_period_param=EvaluationPeriodParam(cv_max_splits=1))
...
def test_cv_not_enough(daily_data):
    ...
    """
    df = daily_data
    config = ForecastConfig(forecast_horizon=1, evaluation_period_param=EvaluationPeriodParam(cv_max_splits=5, test_horizon=len(df) - 5))
    model_template = get_auto_silverkite_model_template(df=df, default_model_template_name=DEFAULT_MODEL_TEMPLATE, config=config)
