...
from abvelocity.ts.common.python_utils import assert_equal, unique_elements_in_list
from abvelocity.ts.common.testing_utils import assert_eval_function_equal, generate_df_with_reg_for_tests
from abvelocity.ts.gk.framework.templates.autogen.forecast_config import EvaluationMetricParam, EvaluationPeriodParam, ForecastConfig, MetadataParam
...
class MyTemplate(BaseTemplate):
    ...
    def get_lagged_regressor_info(self):
        return {"lagged_regressor_cols": ["regressor2", "regressor_bool"], "overall_min_lag_order": 1, "overall_max_lag_order": 7}
...
def test_get_pipeline(df):
    ...
    mt.config = ForecastConfig(coverage=0.9, evaluation_metric_param=EvaluationMetricParam(cv_selection_metric=metric.name))
    ...
    assert_eval_function_equal(pipeline.steps[-1][-1].score_func, metric.get_metric_func())
...
def test_apply_template_for_pipeline_params(df):
    ...
    config = ForecastConfig(
        ...
        evaluation_metric_param=EvaluationMetricParam(cv_selection_metric="MeanSquaredError"),
        evaluation_period_param=EvaluationPeriodParam(cv_use_most_recent_splits=True),
    )
    original_config = dataclasses.replace(config)

    # Tests apply_template_for_pipeline_params
    pipeline_params = mt.apply_template_for_pipeline_params(df=df, config=config)
    ...
    assert pipeline_params["pipeline"].named_steps["input"].transformer_list[2][1].named_steps["select_reg"].column_names == expected_col_names
