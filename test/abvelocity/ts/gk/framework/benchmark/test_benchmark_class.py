...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import ACTUAL_COL, PREDICTED_COL, PREDICTED_LOWER_COL, PREDICTED_UPPER_COL, TIME_COL, VALUE_COL
from abvelocity.ts.common.evaluation import EvaluationMetricEnum, root_mean_squared_error
from abvelocity.ts.common.python_utils import assert_equal
from abvelocity.ts.common.testing_utils import generate_df_with_reg_for_tests
from abvelocity.ts.gk.framework.benchmark.benchmark_class import BenchmarkForecastConfig
from abvelocity.ts.gk.framework.constants import FORECAST_STEP_COL
...
def df():
    data = generate_df_with_reg_for_tests(freq="D", periods=20 * 7, train_frac=0.9, remove_extra_cols=True, mask_test_actuals=True)
...
def valid_configs():
    ...
    evaluation_metric = EvaluationMetricParam(
        cv_selection_metric=EvaluationMetricEnum.MeanAbsoluteError.name, cv_report_metrics=None, agg_periods=7, agg_func=np.mean, null_model_params=None
    )
    ...
    silverkite_components_1 = ModelComponentsParam(
        seasonality={"yearly_seasonality": False, "weekly_seasonality": True},
    ...
    silverkite_components_2 = ModelComponentsParam(
        seasonality={"yearly_seasonality": False, "weekly_seasonality": True},
    ...
    valid_silverkite_1 = ForecastConfig(
        ...
        model_components_param=silverkite_components_1,
    )

    valid_silverkite_2 = ForecastConfig(
        model_template=ModelTemplateEnum.SILVERKITE.name,
        metadata_param=metadata,
        computation_param=computation,
        coverage=coverage,
        ...
        evaluation_period_param=evaluation_period,
        forecast_horizon=forecast_horizon,
        model_components_param=silverkite_components_2,
    )

    configs = {"valid_silverkite_1": valid_silverkite_1, "valid_silverkite_2": valid_silverkite_2}
    return configs
...
def test_benchmark_class_init(df, valid_configs, custom_tscv):
    ...
    with pytest.raises(TypeError, match=r"__init__\(\) missing 2 required positional arguments: 'df' and 'configs'"):
...
def test_validate(df, valid_configs, custom_tscv):
    ...
    with pytest.raises(ValueError, match=r"Unexpected key\(s\) found:"):
        ...
        configs["valid_silverkite_1"].model_components_param.regressors = {"invalid_regressors_key": ["regressor1"]}
        bm = BenchmarkForecastConfig(df=df, configs=configs, tscv=custom_tscv)
        bm.validate()

    # Error due to different forecast horizons in configs
    with pytest.raises(ValueError, match=r"valid_silverkite_2's 'forecast_horizon' \(7\) does not " r"match that of 'tscv' \(14\)"):
        configs = copy(valid_configs)
        configs["valid_silverkite_2"].forecast_horizon = 7
        bm = BenchmarkForecastConfig(df=df, configs=configs, tscv=custom_tscv)
        bm.validate()
    ...
    with pytest.raises(
        ValueError,
        match=rf"valid_silverkite_1's 'periods_between_train_test' \(7\) does not match " rf"that of 'tscv' \({custom_tscv.periods_between_train_test}\).",
    ):
        configs = copy(valid_configs)
        configs["valid_silverkite_1"].evaluation_period_param.periods_between_train_test = 7
        bm = BenchmarkForecastConfig(df=df, configs=configs, tscv=custom_tscv)
        bm.validate()
    ...
    with pytest.raises(ValueError, match="All forecast configs must have same coverage."):
        configs = copy(valid_configs)
        configs["valid_silverkite_1"].coverage = 0.1
        bm = BenchmarkForecastConfig(df=df, configs=configs, tscv=custom_tscv)
        bm.validate()
...
def test_extract_forecasts(valid_bm, df, valid_configs, custom_tscv):
    ...
    for config_name, config in bm.result.items():
        ...
        expected_train_end_date_values = np.repeat(
            [np.datetime64("2018-10-06"), np.datetime64("2018-10-13"), np.datetime64("2018-10-20")], custom_tscv.forecast_horizon
        )
        np.testing.assert_array_equal(train_end_date_values, expected_train_end_date_values)
        # Checks forecast_step column
        assert_equal(rolling_forecast_df[FORECAST_STEP_COL].values, np.tile(np.arange(custom_tscv.forecast_horizon) + 1, custom_tscv.max_splits))

        expected_columns = {"train_end_date", FORECAST_STEP_COL, "split_num", TIME_COL, ACTUAL_COL, PREDICTED_COL, PREDICTED_LOWER_COL, PREDICTED_UPPER_COL}
    ...
    expected_columns = {
        ...
        TIME_COL,
        ACTUAL_COL,
        f"valid_silverkite_1_{PREDICTED_COL}",
        f"valid_silverkite_1_{PREDICTED_LOWER_COL}",
        f"valid_silverkite_1_{PREDICTED_UPPER_COL}",
        f"valid_silverkite_2_{PREDICTED_COL}",
        f"valid_silverkite_2_{PREDICTED_LOWER_COL}",
        f"valid_silverkite_2_{PREDICTED_UPPER_COL}",
    }
...
def test_plot_forecasts_by_step(valid_bm):
    ...
    assert fig.data[1].name == "valid_silverkite_1_forecast"
    assert fig.data[2].name == "valid_silverkite_2_forecast"

    # custom value
    fig = bm.plot_forecasts_by_step(forecast_step=5, config_names=["valid_silverkite_2"], xlabel="xlab", ylabel="ylab", title="title", showlegend=False)
    ...
    assert fig.data[0].name == ACTUAL_COL
    assert fig.data[1].name == "valid_silverkite_2_forecast"
    ...
    forecast_step = 20
    with pytest.raises(
        ValueError, match=rf"`forecast_step` \({forecast_step}\) must be less than or equal " rf"to forecast horizon \({bm.tscv.forecast_horizon}\)."
    ):
...
def test_plot_forecasts_by_config(valid_bm):
    ...
    # default value
    fig = bm.plot_forecasts_by_config(config_name="valid_silverkite_1")
    assert fig.layout.showlegend
    assert fig.layout.xaxis.title.text == TIME_COL
    assert fig.layout.yaxis.title.text == VALUE_COL
    assert fig.layout.title.text == "Rolling forecast for valid_silverkite_1"
    assert fig.layout.title.x == 0.5
    ...
    fig = bm.plot_forecasts_by_config(config_name="valid_silverkite_2", colors=[default_color], xlabel="xlab", ylabel="ylab", title="title", showlegend=False)
    assert not fig.layout.showlegend
    assert fig.layout.xaxis.title.text == "xlab"
    assert fig.layout.yaxis.title.text == "ylab"
    assert fig.layout.title.text == "title"
...
def test_get_evaluation_metrics(valid_bm, metric_dict, df, valid_configs, custom_tscv):
    ...
    with pytest.warns(UserWarning):
        ...
        for metric_name in metric_dict.keys():
            expected_columns = expected_columns.union({f"train_{metric_name}", f"test_{metric_name}"})
        ...
        # check metric values
        assert_equal(evaluation_metrics_df["train_MSE"].values, evaluation_metrics_df["train_custom_MSE"].values)
        assert_equal(evaluation_metrics_df["test_MSE"].values, evaluation_metrics_df["test_custom_MSE"].values)
    ...
    # custom config value
    with pytest.warns(UserWarning):
        config_names = ["valid_silverkite_2"]
        evaluation_metrics_df = bm.get_evaluation_metrics(metric_dict=metric_dict, config_names=config_names)
    ...
    # error when `run` method has not been executed yet
    with pytest.raises(ValueError, match="Please execute the 'run' method " "before computing evaluation metrics."):
        bm = BenchmarkForecastConfig(df=df, configs=valid_configs, tscv=custom_tscv)
...
def test_plot_evaluation_metrics(valid_bm, metric_dict):
    ...
    # default value, all configs
    with pytest.warns(UserWarning):
        fig = bm.plot_evaluation_metrics(metric_dict=metric_dict)
        assert fig.layout.showlegend
        ...
        expected_xaxis = set()
        for metric_name in metric_dict.keys():
            expected_xaxis = expected_xaxis.union({f"train_{metric_name}", f"test_{metric_name}"})

        assert fig.data[0].name == "valid_silverkite_1"
        assert_equal(set(fig.data[0].x), expected_xaxis)
        assert fig.data[1].name == "valid_silverkite_2"
        assert_equal(set(fig.data[1].x), expected_xaxis)

    # custom value
    with pytest.warns(UserWarning):
        config_names = ["valid_silverkite_1"]
        fig = bm.plot_evaluation_metrics(metric_dict=metric_dict, config_names=config_names, xlabel="xlab", ylabel="ylab", title="title", showlegend=False)
        assert not fig.layout.showlegend
        assert fig.layout.xaxis.title.text == "xlab"
        assert fig.layout.yaxis.title.text == "ylab"
        assert fig.layout.title.text == "title"
...
def test_get_grouping_evaluation(valid_bm, metric_dict, df, valid_configs, custom_tscv):
    ...
    grouped_evaluation_df = bm.get_grouping_evaluation_metrics(metric_dict=metric_dict, groupby_time_feature="dow")
    expected_columns = set(["config_name", "split_num", "dow"] + [f"train {metric_name}" for metric_name in metric_dict.keys()])
    ...
    # check metric values
    assert_equal(grouped_evaluation_df["dow"].values, np.tile(np.arange(1, 8), bm.tscv.max_splits * len(bm.configs)))
    assert_equal(grouped_evaluation_df["train MSE"].values, grouped_evaluation_df["train custom_MSE"].values)
    assert grouped_evaluation_df["train corr"].dropna().between(-1, 1).all()

    # groupby sliding window size
    config_names = ["valid_silverkite_1"]
    grouped_evaluation_df = bm.get_grouping_evaluation_metrics(metric_dict=metric_dict, config_names=config_names, which="test", groupby_sliding_window_size=7)
    expected_columns = set(["config_name", "split_num", "ts_downsample"] + [f"test {metric_name}" for metric_name in metric_dict.keys()])
    assert set(grouped_evaluation_df.columns) == expected_columns
    ...
    # check metric values
    assert_equal(grouped_evaluation_df["test MSE"].values, grouped_evaluation_df["test custom_MSE"].values)
    ...
    grouped_evaluation_df = bm.get_grouping_evaluation_metrics(metric_dict=metric_dict, which="test", groupby_custom_column=custom_column)
    expected_columns = set(["config_name", "split_num", custom_column.name] + [f"test {metric_name}" for metric_name in metric_dict.keys()])
    assert set(grouped_evaluation_df.columns) == expected_columns
    # forecasting 2 weeks into future, so 2 rows for each split, config
    ...
    # error when `run` method has not been executed yet
    with pytest.raises(ValueError, match="Please execute the 'run' method before " "computing grouped evaluation metrics."):
        bm = BenchmarkForecastConfig(df=df, configs=valid_configs, tscv=custom_tscv)
...
def test_plot_grouping_evaluation(valid_bm, metric_dict):
    ...
    # default value, all configs, groupby time feature
    fig = bm.plot_grouping_evaluation_metrics(metric_dict=metric_dict, groupby_time_feature="woy")
    assert fig.layout.showlegend
    ...
    assert fig.data[0].name == "train MSE_valid_silverkite_1"
    assert fig.data[1].name == "train MSE_valid_silverkite_2"
    assert fig.data[2].name == "train RMSE_valid_silverkite_1"
    assert fig.data[3].name == "train RMSE_valid_silverkite_2"
    ...
    assert fig.data[0].name == "test MSE_valid_silverkite_1"
    assert fig.data[1].name == "test RMSE_valid_silverkite_1"
    assert fig.data[2].name == "test corr_valid_silverkite_1"
    assert fig.data[3].name == "test custom_MSE_valid_silverkite_1"
    assert fig.data[4].name == "test q95_loss_valid_silverkite_1"

    # groupby custom column
    custom_column = pd.Series(np.repeat(["g1", "g2"], 7), name="group")
    fig = bm.plot_grouping_evaluation_metrics(metric_dict=metric_dict, which="test", groupby_custom_column=custom_column, title="Error by custom group")
    assert fig.layout.showlegend
    ...
    assert fig.data[0].name == "test MSE_valid_silverkite_1"
    assert fig.data[1].name == "test MSE_valid_silverkite_2"
    assert fig.data[2].name == "test RMSE_valid_silverkite_1"
    assert fig.data[3].name == "test RMSE_valid_silverkite_2"
...
def test_get_runtimes(valid_bm, valid_configs, custom_tscv):
    ...
    # error when `run` method has not been executed yet
    with pytest.raises(ValueError, match="Please execute the 'run' method " "to obtain runtimes."):
        bm = BenchmarkForecastConfig(df=df, configs=valid_configs, tscv=custom_tscv)
        bm.get_runtimes()
...
def test_plot_runtimes(valid_bm):
    ...
    # custom value
    config_names = ["valid_silverkite_1"]
    fig = bm.plot_runtimes(config_names=config_names, xlabel="xlab", ylabel="ylab", title="title", showlegend=False)
    assert not fig.layout.showlegend
    assert fig.layout.xaxis.title.text == "xlab"
    assert fig.layout.yaxis.title.text == "ylab"
    assert fig.layout.title.text == "title"
...
def test_autocomplete_metric_dict(valid_bm, metric_dict):
    bm = valid_bm
    updated_metric_dict = bm.autocomplete_metric_dict(metric_dict=metric_dict, enum_class=EvaluationMetricEnum)
    assert_equal(list(metric_dict.keys()), list(updated_metric_dict.keys()))

    invalid_metric_dict = {"corr": EvaluationMetricEnum.Correlation, "invalid_metric": 5}
    enum_class = EvaluationMetricEnum
    with pytest.raises(ValueError, match="Value of 'invalid_metric' should be a callable or " f"a member of {enum_class}."):
        bm.autocomplete_metric_dict(metric_dict=invalid_metric_dict, enum_class=EvaluationMetricEnum)
