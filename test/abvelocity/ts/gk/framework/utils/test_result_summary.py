...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import FRACTION_OUTSIDE_TOLERANCE
...
from abvelocity.ts.gk.framework.pipeline.pipeline import forecast_pipeline
from abvelocity.ts.gk.framework.utils.result_summary import get_ranks_and_splits, summarize_grid_search_results
...
def test_get_ranks_and_splits(pipeline_results, recwarn):
    ...
    ranks_and_splits = get_ranks_and_splits(
        grid_search, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=None, combine_splits=True, decimals=None
    )
    ...
    # `decimals=2`
    ranks_and_splits = get_ranks_and_splits(
        grid_search, score_func=EvaluationMetricEnum.MeanAbsolutePercentError.name, greater_is_better=None, combine_splits=True, decimals=2
    )
    short_name = EvaluationMetricEnum.MeanAbsolutePercentError.get_metric_name()
    assert ranks_and_splits["short_name"] == short_name
    assert ranks_and_splits["ranks"].argmin() == grid_search.cv_results_[f"mean_test_{short_name}"].argmin()
    ...
    ranks_and_splits = get_ranks_and_splits(grid_search, score_func=mean_absolute_error, greater_is_better=True, combine_splits=False, decimals=None)
    ...
    with pytest.warns(Warning, match="Metric 'MAPE' is not available in the CV results."):
        ...
        assert ranks_and_splits == {"short_name": "MAPE", "ranks": None, "split_train": None, "split_test": None}
...
def test_summarize_grid_search_results(pipeline_results):
    ...
    cv_result = summarize_grid_search_results(
        grid_search=grid_search, only_changing_params=True, score_func=FRACTION_OUTSIDE_TOLERANCE, score_func_greater_is_better=False
    )
    ...
    cv_max_splits = 2
    cv_result = summarize_grid_search_results(
        grid_search=grid_search, only_changing_params=True, score_func=mean_absolute_error, score_func_greater_is_better=True
    )
    ...
    assert cv_result[f"mean_test_{short_name}"][1] == pytest.approx(2.387, rel=0.05)
    assert cv_result[f"mean_train_{short_name}"][1] == pytest.approx(1.839883, rel=0.05)
    assert cv_result[f"std_test_{short_name}"][1] == pytest.approx(0.12217, rel=0.05)
    assert cv_result[f"split_test_{short_name}"][1][0] == pytest.approx(2.26492, rel=0.05)
    assert cv_result[f"split_train_{short_name}"][1][0] == pytest.approx(1.84082, rel=0.05)
    ...
    cv_result = summarize_grid_search_results(
        grid_search=grid_search, only_changing_params=False, decimals=2, score_func=mean_absolute_error, score_func_greater_is_better=False
    )
    assert cv_result.shape == (4, 20)
    # only_changing_params=False, so all params in hyperparameter_grid are included
    assert len(cv_result["params"][0]) == 4
    # rounding is applied
    assert cv_result[f"mean_test_{short_name}"][1] == 2.39
    assert cv_result[f"mean_train_{short_name}"][1] == 1.84
    assert cv_result[f"std_test_{short_name}"][1] == 0.12
...
def pipeline_results():
    ...
    hyperparameter_grid = [
        {"estimator__strategy": ["quantile"], "estimator__quantile": [0.9]},
        {"estimator__strategy": ["mean"]},
        {"estimator__strategy": ["constant"], "estimator__constant": [1.0, 2.0]},
    ]
    ...
    fs1 = pd.DataFrame({"name": ["tow", "conti_year"], "period": [7.0, 1.0], "order": [3, 3], "seas_names": ["weekly", None]})
    fs2 = pd.DataFrame({"name": ["tow"], "period": [7.0], "order": [3], "seas_names": ["weekly"]})
    hyperparameter_grid = {
