...
from abvelocity.ts.gk.sklearn.uncertainty.quantile_regression_uncertainty_model import QuantileRegressionUncertaintyModel
...
def forecast_result():
    ...
    fit_df["y"] = df["y"]
    fut_df = pd.DataFrame({"ts": pd.date_range(df["ts"].max(), freq="D", periods=15)}).iloc[1:].reset_index(drop=True)
    predict_df = model.predict(fut_df)
    fut_x_mat = model.forecast_x_mat
    return {"df": df, "fit_df": fit_df, "x_mat": x_mat, "predict_df": predict_df, "fut_x_mat": fut_x_mat}


def test_init():
    """Tests instantiation."""
    model = QuantileRegressionUncertaintyModel(uncertainty_dict={})
...
def test_check_input(forecast_result):
    ...
    # Method name wrong.
    with pytest.raises(UncertaintyError, match="The uncertainty method uncertainty_method is not as expected quantile_regression."):
    ...
    with pytest.raises(UncertaintyError, match="`value_col` col not found in `train_df`."):
        model = QuantileRegressionUncertaintyModel(uncertainty_dict={"uncertainty_method": "quantile_regression", "params": {"value_col": "col"}})
        model.train_df = forecast_result["df"]
        model.x_mat = forecast_result["x_mat"]
        model._check_input()

    # No time column or ``x_mat``.
    with pytest.raises(UncertaintyError, match="Time column must be provided when `x_mat` is not given."):
        model = QuantileRegressionUncertaintyModel(uncertainty_dict={"uncertainty_method": "quantile_regression", "params": {}})
...
def test_is_residual_based_x_mat(forecast_result):
    ...
    model = QuantileRegressionUncertaintyModel(
        uncertainty_dict={"uncertainty_method": "quantile_regression", "params": {"value_col": "y", "is_residual_based": True}}, coverage=0.9
    )
    model.fit(train_df=forecast_result["fit_df"], x_mat=forecast_result["x_mat"])
    pred = model.predict(fut_df=forecast_result["fit_df"], x_mat=forecast_result["x_mat"])
    assert cst.PREDICTED_LOWER_COL in pred
    assert cst.PREDICTED_UPPER_COL in pred
    pred = model.predict(fut_df=forecast_result["predict_df"], x_mat=forecast_result["fut_x_mat"])
    assert cst.PREDICTED_LOWER_COL in pred
    assert cst.PREDICTED_UPPER_COL in pred
...
def test_is_residual_based_no_x_mat(forecast_result):
    ...
    model.fit(train_df=forecast_result["fit_df"], x_mat=None)
    pred = model.predict(fut_df=forecast_result["fit_df"])
    assert cst.PREDICTED_LOWER_COL in pred
    assert cst.PREDICTED_UPPER_COL in pred
    pred = model.predict(fut_df=forecast_result["predict_df"])
    assert cst.PREDICTED_LOWER_COL in pred
    assert cst.PREDICTED_UPPER_COL in pred
...
def test_non_is_residual_based_x_mat(forecast_result):
    ...
    model = QuantileRegressionUncertaintyModel(
        uncertainty_dict={"uncertainty_method": "quantile_regression", "params": {"value_col": "y", "is_residual_based": False}}, coverage=0.9
    )
    model.fit(train_df=forecast_result["fit_df"], x_mat=forecast_result["x_mat"])
    pred = model.predict(fut_df=forecast_result["fit_df"], x_mat=forecast_result["x_mat"])
    assert cst.PREDICTED_LOWER_COL in pred
