...
from abvelocity.ts.common.constants import EVENT_DF_DATE_COL, EVENT_DF_LABEL_COL
...
from abvelocity.ts.gk.sklearn.estimator.testing_utils import params_component_breakdowns
...
def params():
    ...
    return {
        ...
        "fit_algorithm_dict": {"fit_algorithm": "sgd", "fit_algorithm_params": {"alpha": 0.1}},
...
def test_setup(params):
    ...
    coverage = 0.90
    silverkite = SimpleSilverkiteForecast()
    model = SimpleSilverkiteEstimator(silverkite=silverkite, score_func=mean_squared_error, coverage=coverage, null_model_params=None, **params)
    ...
    initialized_params_subset = {k: v for k, v in initialized_params.items() if k in params.keys()}
    ...
    model.fit(train_df)
    assert model.fit_algorithm_dict == {"fit_algorithm": "sgd", "fit_algorithm_params": {"alpha": 0.1}}
...
def test_setup2(params2):
    ...
    coverage = 0.90
    silverkite = SimpleSilverkiteForecast()
    model = SimpleSilverkiteEstimator(silverkite=silverkite, score_func=mean_squared_error, coverage=coverage, null_model_params=None, **params2)
    ...
    initialized_params = model.__dict__
    initialized_params_subset = {k: v for k, v in initialized_params.items() if k in params2.keys()}
    ...
    train_df = daily_data_reg().get("train_df").copy()
    model.fit(train_df)
    assert model.fit_algorithm_dict == {"fit_algorithm": "ridge", "fit_algorithm_params": None}
    assert model.model_dict is not None
    assert model.model_dict["training_evaluation"] is not None
    assert model.model_dict["test_evaluation"] is None
    assert model.pred_cols is not None
...
def test_score_function(daily_data_with_reg):
    ...
    assert score == pytest.approx(6.43, rel=1e-2)
...
def test_summary(daily_data):
    ...
    model = SimpleSilverkiteEstimator(fit_algorithm_dict={"fit_algorithm": "linear"})
    train_df = daily_data["train_df"].iloc[:30]
    model.summary()

    model.fit(train_df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL)
    assert model.fit_algorithm_dict == {"fit_algorithm": "linear", "fit_algorithm_params": None}
    model.summary()
...
def test_uncertainty_with_nonstandard_cols(daily_data):
    ...
    df = daily_data["df"].rename(columns={cst.TIME_COL: "t", cst.VALUE_COL: "z"})
    model.fit(df, time_col="t", value_col="z")
...
def test_auto_config():
    ...
    model.fit(X=df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL)
    # Seasonality is overridden by auto seasonality.
    assert model.model_dict["fs_components_df"][["name", "period", "order", "seas_names"]].equals(
...
def test_quantile_regression_uncertainty_model():
    ...
    # Residual based
    model = SimpleSilverkiteEstimator(coverage=0.99, uncertainty_dict=dict(uncertainty_method="quantile_regression", params=dict(is_residual_based=True)))
    model.fit(X=df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL)
    assert model.uncertainty_model is not None
    assert len(model.uncertainty_model.models) == 2
    df_fut = pd.DataFrame({"ts": pd.date_range(df["ts"].max(), freq="D", periods=15), "y": np.nan}).iloc[1:].reset_index(drop=True)
    ...
    # Not residual based
    model = SimpleSilverkiteEstimator(coverage=0.95, uncertainty_dict=dict(uncertainty_method="quantile_regression", params=dict(is_residual_based=False)))
    model.fit(X=df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL)
    assert model.uncertainty_model is not None
    assert len(model.uncertainty_model.models) == 2
