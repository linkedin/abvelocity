...
from abvelocity.ts.gk.sklearn.estimator.base_silverkite_estimator import BaseSilverkiteEstimator
...
def test_init(params):
    ...
    model = BaseSilverkiteEstimator(score_func=mean_squared_error, coverage=coverage, null_model_params=None, uncertainty_dict=uncertainty_dict)
...
def test_null_model(X):
    """Checks null model"""
    model = BaseSilverkiteEstimator(null_model_params={"strategy": "quantile", "constant": None, "quantile": 0.8})
    ...
    model = BaseSilverkiteEstimator(score_func=mean_absolute_error, null_model_params={"strategy": "quantile", "constant": None, "quantile": 0.8})
    model.fit(X)
    y = np.repeat(2.0, X.shape[0])
    null_score = model.null_model.score(X, y=y)
...
def test_fit_predict(daily_data):
    ...
    with pytest.raises(NotFittedError, match="Call `fit` before calling `predict`."):
    ...
    model.model_dict = silverkite.forecast(
        ...
        fs_components_df=pd.DataFrame(
            {"name": ["tod", "tow", "conti_year"], "period": [24.0, 7.0, 1.0], "order": [3, 3, 5], "seas_names": ["daily", "weekly", "yearly"]}
        ),
        autoreg_dict=None,
        min_admissible_value=None,
        max_admissible_value=None,
    ...
    with pytest.raises(NotFittedError, match="Subclass must call `finish_fit` inside the `fit` method."):
        model.predict(train_df)
    ...
    assert_equal(model.coef_, pd.DataFrame(model.model_dict["ml_model"].coef_, index=model.feature_cols))
    ...
    with LogCapture(cst.LOGGER_NAME) as log_capture:
        ...
        log_capture.check((cst.LOGGER_NAME, "DEBUG", "``past_df`` not provided during prediction, use the ``train_df`` from " "training results."))

    # Uses cached predictions
    with LogCapture(cst.LOGGER_NAME) as log_capture:
        assert_equal(model.predict(test_df), predicted)
        log_capture.check((cst.LOGGER_NAME, "DEBUG", "Returning cached predictions."))
...
def test_score_function(daily_data_with_reg):
    ...
    assert score == pytest.approx(6.5, rel=1e-1)
...
def test_set_uncertainty_dict(daily_data):
    ...
    model = BaseSilverkiteEstimator(coverage=coverage, uncertainty_dict=uncertainty_dict)
    model.fit(train_df)
...
def test_silverkite_with_components_daily_data(expected_component_names):
    ...
    daily_data = generate_df_with_reg_for_tests(freq="D", periods=35, train_start_date=datetime.datetime(2018, 1, 1), conti_year_origin=2018)
    ...
    model = BaseSilverkiteEstimator(coverage=coverage, uncertainty_dict=params_daily["uncertainty_dict"])

    with pytest.raises(NotFittedError, match="Call `fit` before calling `plot_components`."):
    ...
    with pytest.warns(Warning):
        ...
        model.fit(X=train_df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL)
        silverkite = SilverkiteForecast()
        model.model_dict = silverkite.forecast(df=train_df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL, **params_daily)
        model.finish_fit()
    ...
    with pytest.raises(ValueError, match="Call the predict"):
    ...
    with pytest.raises(TypeError, match="center_components must be bool: True/False"):
        model.plot_components(center_components="Yes please")

    with pytest.raises(ValueError, match="Choose denominator from"):
        model.plot_components(denominator="Abs_Y")

    with pytest.raises(TypeError, match="grouping_regex_patterns_dict must be"):
        model.plot_components(grouping_regex_patterns_dict=[])

    with pytest.raises(ValueError, match="grouping_regex_patterns_dict must be"):
        model.plot_components(grouping_regex_patterns_dict={})
    ...
    with pytest.warns(Warning):
        ...
        model.finish_fit()
    assert model.coef_ is None
    with pytest.raises(NotImplementedError, match="Component plot has only been implemented for additive linear models."):
        model.plot_components()


def test_silverkite_with_components_hourly_data(expected_component_names):
    """Tests ``plot_components`` with hourly data"""
    hourly_data = generate_df_with_reg_for_tests(freq="H", periods=24 * 3, train_start_date=datetime.datetime(2018, 1, 1), conti_year_origin=2018)
    ...
    model = BaseSilverkiteEstimator(coverage=coverage, uncertainty_dict=params_hourly["uncertainty_dict"])
    model.fit(X=train_df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL)
    silverkite = SilverkiteForecast()
    model.model_dict = silverkite.forecast(df=train_df, time_col=cst.TIME_COL, value_col=cst.VALUE_COL, **params_hourly)
    model.finish_fit()
...
def test_plot_trend_changepoint_detection(df_pt):
    ...
    silverkite = SilverkiteForecast()
    model.model_dict = silverkite.forecast(df=df_pt, time_col=cst.TIME_COL, value_col=cst.VALUE_COL, **params)
    model.finish_fit()
...
def test_model_summary(df_pt):
    model = BaseSilverkiteEstimator()
    model.fit(X=df_pt.iloc[:100], time_col=cst.TIME_COL, value_col=cst.VALUE_COL)  # speeds up
    params = {"fit_algorithm": "ridge", "training_fraction": 0.8}
    silverkite = SilverkiteForecast()
    model.model_dict = silverkite.forecast(df=df_pt.iloc[:100], time_col=cst.TIME_COL, value_col=cst.VALUE_COL, **params)
    model.finish_fit()
...
def test_pred_category(df_pt):
    ...
    with pytest.raises(NotFittedError, match="Must fit before getting predictor category."):
        print(model.pred_category)
    model.fit(X=df_pt.iloc[:100], time_col=cst.TIME_COL, value_col=cst.VALUE_COL)  # speeds up
    params = {"fit_algorithm": "ridge", "training_fraction": 0.8, "extra_pred_cols": ["ct1", "x", "x:ct1"]}
...
def test_pred_category_regressor(df_pt):
    ...
    params = {
        ...
        "extra_pred_cols": ["ct1", "x", "x:ct1", "weather_wghtd_avg_cld_cvr_tot_pct_max_mid"],
    ...
    silverkite = SilverkiteForecast()
    model.model_dict = silverkite.forecast(df=df_pt.iloc[:100], time_col=cst.TIME_COL, value_col=cst.VALUE_COL, **params)
    model.extra_pred_cols = ["ct1", "x", "x:ct1", "weather_wghtd_avg_cld_cvr_tot_pct_max_mid", "media_total_spend_lag4"]  # set in subclass initialization
    ...
    assert pred_category["lag_features"] == ["media_total_spend_lag4_lag5", "media_total_spend_lag4_avglag_7_14_21", "media_total_spend_lag4_avglag_8_to_14"]
    assert pred_category["regressor_features"] == ["x", "x:ct1", "weather_wghtd_avg_cld_cvr_tot_pct_max_mid"]
    assert pred_category["interaction_features"] == ["x:ct1"]
...
def test_get_max_ar_order():
    ...
    model.autoreg_dict = dict(lag_dict=dict(orders=[3, 5]), agg_lag_dict=dict(interval_list=[(7, 15)]))
...
def test_past_df_in_predict(daily_data):
    ...
    pred = model.predict(X=past_df.iloc[[-1]])
    assert pred[cst.PREDICTED_COL].iloc[0] == pytest.approx(model.model_dict["ml_model"].intercept_ + coef[0] + coef[1] * past_df[cst.VALUE_COL].iloc[-2])
...
def test_x_mat_in_predict(daily_data):
    ...
    pred_df = model.predict(test_df)
    cols = [cst.TIME_COL, cst.QUANTILE_SUMMARY_COL, cst.ERR_STD_COL, cst.PREDICTED_LOWER_COL, cst.PREDICTED_UPPER_COL]
...
def test_forecast_breakdown(daily_data):
    ...
    forecast_x_mat = model.forecast_x_mat
    assert list(forecast_x_mat.columns) == ["Intercept", "ct1", "ct2", "month", "dow"]
    assert len(forecast_x_mat) == len(pred_df)

    grouping_regex_patterns_dict = {"seasonality": ".*month.*|dow", "growth": "ct1|ct2"}

    # applies to estimator with original forecasts
    breakdown_result = model.forecast_breakdown(grouping_regex_patterns_dict=grouping_regex_patterns_dict)
    ...
    assert column_grouping_result == {"str_groups": [["month", "dow"], ["ct1", "ct2"]], "remainder": []}
    ...
    breakdown_result = model.forecast_breakdown(grouping_regex_patterns_dict=grouping_regex_patterns_dict, forecast_x_mat=forecast_x_mat.head(10))
    ...
    breakdown_result = model.forecast_breakdown(
        grouping_regex_patterns_dict=grouping_regex_patterns_dict, forecast_x_mat=forecast_x_mat.head(10), time_values=pred_df.head(10)[cst.TIME_COL]
    )
...
def test_forecast_breakdown_center_components(daily_data):
    ...
    breakdown_result = model.forecast_breakdown(grouping_regex_patterns_dict=grouping_regex_patterns_dict, center_components=False)
    ...
    breakdown_result = model.forecast_breakdown(grouping_regex_patterns_dict=grouping_regex_patterns_dict, center_components=True)
