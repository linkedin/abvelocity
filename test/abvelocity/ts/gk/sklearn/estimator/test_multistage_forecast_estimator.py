...
@pytest.fixture
def daily_data():
    data = generate_df_for_tests(freq="D", periods=500, train_frac=0.99, train_start_date=datetime.datetime(2018, 1, 1), conti_year_origin=2018)
    return data
...
def test_get_agg_func(params):
    model = MultistageForecastEstimator(**params)
    with pytest.raises(ValueError, match="The aggregation function "):
...
def test_get_num_points_per_agg_freq(params):
    model = MultistageForecastEstimator(**params)
    assert model._get_num_points_per_agg_freq(data_freq="H", agg_freqs=["D", "2H"]) == [24, 2]
...
def test_add_agg_freq_cols(params, hourly_data_with_reg):
    ...
    assert list(df.columns) == [cst.TIME_COL, cst.VALUE_COL, "regressor", f"{cst.TIME_COL}__0__D", f"{cst.TIME_COL}__1__H"]
    ...
    # Tests error.
    with pytest.raises(ValueError, match="The df size is zero. Does your"):
...
def test_drop_incomplete_agg(params, hourly_data_with_reg):
    ...
    df_new = model._drop_incomplete_agg(df=df, agg_freq="D", location=-1, num_points_per_agg_freq=24, index=0)
...
def test_aggregate_values(params, hourly_data_with_reg):
    ...
    model._initialize()
    df = model._add_agg_freq_cols(df=hourly_data_with_reg)
    df_agg = model._aggregate_values(df=df[[f"{cst.TIME_COL}__0__D", cst.VALUE_COL, "regressor"]], agg_freq="D", agg_func=np.nanmean, index=0)
...
def test_train_and_predict(params, hourly_data_with_reg):
    ...
    pred = model.predict(
        pd.DataFrame(
            {cst.TIME_COL: pd.date_range(start=hourly_data_with_reg[cst.TIME_COL].max(), freq="H", periods=13)[1:], cst.VALUE_COL: np.nan, "regressor": 1}
        )
    )
...
def test_error(params, hourly_data_with_reg):
    ...
    with pytest.raises(ValueError, match="Please call `fit` before"):
    ...
    with pytest.raises(ValueError, match="The minimum aggregation frequency"):
        model.fit(hourly_data_with_reg)
...
def test_incomplete_fit_df_warning(params, hourly_data_with_reg):
    ...
    with LogCapture(cst.LOGGER_NAME) as log_capture:
        ...
        log_capture.check((cst.LOGGER_NAME, "WARNING", "There are incomplete periods in `fit_df`, thus the regressor values are " "biased after aggregation."))
...
def test_missing_timestamps_during_aggregation(params, hourly_data_with_reg):
    ...
    with LogCapture(cst.LOGGER_NAME) as log_capture:
        model._drop_incomplete_agg_and_aggregate_values(df=df, agg_freq="D", agg_func=np.mean, num_points_per_agg_freq=24, drop_incomplete=True, index=0)
    ...
    with LogCapture(cst.LOGGER_NAME) as log_capture:
        ...
        log_capture.check_present(
            (
                cst.LOGGER_NAME,
                "INFO",
                "Some `fit_length` is None or is shorter than `train_length`. " "These `fit_length` have been replaced with `train_length`.",
            )
        )
...
def test_uncertainty(hourly_data_with_reg, params):
    ...
    params["uncertainty_dict"] = dict(uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params={})
    model = MultistageForecastEstimator(**params)
    # fit
    model.fit(hourly_data_with_reg)
    # predict
    ...
    assert (pred[cst.PREDICTED_LOWER_COL] + pred[cst.PREDICTED_UPPER_COL]).round(2).equals((pred[cst.PREDICTED_COL] * 2).round(2))
...
def test_uncertainty_nonstandard_cols(hourly_data_with_reg, params):
    ...
    model = MultistageForecastEstimator(**params)
    # fit
    model.fit(hourly_data_with_reg.rename(columns={cst.TIME_COL: "t", cst.VALUE_COL: "z"}), time_col="t", value_col="z")
    # predict
    pred = model.predict(
        pd.DataFrame(
            {
...
def test_uncertainty_with_error(hourly_data_with_reg, params):
    ...
    with LogCapture(cst.LOGGER_NAME) as log_capture:
        params["coverage"] = 0.95
        params["uncertainty_dict"] = dict(uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params={"value_col": "non_exist"})
        model = MultistageForecastEstimator(**params)
        # fit
        model.fit(hourly_data_with_reg)
        # predict
...
def test_same_agg_freq(daily_data, config_silverkite_daily, config_silverkite_daily_2):
    ...
    params = dict(forecast_horizon=forecast_horizon, freq="D", model_configs=[config_silverkite_daily, config_silverkite_daily_2])
    model = MultistageForecastEstimator(**params)
    # Tests fit
    model.fit(df)
    # Checks models
    ...
    params = dict(
        ...
        model_configs=[
            ...
            MultistageForecastModelConfig(
                ...
                estimator_params=dict(lags=[1], lag_unit="week", agg_func="mean", series_na_fill_func=lambda x: x.bfill().ffill()),
            ),
        ],
    )
    model = MultistageForecastEstimator(**params)
