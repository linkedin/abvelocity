...
from abvelocity.ts.common.evaluation import r2_null_model_score
...
@pytest.fixture
def params():
    return {"score_func": mean_absolute_error, "coverage": 0.9, "null_model_params": {"strategy": "quantile", "constant": None, "quantile": 0.8}}
...
class ConstantBaseForecastEstimator(BaseForecastEstimator):
    ...
    def predict(self, X, y=None):
        ...
        predictions = pd.DataFrame({TIME_COL: X[self.time_col_], PREDICTED_COL: np.repeat(cst_pred, X.shape[0])})
...
def test_fit_predict(params, X_custom):
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        expected = pd.DataFrame({TIME_COL: X_custom["time_name"], PREDICTED_COL: np.repeat(cst_pred, X_custom.shape[0])})
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        log_capture.check((LOGGER_NAME, "DEBUG", "Returning cached predictions."))
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        expected = pd.DataFrame({TIME_COL: X_new["time_name"], PREDICTED_COL: np.repeat(cst_pred, X_new.shape[0])})
        assert_frame_equal(predicted, expected)
    ...
    null_expected = pd.DataFrame({TIME_COL: X_custom["time_name"], PREDICTED_COL: np.repeat(8.0, X_custom.shape[0])})
...
def test_populate_uncertainty_params():
    ...
    assert model._populate_uncertainty_params(uncertainty_dict=uncertainty_dict) == dict(
        uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(value_col=VALUE_COL, residual_col="residual_col")
    )

    # No ``residual_col``.
    uncertainty_dict = dict(uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(value_col="some_col"))
    assert model._populate_uncertainty_params(uncertainty_dict=uncertainty_dict) == dict(
        uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(value_col="some_col", residual_col="residual_col")
    )

    # No ``value_col``.
    uncertainty_dict = dict(uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(residual_col="some_col"))
    assert model._populate_uncertainty_params(uncertainty_dict=uncertainty_dict) == dict(
        uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(value_col=VALUE_COL, residual_col="some_col")
    )
...
def test_uncertainty(df):
    ...
    model.fit_uncertainty(df=df, uncertainty_dict={})
    ...
    df_pred = model.predict_uncertainty(df=df)
    assert PREDICTED_LOWER_COL in df_pred.columns
    assert PREDICTED_UPPER_COL in df_pred.columns
    assert (df_pred[PREDICTED_LOWER_COL] + df_pred[PREDICTED_UPPER_COL]).round(2).equals((df_pred[PREDICTED_COL] * 2).round(2))

    # Both coverage and uncertainty dict are given.
    model.fit_uncertainty(
        df=df,
        uncertainty_dict=dict(
            uncertainty_method=UncertaintyMethodEnum.simple_conditional_residuals.name, params=dict(value_col=VALUE_COL, is_residual_based=True)
        ),
    )
...
def test_uncertainty_errors(df):
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        # Only coverage is given.
        # The default uncertainty method is used.
        res = model.fit_uncertainty(df=df, uncertainty_dict=dict(uncertainty_method="some_method"))
        assert (
            (
                LOGGER_NAME,
                "WARNING",
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        model.time_col_ = TIME_COL
        model.value_col_ = VALUE_COL
        model.predict_uncertainty(df=df)
        assert ((LOGGER_NAME, "WARNING", "The uncertainty model is not trained.")) in log_capture.actual()
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        model.fit_uncertainty(
            df=df,
            uncertainty_dict={"uncertainty_method": "simple_conditional_residuals", "params": {"is_residual_based": True, "predicted_col": PREDICTED_COL}},
        )
        model.predict_uncertainty(df=df[[TIME_COL]])
        assert (
            (
                LOGGER_NAME,
                "WARNING",
