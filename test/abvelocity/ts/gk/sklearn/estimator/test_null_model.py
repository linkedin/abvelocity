import numpy as np
import pandas as pd
from abvelocity.ts.common.constants import PREDICTED_COL, TIME_COL, VALUE_COL
...
def test_fit_predict():
    ...
    model = DummyEstimator()
    X = pd.DataFrame({TIME_COL: pd.date_range("2018-01-01", periods=3, freq="1D"), VALUE_COL: [2, 3, 4]})

    model.fit(X)
    predicted = model.predict(X)

    expected = pd.DataFrame({TIME_COL: X[TIME_COL], PREDICTED_COL: np.repeat(3.0, X.shape[0])})

    assert predicted.equals(expected)

    # with np.nan value
    model = DummyEstimator()
    X = pd.DataFrame({TIME_COL: pd.date_range("2018-01-01", periods=4, freq="1D"), VALUE_COL: [2, 3, np.nan, 4]})
...
def test_fit_predict1():
    ...
    X = pd.DataFrame({TIME_COL: pd.date_range("2018-01-01", periods=3, freq="1D"), VALUE_COL: [2, 3, 5]})

    df_test = pd.DataFrame({TIME_COL: pd.date_range("2018-01-01", periods=4, freq="1D")})

    model.fit(X, sample_weight=[1, 1, 2])
    predicted = model.predict(df_test)

    expected = pd.DataFrame({TIME_COL: df_test[TIME_COL], PREDICTED_COL: np.repeat(3.75, df_test.shape[0])})
...
def test_constant_model():
    ...
    expected = pd.DataFrame({TIME_COL: X[TIME_COL], PREDICTED_COL: np.repeat(constant, X.shape[0])})
...
def test_quantile_model():
    ...
    X = pd.DataFrame({"time_name": pd.date_range("2018-01-01", periods=11, freq="D"), "value_name": np.arange(11)})

    model.fit(X, time_col="time_name", value_col="value_name")
    predicted = model.predict(X)

    expected = pd.DataFrame({TIME_COL: X["time_name"], PREDICTED_COL: np.repeat(8.0, X.shape[0])})
...
def test_score():
    ...
    X = pd.DataFrame({TIME_COL: pd.date_range("2018-01-01", periods=3, freq="1D"), VALUE_COL: [2.0, 3.0, 4.0]})
