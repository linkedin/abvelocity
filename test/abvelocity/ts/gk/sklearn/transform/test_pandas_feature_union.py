...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import ACTUAL_COL, TIME_COL, VALUE_COL
...
from abvelocity.ts.gk.sklearn.transform.zscore_outlier_transformer import ZscoreOutlierTransformer
...
def fs():
    ...
    return PandasFeatureUnion(
        [
            ("date", Pipeline([("select_date", ColumnSelector([TIME_COL]))])),  # leaves time column unmodified
            (
                "response",
                Pipeline(
...
def test_feature_union(X):
    ...
    fs = PandasFeatureUnion(
        [("select_value", select_value), ("select_actual", select_actual), ("select_time", select_time), ("select_time_again", select_time)]
    )
...
def test_pipeline_union(X, fs):
    ...
    model_estimator = Pipeline(
        [("input", fs), ("estimator", SimpleSilverkiteEstimator(score_func=mean_squared_error, coverage=0.80, null_model_params={"strategy": "mean"}))]
    )
    ...
    model_dummy = Pipeline([("input", fs), ("dummy", DummyEstimator(score_func=mean_squared_error, strategy="mean"))])
    model_dummy.fit(X)
