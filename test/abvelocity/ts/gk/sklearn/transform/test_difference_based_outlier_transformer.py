...
def test_difference_based_outlier_transformer(data):
    ...
    transformer = DifferenceBasedOutlierTransformer(
        method="z_score", score_type="difference", params=dict(agg_func=np.nanmean, lag_orders=[-1, 1], z_cutoff=3.5, max_outlier_percent=5.0)
    )
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        log_capture.check((LOGGER_NAME, "INFO", "Detected 6 outlier(s)."))

    transformer = DifferenceBasedOutlierTransformer(
        method="tukey", score_type="ratio", params=dict(agg_func=np.nanmean, lag_orders=[-1, 1], tukey_cutoff=3.5, max_outlier_percent=5.0)
    )
    # init does not modify parameters.
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        assert_equal(result, expected)
        log_capture.check((LOGGER_NAME, "INFO", "Detected 5 outlier(s)."))

    transformer = DifferenceBasedOutlierTransformer(method="neither_z_score_nor_tukey")
    with pytest.raises(NotImplementedError, match="is an invalid 'method'"):
        transformer.fit(data)

    transformer = DifferenceBasedOutlierTransformer(
        method="z_score", score_type="neither_difference_nor_ratio", params=dict(agg_func=np.nanmean, lag_orders=[-1, 1], z_cutoff=3.0, max_outlier_percent=5.0)
    )
    with pytest.raises(NotImplementedError, match="is an invalid 'score_type'"):
        transformer.fit(data)

    transformer = DifferenceBasedOutlierTransformer(
        method="z_score", score_type="difference", params=dict(agg_func=np.nanmean, lag_orders=[-1, 1], z_cutoff=3.0, max_outlier_percent=5.0)
    )
