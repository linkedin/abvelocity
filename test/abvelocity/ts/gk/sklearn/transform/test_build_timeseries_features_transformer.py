...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import VALUE_COL
from abvelocity.ts.common.features.timeseries_features import build_time_features_df
from abvelocity.ts.gk.sklearn.transform.build_timeseries_features_transformer import BuildTimeseriesFeaturesTransformer
...
def test_BuildTimeseriesFeaturesTransformer_1():
    ...
    date_list = pd.date_range(start=datetime(2019, 1, 1), periods=100, freq="H").tolist()
...
def test_BuildTimeseriesFeaturesTransformer_2():
    ...
    result = timeseries_transform.fit_transform(df)
    pd.testing.assert_series_equal(result["year"], pd.Series([2018, 2018, 2018, 2018]), check_dtype=False, check_names=False)
    pd.testing.assert_series_equal(result["month"], pd.Series([1, 1, 1, 1]), check_dtype=False, check_names=False)
    pd.testing.assert_series_equal(result["dom"], pd.Series([1, 2, 3, 4]), check_dtype=False, check_names=False)
    pd.testing.assert_series_equal(result["hour"], pd.Series([1, 2, 4, 10]), check_dtype=False, check_names=False)
