...
def data():
    ...
    return pd.DataFrame({"a": (0.0, np.nan, -1.0, 1.0), "b": (np.nan, 2.0, np.nan, np.nan), "c": (2.0, 3.0, np.nan, 9.0), "d": (np.nan, 4.0, -4.0, 16.0)})
...
def test_null_transformer1(data):
    ...
    with pytest.raises(ValueError, match="method 'ffill' is not allowed"):
        NullTransformer(impute_algorithm="interpolate", impute_params=dict(method="ffill"), impute_all=False)

    # impute_all=False leaves NaNs
    impute_params = dict(method="linear", limit_direction="forward")
    null_transform = NullTransformer(max_frac=0.8, impute_algorithm="interpolate", impute_params=impute_params, impute_all=False)
    ...
    null_transform = NullTransformer(impute_algorithm="interpolate", impute_params=dict(method="linear", limit_direction="forward"), impute_all=True)
...
def test_null_transformer2(data):
    ...
    null_transform = NullTransformer(impute_algorithm="ts_interpolate", impute_all=False)
    null_transform.fit(data)
    assert null_transform.impute_params == dict(orders=[7, 14, 21], agg_func=np.mean, iter_num=5)
    result = null_transform.transform(data)
    # `orders` is too large for this dataset, nothing is imputed
    assert_equal(result, data)

    # two iterations
    null_transform = NullTransformer(impute_algorithm="ts_interpolate", impute_params=dict(orders=[1], agg_func=np.nanmean, iter_num=2), impute_all=False)
    result = null_transform.fit_transform(data)
    expected = pd.DataFrame(
        {
            "a": (0.0, 0.0, -1.0, 1.0),
    ...
    # impute_all=True
    null_transform = NullTransformer(impute_algorithm="ts_interpolate", impute_params=dict(orders=[1], agg_func=np.nanmean, iter_num=2), impute_all=True)
    result = null_transform.fit_transform(data)
    expected = pd.DataFrame(
        {
            "a": (0.0, 0.0, -1.0, 1.0),
...
def test_null_transformer3(data):
    ...
    null_transform = NullTransformer(impute_all=False)
    null_transform.fit(data)
    ...
    null_transform = NullTransformer(impute_algorithm=None, impute_all=True)
    null_transform.fit(data)
    assert null_transform.impute_params is None
    result = null_transform.transform(data)
    assert_equal(result, data)
    ...
    with LogCapture(LOGGER_NAME) as log_capture:
        ...
        log_capture.check(
            (
                LOGGER_NAME,
                "INFO",
                "Missing data detected: 37.50% of all input values are null. "
