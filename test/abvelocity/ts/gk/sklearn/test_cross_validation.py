...
def test_rolling_time_series_split3():
    ...
    with pytest.warns(Warning) as record:
        max_splits = 1
        tscv = RollingTimeSeriesSplit(
            forecast_horizon=2, min_train_periods=4, expanding_window=True, periods_between_splits=4, periods_between_train_test=2, max_splits=max_splits
        )
...
def test_rolling_time_series_split_empty():
    ...
    tscv = RollingTimeSeriesSplit(
        forecast_horizon=50, min_train_periods=160, expanding_window=True, periods_between_splits=4, periods_between_train_test=0, max_splits=None
    )
...
def test_get_n_splits():
    ...
    tscv = RollingTimeSeriesSplit(
        forecast_horizon=10, min_train_periods=20, expanding_window=True, periods_between_splits=16, periods_between_train_test=3, max_splits=None
    )
    assert tscv.get_n_splits(X=X) == 5
    ...
    # normal case, no capping
    tscv = RollingTimeSeriesSplit(
        forecast_horizon=7, min_train_periods=30, expanding_window=False, periods_between_splits=30, periods_between_train_test=10, max_splits=None
    )
    ...
    # max_splits reached
    tscv = RollingTimeSeriesSplit(
        forecast_horizon=10, min_train_periods=20, expanding_window=True, periods_between_splits=16, periods_between_train_test=3, max_splits=4
    )
    assert tscv.get_n_splits(X=X) == 4
    assert tscv.get_n_splits_without_capping(X=X) == 5
    ...
    tscv = RollingTimeSeriesSplit(
        forecast_horizon=10, min_train_periods=90, expanding_window=True, periods_between_splits=16, periods_between_train_test=3, max_splits=None
    )
    assert tscv.get_n_splits(X=X) == 1
...
def test_get_offset():
    """Tests _get_offset"""
    tscv = RollingTimeSeriesSplit(forecast_horizon=5, min_train_periods=10, periods_between_train_test=5, periods_between_splits=5)
    ...
    tscv = RollingTimeSeriesSplit(forecast_horizon=3, min_train_periods=6, periods_between_train_test=11, periods_between_splits=3)
    assert tscv._get_offset(X=np.random.rand(20, 1)) == 0
    assert tscv._get_offset(X=np.random.rand(21, 1)) == 1
    assert tscv._get_offset(X=np.random.rand(22, 1)) == 2
