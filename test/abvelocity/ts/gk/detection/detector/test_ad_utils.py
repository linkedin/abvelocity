...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import ANOMALY_COL, END_TIME_COL, LOGGER_NAME, START_TIME_COL, TIME_COL, TimeFeaturesEnum
from abvelocity.ts.common.testing_utils import assert_equal
from abvelocity.ts.gk.detection.detector.ad_utils import (
    add_new_params_to_records,
    get_anomaly_df,
    get_anomaly_df_from_outliers,
    get_canonical_anomaly_df,
    get_timestamp_ceil,
    get_timestamp_floor,
    optimize_df_with_constraints,
    partial_return,
...
def test_vertical_concat_dfs():
    ...
    df0 = pd.DataFrame({"ts": [0, 1, 2, 3, 4], "day": ["Mon", "Tue", "Wed", "Thu", "Fri"], "y": [10, 20, 30, 40, 50]})

    df1 = pd.DataFrame({"ts": [0, 1, 2, 3, 4], "day": ["Mon", "Tue", "Wed", "Thu", "Fri"], "y": [11, 21, 31, 41, 51]})

    df = vertical_concat_dfs(df_list=[df0, df1], join_cols=["ts"], common_value_cols=["day"], different_value_cols=["y"])

    expected_df = pd.DataFrame({"ts": [0, 1, 2, 3, 4], "day": ["Mon", "Tue", "Wed", "Thu", "Fri"], "y0": [10, 20, 30, 40, 50], "y1": [11, 21, 31, 41, 51]})
...
def test_add_new_params_to_records():
    ...
    grid_seed_dict = {"a": [1, 2, 3], "cat": ["boz", "asb"]}

    var_names = list(grid_seed_dict.keys())
    combinations_list = list(itertools.product(*[grid_seed_dict[var] for var in var_names]))

    df = pd.DataFrame(combinations_list, columns=var_names)

    records = df.to_dict("records")
    expanded_param_list = add_new_params_to_records(new_params={"dog": [1, 3], "horse": [13, 17]}, records=records)
...
def test_get_anomaly_df():
    ...
    # All anomalies
    df = pd.DataFrame(
        {TIME_COL: pd.date_range(start="2020-01-01", periods=10, freq="D"), ANOMALY_COL: [True, True, True, True, True, True, True, True, True, True]}
    )
    anomaly_df = get_anomaly_df(df=df, time_col=TIME_COL, anomaly_col=ANOMALY_COL)
    expected_anomaly_df = pd.DataFrame({START_TIME_COL: pd.to_datetime(["2020-01-01"]), END_TIME_COL: pd.to_datetime(["2020-01-10"])})
    assert_equal(anomaly_df, expected_anomaly_df)

    # No anomalies
    df = pd.DataFrame(
        {TIME_COL: pd.date_range(start="2020-01-01", periods=10, freq="D"), ANOMALY_COL: [False, False, False, False, False, False, False, False, False, False]}
    )
    anomaly_df = get_anomaly_df(df=df, time_col=TIME_COL, anomaly_col=ANOMALY_COL)
    expected_anomaly_df = pd.DataFrame({START_TIME_COL: pd.to_datetime([]), END_TIME_COL: pd.to_datetime([])})
    assert_equal(anomaly_df, expected_anomaly_df)

    # Distinct anomalies (single data point and multiple data points)
    df = pd.DataFrame(
        {TIME_COL: pd.date_range(start="2020-01-01", periods=10, freq="D"), ANOMALY_COL: [False, True, True, True, False, True, False, False, False, False]}
    )
    anomaly_df = get_anomaly_df(df=df, time_col=TIME_COL, anomaly_col=ANOMALY_COL)
    expected_anomaly_df = pd.DataFrame(
        {START_TIME_COL: pd.to_datetime(["2020-01-02", "2020-01-06"]), END_TIME_COL: pd.to_datetime(["2020-01-04", "2020-01-06"])}
    )
    assert_equal(anomaly_df, expected_anomaly_df)
...
def test_get_canonical_anomaly_df():
    ...
    anomaly_df = pd.DataFrame({START_TIME_COL: pd.to_datetime(["2020-01-01", "2020-02-01"]), END_TIME_COL: pd.to_datetime(["2020-01-02", "2020-02-05"])})
    canonical_anomaly_df = get_canonical_anomaly_df(anomaly_df=anomaly_df, freq="D")
    ...
    canonical_anomaly_df = get_canonical_anomaly_df(anomaly_df=anomaly_df, freq="H", start_time_col="begin", end_time_col="end")
    expected_anomaly_df = pd.DataFrame({START_TIME_COL: pd.to_datetime(["2020-01-02-03"]), END_TIME_COL: pd.to_datetime(["2020-01-02-20"])})
    assert_equal(canonical_anomaly_df, expected_anomaly_df)
    # One anomaly period covers others
    anomaly_df = pd.DataFrame(
        {"begin": ["2020-02-05", "2020-02-19", "2020-02-03", "2020-02-05"], "end": ["2020-02-15", "2020-02-20", "2020-02-17", "2020-02-13"]}
    )
    canonical_anomaly_df = get_canonical_anomaly_df(anomaly_df=anomaly_df, freq="D", start_time_col="begin", end_time_col="end")
    expected_anomaly_df = pd.DataFrame(
        {START_TIME_COL: pd.to_datetime(["2020-02-03", "2020-02-19"]), END_TIME_COL: pd.to_datetime(["2020-02-17", "2020-02-20"])}
    )
    assert_equal(canonical_anomaly_df, expected_anomaly_df)
    # Checks anomaly merging logic.
    anomaly_df = pd.DataFrame(
        {START_TIME_COL: pd.to_datetime(["2020-01-01", "2020-01-02", "2020-01-04"]), END_TIME_COL: pd.to_datetime(["2020-01-02", "2020-01-03", "2020-01-05"])}
    )
    canonical_anomaly_df = get_canonical_anomaly_df(anomaly_df=anomaly_df, freq="D")
    expected_anomaly_df = pd.DataFrame({START_TIME_COL: pd.to_datetime(["2020-01-01"]), END_TIME_COL: pd.to_datetime(["2020-01-05"])})
    assert_equal(canonical_anomaly_df, expected_anomaly_df)
    # Checks anomaly merging logic
    anomaly_df = pd.DataFrame(
        {START_TIME_COL: pd.to_datetime(["2020-01-01", "2020-01-02", "2020-01-04"]), END_TIME_COL: pd.to_datetime(["2020-01-02", "2020-01-03", "2020-01-05"])}
    )
    canonical_anomaly_df = get_canonical_anomaly_df(anomaly_df=anomaly_df, freq="H")
    expected_anomaly_df = pd.DataFrame(
        {START_TIME_COL: pd.to_datetime(["2020-01-01", "2020-01-04"]), END_TIME_COL: pd.to_datetime(["2020-01-03", "2020-01-05"])}
    )
    assert_equal(canonical_anomaly_df, expected_anomaly_df)
...
def test_optimize_df_with_constraints():
    ...
    df = pd.DataFrame({"a": [20, 20, 5, 10, 10, 0, 5], "b": [1, 1, 2, 2, 3, 3, 4], "c": [1, 2, 3, 4, 5, 6, 7]})
    # Constraint is satisfied, unique optimal
    with LogCapture(LOGGER_NAME) as log_capture:
        optimal_dict = optimize_df_with_constraints(df=df, objective_col="a", constraint_col="b", constraint_value=4)
        ...
        log_capture.check(
            (
                LOGGER_NAME,
                "INFO",
                "Values satisfying the constraint are found.\n" "Solving the following optimization problem:\n" "Maximize a subject to b >= 4.",
            )
        )
    # Constraint is satisfied. Multiple rows with the maximum value
    # in ``objective_col``, resolved by considering ``constraint_col``
    optimal_dict = optimize_df_with_constraints(df=df, objective_col="a", constraint_col="b", constraint_value=2)
    expected_optimal_dict = {"a": 10, "b": 3, "c": 5}
    assert_equal(optimal_dict, expected_optimal_dict)
    # Constraint is satisfied. Multiple rows with the maximum value
    # in both ``objective_col`` and ``constraint_col``, last entry is chosen
    optimal_dict = optimize_df_with_constraints(df=df, objective_col="a", constraint_col="b", constraint_value=1)
    expected_optimal_dict = {"a": 20, "b": 1, "c": 2}
    assert_equal(optimal_dict, expected_optimal_dict)

    # Constraint is NOT satisfied, unique optimal
    with LogCapture(LOGGER_NAME) as log_capture:
        optimal_dict = optimize_df_with_constraints(df=df, objective_col="a", constraint_col="b", constraint_value=4.5)
        expected_optimal_dict = {"a": 5, "b": 4, "c": 7}
        assert_equal(optimal_dict, expected_optimal_dict)
        log_capture.check(
            (
                LOGGER_NAME,
                "INFO",
                "No values satisfy the constraint.\n"
                "Maximizing ``constraint_col`` (b) so that it is as "
                "close as possible to the ``constraint_value`` (4.5).",
            )
        )
    # Constraint is NOT satisfied. Multiple rows with the maximum value
    # in ``objective_col``, resolved by considering ``constraint_col``
    df = df[:6]
    optimal_dict = optimize_df_with_constraints(df=df, objective_col="a", constraint_col="b", constraint_value=3.5)
    expected_optimal_dict = {"a": 10, "b": 3, "c": 5}
    assert_equal(optimal_dict, expected_optimal_dict)
    # Constraint is NOT satisfied. Multiple rows with the maximum value
    # in both ``objective_col`` and ``constraint_col``, last entry is chosen
    df = df[:2]
    optimal_dict = optimize_df_with_constraints(df=df, objective_col="a", constraint_col="b", constraint_value=1.5)
    expected_optimal_dict = {"a": 20, "b": 1, "c": 2}
    assert_equal(optimal_dict, expected_optimal_dict)
...
def test_validate_volatility_features():
    ...
    validated_features_list = validate_volatility_features(volatility_features_list=volatility_features_list)
    ...
    with pytest.raises(ValueError, match="Unknown feature\\(s\\) \\({'is_holiday'}\\) in `volatility_features_list`."):
        ...
        validate_volatility_features(volatility_features_list=volatility_features_list, valid_features=valid_features)
...
def test_get_anomaly_df_from_outliers1():
    ...
    df = pd.DataFrame({TIME_COL: pd.date_range(start="2020-01-01", end="2020-02-11 16:00:00", freq=freq), value_col: [2] * 1001})
    empty_anomaly_df = pd.DataFrame(columns=[START_TIME_COL, END_TIME_COL])
    anomaly_df = get_anomaly_df_from_outliers(df=df, time_col=TIME_COL, value_col=value_col, freq=freq)
...
def test_get_anomaly_df_from_outliers2(y_clean):
    ...
    df = pd.DataFrame({TIME_COL: pd.date_range(start="2020-01-01", end="2020-02-11 15:00:00", freq=freq), value_col: y})
    expected_anomaly_df = pd.DataFrame({START_TIME_COL: pd.to_datetime(["2020-01-01 01:00:00"]), END_TIME_COL: pd.to_datetime(["2020-01-01 01:00:00"])})
    anomaly_df = get_anomaly_df_from_outliers(df=df, time_col=TIME_COL, value_col=value_col, freq=freq)
    assert_equal(anomaly_df, expected_anomaly_df)
...
def test_get_anomaly_df_from_outliers_raise_error():
    ...
    with pytest.raises(ValueError, match=f"`df` does not have `time_col` with name {non_exist_col}."):
        get_anomaly_df_from_outliers(df=df, time_col=non_exist_col, value_col=value_col, freq=freq)
    ...
    with pytest.raises(ValueError, match=f"`df` does not have `value_col` with name {non_exist_col}."):
        get_anomaly_df_from_outliers(df=df, time_col=TIME_COL, value_col=non_exist_col, freq=freq)
...
def test_get_anomaly_df_from_outliers_small_data_size():
    ...
    df = pd.DataFrame({TIME_COL: ts, value_col: y})

    expected_anomaly_df = pd.DataFrame({START_TIME_COL: pd.to_datetime(["2020-01-03"]), END_TIME_COL: pd.to_datetime(["2020-01-03"])})

    anomaly_df = get_anomaly_df_from_outliers(df=df, time_col=TIME_COL, value_col=value_col, freq=freq, trim_percent=1.0)
    assert_equal(anomaly_df, expected_anomaly_df)

    # Disables trimming by setting it to zero.
    # In this case, we do not expect an empty dataframe
    anomaly_df = get_anomaly_df_from_outliers(df=df, time_col=TIME_COL, value_col=value_col, freq=freq, trim_percent=0.0)
...
def test_get_anomaly_df_from_outliers_with_missing2(y_clean):
    ...
    ts = pd.date_range(start="2020-01-01", end="2020-02-11 15:00:00", freq=freq)
    assert len(ts) == len(y)
    df = pd.DataFrame({TIME_COL: ts, value_col: y})
...
def test_get_anomaly_df_from_outliers_with_missing_raise_error():
    ...
    # Tests when outliers exist and there is a missing value.
    # The outliers are to be identified and saved to `anomaly_df`.
    df = pd.DataFrame({TIME_COL: pd.date_range(start="2020-01-01", end="2020-02-11 15:00:00", freq=freq), value_col: [2, 1e10] + list(range(998))})

    df[value_col][2] = None
    # Captures Error when y does not have at least two values after removing outliers.
    df0 = df[:3]
    df0[value_col] = [None, 3, None]
    with pytest.raises(ValueError, match="Length of y after removing NAs is less than 2."):
        get_anomaly_df_from_outliers(df=df0, time_col=TIME_COL, value_col=value_col, freq=freq)
    ...
    with pytest.warns(UserWarning, match="After trimming there were less than two values:"):
        get_anomaly_df_from_outliers(df=df0, time_col=TIME_COL, value_col=value_col, freq=freq, trim_percent=5.0)
