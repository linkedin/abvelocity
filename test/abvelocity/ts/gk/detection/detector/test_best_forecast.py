...
from abvelocity.ts.gk.detection.detector.best_forecast import BestForecastDetector
...
def test_best_forecast_detector():
    ...
    detector = BestForecastDetector(
        value_cols=["y"], pred_cols=["y_pred"], is_anomaly_col="is_anomaly", join_cols=["ts"], reward=Reward(reward_func), param_iterable=APE_PARAM_ITERABLE
    )
    ...
    def add_features_one_df(joined_df):
        joined_df["ape"] = abs(joined_df["y"] - joined_df["y_pred"]) / abs(joined_df["y"])
        return joined_df
    ...
    for i in [0, 1]:
        ...
        assert joined_df_direct[common_cols].equals(joined_df_from_detector[common_cols])
