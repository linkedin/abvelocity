...
from abvelocity.ts.gk.detection.detector.forecast_based import ForecastBasedDetector
...
def test_forecast_based_detector():
    ...
    detector = ForecastBasedDetector(reward=Reward(reward_func), value_cols=["y"], pred_cols=["y_pred"], is_anomaly_col="is_anomaly", join_cols=["ts"])
    ...
    joined_dfs = detector.join_with_forecasts(df=df, forecast_dfs=forecast_dfs)
