...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import PREDICTED_ANOMALY_COL
from abvelocity.ts.common.viz.timeseries_annotate import plot_lines_markers
from abvelocity.ts.gk.detection.common.ad_evaluation import f1_score
from abvelocity.ts.gk.detection.detector.data import DetectorData as Data
from abvelocity.ts.gk.detection.detector.detector import Detector, build_anomaly_percent_reward
...
class TukeyDetector(Detector):
    ...
    def __init__(self, value_col, is_anomaly_col=None, reward=None, anomaly_percent_dict=None, param_iterable=None):

        super().__init__(reward=reward, anomaly_percent_dict=anomaly_percent_dict, param_iterable=param_iterable)
    ...
    def fit(self, data):
        ...
        default_param = {"q1": q1, "q3": q3, "iqr": iqr, "iqr_coef": None, "lower": None, "upper": None}

        y_true = df[self.is_anomaly_col]
        data = Data(df=df, y_true=y_true)

        param_iterable = self.param_iterable

        optim_res = self.optimize_param(data=data, param_iterable=param_iterable, default_param=default_param)
        ...
        self.fitted_df = self.predict(Data(df=df.copy()))
    ...
    def calc_with_param(self, param, data):
        ...
        df[PREDICTED_ANOMALY_COL] = (df[self.value_col] < param["lower"]) | (df[self.value_col] > param["upper"])
...
class NormalDetector(Detector):
    ...
    The optimizer implementation is inherited from
        `~abvelocity.ts.gk.detection.detector.detector.Detector`
    ...
    def __init__(self, value_col, is_anomaly_col=None, reward=None, anomaly_percent_dict=None, param_iterable=None):
        ...
        if param_iterable is None:
            step = 0.005
            self.param_iterable = [{"prob_thresh": x} for x in np.arange(0.5 + step, 1 - step, step)]
...
def test_normal_detector():
    ...
    detector = NormalDetector(value_col="y", anomaly_percent_dict=anomaly_percent_dict)
    ...
    fig = plot_lines_markers(df=param_obj_df, x_col="prob_thresh", line_cols=["obj_value"])
...
def test_iqr_detector():
    ...
    def reward_func(data):
        f1 = f1_score(y_true=data.y_true, y_pred=data.y_pred)
        return f1[True]
    ...
    df = pd.DataFrame({"y": list(anomalies) + list(np.random.normal(size=normal_size)), "is_anomaly": [True] * anomaly_size + [False] * normal_size})

    new_df = pd.DataFrame({"y": [500, -10, -100, 0, 0.1, -0.2, 0.3, 200, 8]})

    detector = TukeyDetector(is_anomaly_col="is_anomaly", value_col="y", reward=reward)
    ...
    param_obj_list = detector.fit_info["param_obj_list"]
    param_obj_df = pd.DataFrame.from_records(param_obj_list)
    fig = plot_lines_markers(df=param_obj_df, x_col="iqr_coef", line_cols=["obj_value"])
    ...
    assert np.allclose(y_pred, np.array([True] * 3 + [False] * 4 + [True] * 2))
