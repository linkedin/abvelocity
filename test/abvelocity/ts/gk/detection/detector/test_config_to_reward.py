...
from abvelocity.ts.gk.detection.detector.config import F1, PRECISION, RECALL, ADConfig
...
def test_config_to_reward():
    ...
    ad_config = ADConfig(objective=F1, soft_window_size=2)
    assert ad_config.objective == F1
    reward = config_to_reward(ad_config)
    ...
    ad_config = ADConfig(objective=RECALL, target_precision=raw_precision_value)
    reward = config_to_reward(ad_config)
    ...
    ad_config = ADConfig(objective=RECALL, target_precision=raw_precision_value + 0.01)
