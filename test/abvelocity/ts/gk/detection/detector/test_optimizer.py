...
from abvelocity.ts.gk.detection.detector.optimizer import CalcResult, Optimizer
...
def test_optimizer1():
    ...
    optimizer = Optimizer(reward=reward, param_iterable=[{"x": x} for x in np.arange(-5, 5, 0.1)])
    ...
    fig = plot_lines_markers(df=param_obj_df, x_col="x", line_cols=["obj_value"])
...
def test_optimizer2():
    ...
    fig = plot_lines_markers(df=param_obj_df, x_col="x", line_cols=["obj_value"])
    fig.layout.update(title="`Optimizer` parameter search for roots of `x**p + p*x + p`; p=3")
    assert fig is not None
    # fig.show()
