...
def test_reward():
    ...
    obj_value = Reward(f1, min_unpenalized=0.75, penalty=-1).apply(y_true=y_true, y_pred=y_pred)

    assert obj_value == raw_f1_value - 1.0

    # The penalty won't take effect since `min_unpenalized < raw_f1_value`
    obj_value = Reward(f1, min_unpenalized=0.70, penalty=-1).apply(y_true=y_true, y_pred=y_pred)

    assert obj_value == raw_f1_value

    # Multiplicative penalty
    obj_value = Reward(f1, min_unpenalized=0.75, penalize_method="multiplicative", penalty=0.1).apply(y_true=y_true, y_pred=y_pred)

    assert obj_value == raw_f1_value * 0.1

    # Recall, penalty won't take effect since `0.66 < raw_recall_value`
    obj_value = Reward(rec, min_unpenalized=0.66, penalize_method="additive", penalty=-1).apply(y_true=y_true, y_pred=y_pred)

    assert obj_value == raw_recall_value

    # Recall, penalty will take effect since `0.66 > raw_recall_value`
    obj_value = Reward(rec, max_unpenalized=0.66, penalize_method="additive", penalty=+3.0).apply(y_true=y_true, y_pred=y_pred)
    ...
    combined_reward = Reward(rec, min_unpenalized=0.8, penalize_method="additive", penalty=-1) + Reward(f1)

    obj_value = combined_reward.apply(y_true=y_true, y_pred=y_pred)
    assert obj_value == pytest.approx(raw_f1_value + raw_recall_value - 1.0)
    ...
    obj_value = combined_reward.apply(y_true=y_true, y_pred=y_pred)
    assert obj_value == pytest.approx(17.0 / raw_f1_value)
    ...
    combined_reward = Reward(rec, min_unpenalized=0.8, penalize_method="additive", penalty=float("-inf")) + Reward(f1)

    obj_value = combined_reward.apply(y_true=y_true, y_pred=y_pred)
    assert obj_value == float("-inf")

    # Multiplication of recall and F1 with penalty on the recall part
    combined_reward = Reward(rec, min_unpenalized=0.8, penalize_method="multiplicative", penalty=0.1) * Reward(f1)

    obj_value = combined_reward.apply(y_true=y_true, y_pred=y_pred)
    assert obj_value == pytest.approx(raw_f1_value * raw_recall_value * 0.1)
    ...
    assert obj_value == pytest.approx(raw_f1_value)
