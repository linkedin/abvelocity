...
from abvelocity.ts.common.viz.timeseries_annotate import plot_anomalies_over_forecast_vs_actual, plot_lines_markers, plt_compare_series_annotations
...
from abvelocity.ts.gk.detection.detector.config import F1, ADConfig
from abvelocity.ts.gk.detection.detector.data import DetectorData
...
def hourly_data():
    ...
    df = generate_df_for_tests(freq="H", train_start_date=datetime.datetime(2020, 1, 1), intercept=50, train_frac=0.99, periods=24 * 28)["df"]
    ...
    fig = plot_lines_markers(df=df, x_col=TIME_COL, line_cols=["y", "contaminated_y"])
...
def forecast_config_info_hourly():
    ...
    model_components = ModelComponentsParam(
        autoregression={"autoreg_dict": {"lag_dict": {"orders": [24]}, "agg_lag_dict": None}},
        ...
        custom={"extra_pred_cols": ["dow_hr"], "min_admissible_value": 0, "normalize_method": "zero_to_one"},
    )
...
def test_greykite_detector_hourly_f1(hourly_data, forecast_config_info_hourly):
    ...
    def reward_func(data):
        return calc_f1(y_true=data.y_true, y_pred=data.y_pred)
    ...
    assert detector.fit_info["param"] == {"coverage": 0.99, "volatility_features": ["dow"]}
    ...
    fig = px.line(
        param_eval_df, x="coverage", y="obj_value", color="volatility_features", title="'GreykiteDetector' result of parameter search: f1, hourly data"
    )
    assert fig is not None
    if FIG_SHOW:
        fig.show()

    test_data = DetectorData(df=df_test, y_true=df_test[ANOMALY_COL])

    test_data = detector.predict(test_data)
    test_obj_value = detector.reward.apply(test_data)
    assert test_obj_value == pytest.approx(0.667, rel=0.05)
    ...
    assert test_precision == pytest.approx(0.5, rel=0.05)
    ...
    fig.add_vline(x=fit_df[TIME_COL].max(), line_width=1, line_dash="dash", line_color="green")

    fig.add_annotation(x=fit_df[TIME_COL].max(), y=fit_pred_df[ACTUAL_COL].max(), text="end of training")
    assert fig is not None
    if FIG_SHOW:
        fig.show()


def test_greykite_detector_hourly_anomaly_percent(hourly_data, forecast_config_info_hourly):
    ...
    detector = GreykiteDetector(forecast_config=forecast_config, ad_config=ad_config, reward=None)

    detector.fit(data=train_data)

    # Checks optimal parameter.
    assert detector.fit_info["param"] == {"coverage": 0.99, "volatility_features": ["dow"]}
    assert {TIME_COL, ACTUAL_COL, PREDICTED_COL, PREDICTED_LOWER_COL, PREDICTED_UPPER_COL, PREDICTED_ANOMALY_COL}.issubset(set(detector.fitted_df.columns))
    # Checks parameter grid.
    param_obj_list = detector.fit_info["param_obj_list"]
    param_eval_df = pd.DataFrame.from_records(param_obj_list)
    assert list(param_eval_df.columns) == ["coverage", "volatility_features", "obj_value"]
    ...
    test_data = detector.predict(test_data)
    test_obj_value = detector.reward.apply(test_data)
    assert test_obj_value == pytest.approx(-1.15, rel=0.05)
...
def test_greykite_detector_daily_f1(daily_data, forecast_config_info_daily):
    ...
    ad_config = ADConfig(
        volatility_features_list=[["dow"], ["is_weekend"]], coverage_grid=[0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.9, 0.95, 0.99, 0.999], variance_scaling=True
    )
    ...
    detector.fit(data=train_data)
    # Checks optimal parameter.
    assert detector.fit_info["param"] == {"coverage": 0.99, "volatility_features": ["is_weekend"]}
    # Checks parameter grid.
    param_obj_list = detector.fit_info["param_obj_list"]
    param_eval_df = pd.DataFrame.from_records(param_obj_list)
    ...
    param_eval_df["volatility_features"] = param_eval_df["volatility_features"].map(str)
    fig = px.line(
        param_eval_df, x="coverage", y="obj_value", color="volatility_features", title="'GreykiteDetector' result of parameter search: reward=f1, daily data"
    )
    assert fig is not None
    if FIG_SHOW:
        fig.show()

    test_data = DetectorData(df=df_test.copy(), y_true=df_test[ANOMALY_COL])
    ...
    test_recall_new = calc_recall(y_true=test_data.y_true, y_pred=test_data.y_pred)

    test_precision_new = calc_precision(y_true=test_data.y_true, y_pred=test_data.y_pred)
    ...
    fit_recall_new = calc_recall(y_true=fit_data.y_true, y_pred=fit_data.y_pred)

    fit_precision_new = calc_precision(y_true=fit_data.y_true, y_pred=fit_data.y_pred)

    assert fit_recall_new == pytest.approx(fit_recall, 0.001)
    assert fit_precision_new == pytest.approx(fit_precision, 0.001)


def test_greykite_detector_daily_outlier(daily_data, forecast_config_info_daily):
    ...
    # Tests plot method.
    fig = detector.plot(phase="train", title="test_greykite_detector_detector_daily_f1_outlier - fit phase")
    assert fig is not None
    if FIG_SHOW:
        fig.show()
...
def test_greykite_detector_daily_f1_with_ape_filter(daily_data, forecast_config_info_daily):
    ...
    forecast_config = forecast_config_info_daily
    ad_config = ADConfig(
        coverage_grid=[0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.9, 0.95, 0.99, 0.999], target_anomaly_percent=None, ape_grid=[0, 20, 50], variance_scaling=True
    )
    ...
    detector.fit(data=train_data)
    # Checks optimal parameter.
    assert detector.fit_info["param"] == {"coverage": 0.5, "volatility_features": [], "absolute_percent_error": 20}
    # Checks parameter grid.
    param_obj_list = detector.fit_info["param_obj_list"]
    param_eval_df = pd.DataFrame.from_records(param_obj_list)

    assert list(param_eval_df.columns) == ["coverage", "volatility_features", "absolute_percent_error", "obj_value"]
...
def test_merge_anomaly_info():
    ...
    anomaly_df = pd.DataFrame({START_TIME_COL: ["2020-01-04"], END_TIME_COL: ["2020-01-06"]})
    data = DetectorData(df=df, y_true=y_true, anomaly_df=anomaly_df)
    ...
    expected_anomaly_df = pd.DataFrame(
        {START_TIME_COL: pd.to_datetime(["2020-01-03", "2020-01-10"]), END_TIME_COL: pd.to_datetime(["2020-01-06", "2020-01-10"])}
    )
...
def test_greykite_detector_hourly_anomaly_pickup():
    ...
    model_components = ModelComponentsParam(
        ...
        hyperparameter_override={
            ...
            "input__response__null__impute_params": {"orders": [168, 336, 504]},
        },
        seasonality={"monthly_seasonality": 2, "yearly_seasonality": 7},
        events={"holiday_lookup_countries": ["US"], "holiday_pre_num_days": 2, "holiday_post_num_days": 2},
        autoregression={"autoreg_dict": {"agg_lag_dict": {"orders_list": [[168, 336, 504]], "agg_func": "median"}}},
        custom={
    ...
    input_data = generate_df_for_tests(freq="H", train_start_date=datetime.datetime(2020, 1, 1), intercept=50, train_frac=0.99, periods=24 * 60)
...
def test_summary(daily_data, forecast_config_info_daily):
    ...
    forecast_config = forecast_config_info_daily
    ad_config = ADConfig(
        coverage_grid=[0.2, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.9, 0.95, 0.99, 0.999], target_anomaly_percent=None, ape_grid=[0, 20, 50], variance_scaling=True
    )
