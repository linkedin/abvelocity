...
from abvelocity.ts.common.testing_utils_anomalies import contaminate_df_with_anomalies
from abvelocity.ts.common.viz.timeseries_annotate import plot_anomalies_over_forecast_vs_actual
from abvelocity.ts.gk.detection.common.ad_evaluation import f1_score, precision_score, recall_score
from abvelocity.ts.gk.detection.common.pickler import GreykitePickler
from abvelocity.ts.gk.detection.detector.ad_utils import partial_return
from abvelocity.ts.gk.detection.detector.config import ADConfig
from abvelocity.ts.gk.detection.detector.constants import FIG_SHOW
from abvelocity.ts.gk.detection.detector.data import DetectorData as Data
from abvelocity.ts.gk.detection.detector.greykite import DETECTOR_PREDICT_COLS, GreykiteDetector
from abvelocity.ts.gk.detection.detector.reward import Reward
...
def daily_data():
    ...
    df = generate_df_for_tests(freq="D", train_start_date=datetime.datetime(2020, 1, 1), intercept=50, train_frac=0.99, periods=200)["df"]
    ...
    df = df.drop(columns=[VALUE_COL]).rename(columns={"contaminated_y": VALUE_COL})
    ...
    return {"df_train": df_train, "df_test": df_test, "df": df}


@pytest.fixture(scope="module")
def forecast_config_info_daily():
    """Generates ``forecast_config`` for testing."""
    metadata = MetadataParam(time_col=TIME_COL, value_col=VALUE_COL, train_end_date=None, anomaly_info=None)

    evaluation_period = EvaluationPeriodParam(test_horizon=0, cv_max_splits=0)

    model_components = ModelComponentsParam(
        autoregression={"autoreg_dict": {"lag_dict": {"orders": [7]}, "agg_lag_dict": None}},
        events={"auto_holiday": False, "holiday_lookup_countries": ["US"], "holiday_pre_num_days": 2, "holiday_post_num_days": 2, "daily_event_df_dict": None},
        custom={"extra_pred_cols": ["dow"], "min_admissible_value": 0, "normalize_method": "zero_to_one"},
    )
...
class TestClass:
    def __init__(self, a, b):
        self.a = a
        self.b = X(b)
        self.c = {Term([]): None, Term(["a", "b"]): [3, 4, 5]}
        self.d = {Term([]): {"d_1_1": 1, Term([]): Term(["a", "b"])}, "d_2": [1, 2]}
...
def test_silverkite_forecast_result():
    ...
    df = generate_df_for_tests(freq="D", periods=100)["df"]
    ...
    result = forecaster.run_forecast_config(
        ...
        config=ForecastConfig(
            ...
            evaluation_period_param=EvaluationPeriodParam(cv_max_splits=1, cv_horizon=7, test_horizon=7, cv_min_train_periods=50),
            model_components_param=ModelComponentsParam(custom={"fit_algorithm_dict": {"fit_algorithm": "linear"}}, autoregression={"autoreg_dict": "auto"}),
    ...
    assert_equal(summarize_grid_search_results(result.grid_search), summarize_grid_search_results(deserialized.grid_search))
    ...
    for key in result.grid_search.__dict__.keys():
        if key not in ["scoring", "estimator", "refit", "cv", "error_score", "cv_results_", "scorer_", "best_estimator_"]:
            assert_equal(result.grid_search.__dict__[key], deserialized.grid_search.__dict__[key])

    # Model.
    assert_equal(result.model[-1].predict(df), deserialized.model[-1].predict(df))
    ...
    for key in result.model[-1].__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.model[-1].__dict__[key], deserialized.model[-1].__dict__[key])
    assert_equal(inspect.getsource(result.model[-1].__dict__["score_func"]), inspect.getsource(deserialized.model[-1].__dict__["score_func"]))
    ...
    for key in result.model[-1].model_dict.keys():
        ...
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.model[-1].model_dict[key], deserialized.model[-1].model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(inspect.getsource(result.model[-1].model_dict[key]), inspect.getsource(deserialized.model[-1].model_dict[key]))
    ...
    for key in result.model[-1].model_dict["autoreg_dict"].keys():
        if key not in ["series_na_fill_func"]:
            assert_equal(result.model[-1].model_dict["autoreg_dict"][key], deserialized.model[-1].model_dict["autoreg_dict"][key])
    assert_equal(
    ...
    # Forecast.
    assert_equal(result.forecast.estimator.predict(df), deserialized.forecast.estimator.predict(df))
    ...
    for key in result.forecast.__dict__.keys():
        if key not in ["r2_loss_function", "estimator"]:
            assert_equal(result.forecast.__dict__[key], deserialized.forecast.__dict__[key])
    assert_equal(inspect.getsource(result.forecast.__dict__["r2_loss_function"]), inspect.getsource(deserialized.forecast.__dict__["r2_loss_function"]))
    # Forecast: estimator.
    for key in result.forecast.estimator.__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.forecast.estimator.__dict__[key], deserialized.forecast.estimator.__dict__[key])
    assert_equal(inspect.getsource(result.forecast.estimator.__dict__["score_func"]), inspect.getsource(deserialized.forecast.estimator.__dict__["score_func"]))
    # Model: estimator/model_dict
    for key in result.forecast.estimator.model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.forecast.estimator.model_dict[key], deserialized.forecast.estimator.model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(inspect.getsource(result.forecast.estimator.model_dict[key]), inspect.getsource(deserialized.forecast.estimator.model_dict[key]))
    # Model: estimator/model_dict/autoreg_dict.
    for key in result.forecast.estimator.model_dict["autoreg_dict"].keys():
        if key not in ["series_na_fill_func"]:
            assert_equal(result.forecast.estimator.model_dict["autoreg_dict"][key], deserialized.forecast.estimator.model_dict["autoreg_dict"][key])
    assert_equal(
    ...
    # Backtest.
    assert_equal(result.backtest.estimator.predict(df), deserialized.backtest.estimator.predict(df))
    assert result.backtest.estimator.model_dict["x_design_info"] is not None
    # Backtest: attributes.
    for key in result.backtest.__dict__.keys():
        if key not in ["r2_loss_function", "estimator"]:
            assert_equal(result.backtest.__dict__[key], deserialized.backtest.__dict__[key])
    assert_equal(inspect.getsource(result.backtest.__dict__["r2_loss_function"]), inspect.getsource(deserialized.backtest.__dict__["r2_loss_function"]))
    # Backtest: estimator.
    for key in result.backtest.estimator.__dict__.keys():
        if key not in ["score_func", "silverkite", "silverkite_diagnostics", "model_dict"]:
            assert_equal(result.backtest.estimator.__dict__[key], deserialized.backtest.estimator.__dict__[key])
    assert_equal(inspect.getsource(result.backtest.estimator.__dict__["score_func"]), inspect.getsource(deserialized.backtest.estimator.__dict__["score_func"]))
    # Model: estimator/model_dict.
    for key in result.backtest.estimator.model_dict.keys():
        # Functions and classes are not testable.
        if key not in ["x_design_info", "fs_func", "ml_model", "plt_pred", "autoreg_dict", "changepoint_detector", "autoreg_func", "normalize_df_func"]:
            assert_equal(result.backtest.estimator.model_dict[key], deserialized.backtest.estimator.model_dict[key])
        # Tests function source code.
        elif key in ["fs_func", "plt_pred", "autoreg_func", "normalize_df_func"]:
            assert_equal(inspect.getsource(result.backtest.estimator.model_dict[key]), inspect.getsource(deserialized.backtest.estimator.model_dict[key]))
    # Model: estimator/model_dict/autoreg_dict.
    for key in result.backtest.estimator.model_dict["autoreg_dict"].keys():
        if key not in ["series_na_fill_func"]:
            assert_equal(result.backtest.estimator.model_dict["autoreg_dict"][key], deserialized.backtest.estimator.model_dict["autoreg_dict"][key])
    assert_equal(
    ...
    for key in result.timeseries.__dict__.keys():
        assert_equal(result.timeseries.__dict__[key], deserialized.timeseries.__dict__[key])
...
def test_silverkite_ad_result(daily_data, forecast_config_info_daily):
    ...
    def reward_func(data):
        return f1_calc(y_true=data.y_true, y_pred=data.y_pred)

    reward = Reward(reward_func)

    # Trains the anomaly detector.
    detector = GreykiteDetector(forecast_config=forecast_config, ad_config=ad_config, reward=reward)
    ...
    fit_recall = calc_recall(y_true=fit_data.y_true, y_pred=fit_data.y_pred)
    deserialized_f1_recall = calc_recall(y_true=deserialized_fit_data.y_true, y_pred=deserialized_fit_data.y_pred)
    assert fit_recall == deserialized_f1_recall

    fit_precision = calc_precision(y_true=fit_data.y_true, y_pred=fit_data.y_pred)
    deserialized_f1_precision = calc_precision(y_true=deserialized_fit_data.y_true, y_pred=deserialized_fit_data.y_pred)
    ...
    test_data = Data(df=df_test, y_true=df_test[ANOMALY_COL])
    ...
    test_recall = calc_recall(y_true=test_data.y_true, y_pred=test_data.y_pred)
    deserialized_test_recall = calc_recall(y_true=deserialized_test_data.y_true, y_pred=deserialized_test_data.y_pred)
    assert test_recall == deserialized_test_recall

    test_precision = calc_precision(y_true=test_data.y_true, y_pred=test_data.y_pred)
    deserialized_test_precision = calc_precision(y_true=deserialized_test_data.y_true, y_pred=deserialized_test_data.y_pred)
