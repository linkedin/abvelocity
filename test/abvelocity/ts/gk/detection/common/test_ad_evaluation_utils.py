import numpy as np
from abvelocity.ts.gk.detection.common.ad_evaluation_utils import (
    compute_range_based_score,
    get_cardinality_factor,
    get_overlap_size_and_position_reward,
    get_positional_reward,
    prepare_anomaly_ranges,
)
...
def test_prepare_anomaly_ranges():
    ...
    expected_real_anomaly_ranges = [[3, 3], [4, 4], [5, 5], [6, 6], [7, 7], [11, 11], [12, 12], [13, 13], [14, 14], [15, 15], [16, 16], [17, 17], [18, 18]]
    predicted_anomaly_ranges = prepare_anomaly_ranges(np.array(y_pred), range_based=False)
    expected_predicted_anomaly_ranges = [[2, 2], [3, 3], [4, 4], [5, 5], [11, 11], [12, 12], [15, 15], [16, 16], [17, 17]]
...
def test_get_overlap_size_and_position_reward():
    ...
    overlap_size_and_position_reward = get_overlap_size_and_position_reward(
        anomaly_range_1=anomaly_range_1, anomaly_range_2=anomaly_range_2, overlap_count=[0], positional_bias="flat"
    )
    ...
    overlap_size_and_position_reward = get_overlap_size_and_position_reward(
        anomaly_range_1=anomaly_range_1, anomaly_range_2=anomaly_range_2, overlap_count=[0], positional_bias="front"
    )
    assert round(overlap_size_and_position_reward, 3) == 0.714

    overlap_size_and_position_reward = get_overlap_size_and_position_reward(
        anomaly_range_1=anomaly_range_1, anomaly_range_2=anomaly_range_2, overlap_count=[0], positional_bias="middle"
    )
    assert round(overlap_size_and_position_reward, 3) == 0.50

    overlap_size_and_position_reward = get_overlap_size_and_position_reward(
        anomaly_range_1=anomaly_range_1, anomaly_range_2=anomaly_range_2, overlap_count=[0], positional_bias="back"
    )
...
def test_compute_range_based_score():
    ...
    recall = compute_range_based_score(real_anomaly_ranges, predicted_anomaly_ranges, alpha=0.5, positional_bias="flat")
    precision = compute_range_based_score(predicted_anomaly_ranges, real_anomaly_ranges, alpha=0.5, positional_bias="flat")
    assert round(precision, 3) == 0.958
    assert round(recall, 3) == 0.806

    recall = compute_range_based_score(real_anomaly_ranges, predicted_anomaly_ranges, alpha=0.5, positional_bias="front")
    precision = compute_range_based_score(predicted_anomaly_ranges, real_anomaly_ranges, alpha=0.5, positional_bias="front")
    assert round(precision, 3) == 0.933
    assert round(recall, 3) == 0.867

    recall = compute_range_based_score(real_anomaly_ranges, predicted_anomaly_ranges, alpha=0.5, positional_bias="middle")
    precision = compute_range_based_score(predicted_anomaly_ranges, real_anomaly_ranges, alpha=0.5, positional_bias="middle")
    assert round(precision, 3) == 0.972
    assert round(recall, 3) == 0.817

    recall = compute_range_based_score(real_anomaly_ranges, predicted_anomaly_ranges, alpha=0.5, positional_bias="back")
    precision = compute_range_based_score(predicted_anomaly_ranges, real_anomaly_ranges, alpha=0.5, positional_bias="back")
    assert round(precision, 3) == 0.983
    assert round(recall, 3) == 0.746

    recall = compute_range_based_score(real_anomaly_ranges, predicted_anomaly_ranges, alpha=0.5, positional_bias="front", cardinality_bias="reciprocal")
    precision = compute_range_based_score(predicted_anomaly_ranges, real_anomaly_ranges, alpha=0.5, positional_bias="front", cardinality_bias="reciprocal")
    assert round(precision, 3) == 0.933
