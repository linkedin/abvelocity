import numpy as np
import pandas as pd
from abvelocity.ts.common.features.timeseries_impute import impute_with_lags, impute_with_lags_multi
...
def test_impute_with_lags():
    ...
    impute_info = impute_with_lags(df=df, value_col="y", orders=[7], agg_func="mean", iter_num=1)
    ...
    impute_info = impute_with_lags(df=df, value_col="y", orders=[7, 14, 21], agg_func="mean", iter_num=1)

    assert impute_info["initial_missing_num"] == 3
    assert impute_info["final_missing_num"] == 0
    imputed_df = impute_info["df"]

    assert list(imputed_df["y"].values) == (list(range(70)) + [56, 57, 58])

    # same test with larger ``iter_num``
    df = pd.DataFrame({"y": list(range(70)) + [np.nan, np.nan, np.nan]})
    impute_info = impute_with_lags(df=df, value_col="y", orders=[7, 14, 21], agg_func="mean", iter_num=10)  # large ``iter_num``
    ...
    impute_info = impute_with_lags(df=df, value_col="y", orders=[2, 3, 4], agg_func="mean", iter_num=1)
    ...
    assert list(imputed_df["y"].values) == ([1, 2, 3, 1.5, 5, 6, 4])

    # NAs are ignored when a function `agg_func` is passed
    df = pd.DataFrame({"y": [1, 2, 3, np.nan, 5, 6, np.nan]})
    impute_info = impute_with_lags(df=df, value_col="y", orders=[2, 3, 4], agg_func=np.mean, iter_num=1)
    assert impute_info["initial_missing_num"] == 2
    assert impute_info["final_missing_num"] == 0
    imputed_df = impute_info["df"]
    assert list(imputed_df["y"].values) == ([1, 2, 3, 1.5, 5, 6, 4])
    ...
    # One iteration
    impute_info = impute_with_lags(df=df, value_col="y", orders=[7], agg_func=np.mean, iter_num=1)
    ...
    expected_array = np.array(list(range(63)) + list(range(56, 63)) + [np.nan] * 3)

    np.testing.assert_array_equal(obtained_array, expected_array)

    # Two iterations
    impute_info = impute_with_lags(df=df, value_col="y", orders=[7], agg_func=np.mean, iter_num=2)
    ...
    expected_array = np.array(list(range(63)) + list(range(56, 63)) + [56, 57, 58])
...
def test_impute_with_lags_multi():
    ...
    df = pd.DataFrame({"a": (0.0, np.nan, -1.0, 1.0), "b": (np.nan, 2.0, np.nan, np.nan), "c": (2.0, 3.0, np.nan, 9.0), "d": (np.nan, 4.0, -4.0, 16.0)})

    # tests `cols = None`
    impute_info = impute_with_lags_multi(df=df, orders=[1], agg_func=np.mean, iter_num=1, cols=None)
    expected = pd.DataFrame({"a": (0.0, 0.0, -1.0, 1.0), "b": (np.nan, 2.0, 2.0, np.nan), "c": (2.0, 3.0, 3.0, 9.0), "d": (np.nan, 4.0, -4.0, 16.0)})
    ...
    # tests `cols = list`
    impute_info = impute_with_lags_multi(df=df, orders=[1], agg_func=np.mean, iter_num=2, cols=["b", "c", "d"])
