...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import ADJUSTMENT_DELTA_COL, END_TIME_COL, METRIC_ID_COL, START_TIME_COL, TIME_COL
from abvelocity.ts.common.features.adjust_anomalous_data import adjust_anomalous_data, label_anomalies_multi_metric
from abvelocity.ts.common.testing_utils import generate_anomalous_data, generic_test_adjust_anomalous_data
...
def test_adjust_anomalous_data(data):
    ...
    adj_df_info = adjust_anomalous_data(
        ...
        filter_by_dict={METRIC_ID_COL: value_col, "dimension1": "level_1"},
    ...
    adj_df_info = adjust_anomalous_data(
        ...
        end_time_col=END_TIME_COL,
        adjustment_delta_col=ADJUSTMENT_DELTA_COL,
        filter_by_dict={"dimension1": "level_1", "dimension2": ["level_2", "level_1"]},
        filter_by_value_col=METRIC_ID_COL,
        adjustment_method="subtract",
    )

    adj_values = pd.Series([np.nan, np.nan, 2.0, 0.0, 1.0, 2.0, 6.0, 7.0, 8.0, 9.0])
    generic_test_adjust_anomalous_data(value_col=value_col, adj_df_info=adj_df_info, adj_values=adj_values)
    ...
    adj_df_info = adjust_anomalous_data(
        ...
        end_time_col=END_TIME_COL,
        adjustment_delta_col=ADJUSTMENT_DELTA_COL,
        filter_by_dict={METRIC_ID_COL: value_col, "dimension1": "level_1", "dimension2": "level_2"},
        filter_by_value_col=None,
    )

    adj_values = pd.Series([0.0, 1.0, 2.0, 6.0, 7.0, 8.0, 6.0, 7.0, 8.0, 9.0])
    generic_test_adjust_anomalous_data(value_col=value_col, adj_df_info=adj_df_info, adj_values=adj_values)
    ...
    adj_values = pd.Series([20.0, 21.0, 22.0, 23.0, 24.0, 25.0, 26.0, 27.0, 28.0, 29.0])
    generic_test_adjust_anomalous_data(value_col=value_col, adj_df_info=adj_df_info, adj_values=adj_values)
    ...
    adj_df_info = adjust_anomalous_data(
        ...
        end_time_col=END_TIME_COL,
        adjustment_delta_col=ADJUSTMENT_DELTA_COL,
        filter_by_dict={METRIC_ID_COL: value_col, "dimension1": "level_2"},
        filter_by_value_col=None,
    )
    ...
    adj_values = pd.Series([np.nan, np.nan, 2.0, np.nan, np.nan, np.nan, 6.0, np.nan, np.nan, np.nan])
    generic_test_adjust_anomalous_data(value_col=value_col, adj_df_info=adj_df_info, adj_values=adj_values)
...
def test_label_anomalies_multi_metric():
    ...
    assert is_anomaly_cols == ["impressions_is_anomaly", "clicks_is_anomaly", "bookings_is_anomaly"]

    assert anomaly_value_cols == ["impressions_anomaly_value", "clicks_anomaly_value", "bookings_anomaly_value"]

    assert normal_value_cols == ["impressions_normal_value", "clicks_normal_value", "bookings_normal_value"]

    assert set(augmented_df.columns) == set(["ts"] + value_cols + is_anomaly_cols + anomaly_value_cols + normal_value_cols)
