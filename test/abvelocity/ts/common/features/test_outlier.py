...
import pandas as pd
import pytest
from abvelocity.ts.common.features.outlier import (
...
def helper_plot_outliers(y, detection_result, title):
    ...
    y: `pandas.Series`
        Input data.
    detection_result: `~abvelocity.ts.common.features.outlier.DetectionResult`
    ...
    """
    df = pd.DataFrame({"ind": range(len(y)), "y": y, "scores": detection_result.scores, "is_outlier": detection_result.is_outlier})
    ...
    fig = plot_lines_markers(df=df, x_col="ind", line_cols=["y", "scores"], marker_cols=["y_outlier"], title=title)
...
def test_base_outlier_detector_init():
    ...
    detect_outlier = BaseOutlierDetector(trim_percent=1, diff_method=DiffMethod(name="es"))
...
def test_base_outlier_detector_trim():
    ...
    with pytest.raises(ValueError, match="Trim percent:"):
...
def test_base_detect_diff_from_baseline_es():
    ...
    baseline_result = detect_outlier.diff_from_baseline(y=y, diff_method=EXPONENTIAL_SMOOTHING)
    ...
    baseline_result = detect_outlier.diff_from_baseline(y=y, diff_method=diff_method)
    residuals = baseline_result["residuals"]
    baseline_y = baseline_result["baseline_y"]
...
def test_base_detect_diff_from_baseline_moving_med():
    ...
    baseline_result = detect_outlier.diff_from_baseline(y=y, diff_method=MOVING_MEDIAN)
    ...
    # This will imply that `y` is unchanged and residuals are all zero.
    y = pd.Series([0, 0, 0, 10, 10, 10, 10, 0, 0, 0])
    diff_method = DiffMethod(name="moving_med", param={"window": 2, "min_periods": 1, "center": True})
    baseline_result = detect_outlier.diff_from_baseline(y=y, diff_method=diff_method)
    residuals = baseline_result["residuals"]
    baseline_y = baseline_result["baseline_y"]
...
def test_tukey_outlier_detector_init():
    ...
    # Tests `__init__` with parameters.
    detect_outlier = TukeyOutlierDetector(iqr_lower=0.05, iqr_upper=0.95, tukey_cutoff=0.2)
...
def test_z_score_outlier_detector(data):
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Easy detection Z-score")
    if FIG_SHOW:
        fig.show()
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Hard detection with Z-score and no baseline diffing.")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Hard detection with Z-score with diffing.")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
...
def test_tukey_outlier_detector(data):
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Easy detection Tukey")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Hard detection with Tukey and no baseline diffing.")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Hard detection with Tukey with diffing.")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
...
def test_tukey_outlier_detector_corner_cases():
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Tukey: perfectly linear data")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="Tukey: constant")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
...
def test_z_score_outlier_detector_corner_cases():
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="ZScore: perfectly linear data")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
    ...
    fig = helper_plot_outliers(y=y, detection_result=fitted, title="ZScore: constant")
    if FIG_SHOW:
        fig.show()
    assert fig is not None
