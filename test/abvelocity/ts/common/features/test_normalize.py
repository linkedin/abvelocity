...
def test_normalize_df():
    ...
    with pytest.warns(Warning) as record:
        normalization_info = normalize_df(df=df, method="statistical", drop_degenerate_cols=True)
    ...
    with pytest.warns(Warning) as record:
        normalization_info = normalize_df(df=df, method="zero_at_origin", drop_degenerate_cols=True)
        assert expected_warning in record[0].message.args[0]
    ...
    normalization_info = normalize_df(df=df, method="statistical", drop_degenerate_cols=True, replace_zero_denom=True)
    ...
    # `"zero_to_one"` method
    with pytest.warns(Warning) as record:
        normalization_info = normalize_df(df=df, method="zero_to_one", drop_degenerate_cols=True)
        assert expected_warning in record[0].message.args[0]
    ...
    with pytest.warns(Warning) as record:
        normalization_info = normalize_df(df=df, method="minus_half_to_half", drop_degenerate_cols=True)
        assert expected_warning in record[0].message.args[0]
    ...
    normalization_info = normalize_df(df=df, method="statistical", drop_degenerate_cols=False)
    normalize_df_func = normalization_info["normalize_df_func"]
    normalized_df = normalize_df_func(df)
    assert list(normalized_df.columns) == ["x", "y"]
    assert list(normalized_df["x"].values) == [0, -1, 1]
    ...
    with pytest.raises(NotImplementedError, match=expected_match):
        normalize_df(df=df, method="quantile_based", drop_degenerate_cols=True)
    ...
    with pytest.raises(ValueError, match=expected_match):
        normalize_df(df=df, method="statistical", drop_degenerate_cols=True)
