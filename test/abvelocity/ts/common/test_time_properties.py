...
from abvelocity.ts.common.time_properties import (
    describe_timeseries,
    fill_missing_dates,
    find_missing_dates,
    get_canonical_data,
...
def test_describe_timeseries():
    ...
    x = [datetime.datetime(2018, 1, 1, 0, 0, 1), datetime.datetime(2018, 1, 1, 0, 0, 2), datetime.datetime(2018, 1, 1, 0, 0, 3)]
    ...
    x = [datetime.datetime(2018, 1, 1, 0, 0, 1), datetime.datetime(2018, 1, 1, 0, 0, 2), datetime.datetime(2018, 1, 1, 0, 0, 4)]
    ...
    x = [datetime.datetime(2018, 1, 1, 0, 0, 3), datetime.datetime(2018, 1, 1, 0, 0, 2), datetime.datetime(2018, 1, 1, 0, 0, 1)]
...
def test_min_gap_in_seconds():
    ...
    date_list = pd.date_range(start=datetime.datetime(2019, 1, 1), periods=20, freq="H").tolist()
    df = pd.DataFrame({"ts": date_list})
    assert min_gap_in_seconds(df=df, time_col="ts") == TimeEnum.ONE_HOUR_IN_SECONDS.value

    date_list = pd.date_range(start=datetime.datetime(2019, 1, 1), periods=20, freq="6H").tolist()
    df = pd.DataFrame({"ts": date_list})
    assert min_gap_in_seconds(df=df, time_col="ts") == 6 * TimeEnum.ONE_HOUR_IN_SECONDS.value

    date_list = pd.date_range(start=datetime.datetime(2019, 1, 1), periods=20, freq="MIN").tolist()
    df = pd.DataFrame({"ts": date_list})
    assert min_gap_in_seconds(df=df, time_col="ts") == TimeEnum.ONE_MINUTE_IN_SECONDS.value

    date_list = pd.date_range(start=datetime.datetime(2019, 1, 1), periods=20, freq="5S").tolist()
    df = pd.DataFrame({"ts": date_list})
...
def test_gcd_err():
    df = pd.DataFrame(
        {
            TIME_COL: [datetime.datetime(2018, 1, 1, 0, 0, 3), datetime.datetime(2018, 1, 1, 0, 0, 2), datetime.datetime(2018, 1, 1, 0, 0, 1)],
            VALUE_COL: [1, 2, 3],
        }
    )
...
def test_gcd_dates():
    ...
    df = pd.DataFrame(
        {"time": [datetime.datetime(2018, 1, 1, 0, 0, 1), datetime.datetime(2018, 1, 1, 0, 0, 2), datetime.datetime(2018, 1, 1, 0, 0, 3)], "val": [1, 2, 3]}
    )
    canonical_data_dict = get_canonical_data(df=df, time_col="time", value_col="val")
    ...
    canonical_data_dict = get_canonical_data(df=df, time_col=TIME_COL, value_col=VALUE_COL, date_format=date_format, freq="4D")
    expected = pd.DataFrame(
        {
            TIME_COL: pd.Series(
                [datetime.datetime(2018, 1, 1, 0, 0, 0), datetime.datetime(2018, 1, 5, 0, 0, 0), datetime.datetime(2018, 1, 9, 0, 0, 0)], name=TIME_COL
            ),
    ...
    canonical_data_dict = get_canonical_data(df=df, time_col=TIME_COL, value_col=VALUE_COL, freq="4D")
    assert canonical_data_dict["time_stats"]["added_timepoints"] == 0
    assert canonical_data_dict["time_stats"]["dropped_timepoints"] == 0
    assert_equal(canonical_data_dict["df"], expected)

    # time zone
    canonical_data_dict = get_canonical_data(df=df, time_col=TIME_COL, value_col=VALUE_COL, freq="4D", tz="US/Pacific")
...
def test_gcd_freq():
    ...
    df = pd.DataFrame(
        {
            TIME_COL: [datetime.datetime(2018, 1, 1, 0, 0, 0), datetime.datetime(2018, 1, 2, 0, 0, 0), datetime.datetime(2018, 1, 3, 0, 0, 0)],
            VALUE_COL: [1, 2, 3],
        }
    )
    with pytest.warns(UserWarning) as record:
        canonical_data_dict = get_canonical_data(df=df, time_col=TIME_COL, value_col=VALUE_COL, freq="H")
...
def test_gcd_irregular():
    ...
    expected = pd.DataFrame(
        {
            # in sorted order
            TIME_COL: pd.date_range(start=datetime.datetime(2018, 1, 1, 0, 0, 1), end=datetime.datetime(2018, 1, 1, 0, 0, 10), freq="S"),
            VALUE_COL: [1, 2, np.nan, 4, np.nan, np.nan, np.nan, np.nan, np.nan, 3],
        }
    )
    expected.index = expected[TIME_COL]
    ...
    canonical_data_dict = get_canonical_data(df=df, time_col=TIME_COL, value_col=VALUE_COL, freq="S")  # the frequency should be provided when there are gaps
...
def test_gcd_load_data_anomaly():
    ...
    canonical_data_dict = get_canonical_data(df=df, time_col=TIME_COL, value_col=value_col)
    ...
    anomaly_df = pd.DataFrame(
        {
            ...
            METRIC_ID_COL: [dim_one, dim_one, dim_two],  # used to filter rows in this df
        }
    )
    ...
    canonical_data_dict2 = get_canonical_data(df=df, time_col=TIME_COL, value_col=value_col, anomaly_info=anomaly_info)
    assert_equal(canonical_data_dict2["df_before_adjustment"], canonical_data_dict["df"])
    expected_df = canonical_data_dict["df"].copy()
    # first anomaly
    idx = (expected_df[TIME_COL] >= anomaly_df[START_TIME_COL][0]) & (expected_df[TIME_COL] <= anomaly_df[END_TIME_COL][0])
    expected_df.loc[idx, VALUE_COL] = np.nan
    # second anomaly
    idx = (expected_df[TIME_COL] >= anomaly_df[START_TIME_COL][1]) & (expected_df[TIME_COL] <= anomaly_df[END_TIME_COL][1])
    ...
    canonical_data_dict3 = get_canonical_data(df=df, time_col=TIME_COL, value_col=value_col, anomaly_info=anomaly_info)
    # third anomaly. The value is subtracted, according to `adjustment_method`.
    idx = (expected_df[TIME_COL] >= anomaly_df[START_TIME_COL][2]) & (expected_df[TIME_COL] <= anomaly_df[END_TIME_COL][2])
    ...
    value_col = "pm"
    anomaly_df = pd.DataFrame({START_TIME_COL: ["2014-12-31-22"], END_TIME_COL: ["2015-01-01-22"], ADJUSTMENT_DELTA_COL: [np.nan]})
    # Adjusts one column (value_col)
    anomaly_info = {
        "value_col": value_col,
        "anomaly_df": anomaly_df,
    ...
    canonical_data_dict4 = get_canonical_data(df=df, time_col=TIME_COL, value_col=value_col, anomaly_info=anomaly_info)
...
def test_gcd_train_end_date():
    ...
    with pytest.warns(UserWarning) as record:
        ...
        assert (
            ...
            f"({train_end_date})." in all_warning_msg
        )
    ...
    with pytest.warns(UserWarning) as record:
        ...
        all_warning_msg = " ".join([record[i].message.args[0] for i in range(len(record))])
        assert (
            f"{VALUE_COL} column of the provided time series contains null values at the end, "
            f"or the input `train_end_date` is beyond the last timestamp available." in all_warning_msg
        )
    ...
    with pytest.warns(UserWarning) as record:
        ...
        get_canonical_data(df, train_end_date=train_end_date)
        all_warning_msg = " ".join([record[i].message.args[0] for i in range(len(record))])
        assert f"{VALUE_COL} column of the provided time series contains trailing NAs" in all_warning_msg
...
def test_gcd_train_end_date_regressor():
    ...
    data = generate_df_with_reg_for_tests(freq="D", periods=30, train_start_date=datetime.datetime(2018, 1, 1), remove_extra_cols=True, mask_test_actuals=True)
    ...
    with pytest.warns(UserWarning) as record:
        canonical_data_dict = get_canonical_data(df=df, train_end_date=None, regressor_cols=None)
        all_warning_msg = " ".join([record[i].message.args[0] for i in range(len(record))])
        assert (
            f"{VALUE_COL} column of the provided time series contains "
            f"null values at the end, or the input `train_end_date` is beyond the last timestamp available. "
            f"Setting `train_end_date` to the last timestamp with a non-null value "
            f"({result_train_end_date})." in all_warning_msg
        )
    ...
    with pytest.warns(UserWarning) as record:
        ...
        canonical_data_dict = get_canonical_data(df=df, train_end_date=train_end_date, regressor_cols=regressor_cols)
        all_warning_msg = " ".join([record[i].message.args[0] for i in range(len(record))])
        assert (
            f"{VALUE_COL} column of the provided time series contains null values at the end, "
            f"or the input `train_end_date` is beyond the last timestamp available." in all_warning_msg
    ...
    with pytest.warns(UserWarning) as record:
        ...
        canonical_data_dict = get_canonical_data(df=df, train_end_date=train_end_date, regressor_cols=None)
        all_warning_msg = " ".join([record[i].message.args[0] for i in range(len(record))])
        assert f"{VALUE_COL} column of the provided time series contains trailing NAs" in all_warning_msg
    ...
    with pytest.warns(UserWarning) as record:
        ...
        assert canonical_data_dict["last_date_for_val"] == datetime.datetime(2018, 1, 22)
        assert canonical_data_dict["last_date_for_reg"] == datetime.datetime(2018, 1, 28)
        assert ("The following columns are not available to use as regressors: " "['regressor4', 'regressor5']") in all_warning_msg
...
def test_infer_freq():
    ...
    for freq in freq_list:
        date_list = pd.date_range(start="2020-06-10", periods=100, freq=freq).tolist()
