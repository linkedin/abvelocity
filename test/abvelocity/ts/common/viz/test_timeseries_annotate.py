...
import pandas as pd
import pytest
from abvelocity.ts.common.constants import END_TIME_COL, START_TIME_COL
...
from abvelocity.ts.common.viz.timeseries_annotate import (
    add_multi_vrects,
    plot_anomalies_over_forecast_vs_actual,
    plot_event_periods_multi,
    plot_lines_markers,
    plot_overlay_anomalies_multi_metric,
...
def test_plt_annotate_series():
    ...
    df = pd.DataFrame(
        {"x": list(range(10)), "y": [1, 2, 3, 4, 5, 4, 3, 3, 2, 1], "label": ["cat", "cat", "cat", "dog", "dog", "dog", "horse", "cat", "cat", "cat"]}
    )
    fig = plt_annotate_series(df=df, x_col="x", value_col="y", label_col="label", annotate_labels=["cat", "dog"], keep_cols=None, title=None)["fig"]
    ...
    df = pd.DataFrame(
        {
            ...
            "forecast_lower": [0.5, 0.5, 1.5, 3.5, 4.5, 3, 3, 2, -0.5, -1],
        }
    )
...
def test_fill():
    ...
    with pytest.raises(ValueError, match="fill_cols must be a list of lists of strings."):
        plt_annotate_series(
            df=df,
            x_col="x",
            value_col="actual",
...
def test_plot_lines_markers():
    ...
    fig = plot_lines_markers(df=df, x_col="ts", line_cols=["line1", "line2"], marker_cols=["marker1", "marker2"], line_colors=None, marker_colors=None)
    ...
    fig = plot_lines_markers(
        df=df, x_col="ts", line_cols=["line1", "line2"], marker_cols=["marker1", "marker2"], line_colors=line_colors, marker_colors=marker_colors
    )

    assert len(fig.data) == 4
    assert fig.data[0].line.color == "rgba(99, 110, 250, 0.5)"
    assert fig.data[1].line.color == "rgba(239, 85, 59, 0.5)"
    assert fig.data[2].marker.color == "rgba(99, 110, 250, 1.0)"
    assert fig.data[3].marker.color == "rgba(239, 85, 59, 1.0)"
    ...
    with pytest.raises(ValueError, match="If `line_colors` is passed"):
        plot_lines_markers(
            df=df, x_col="ts", line_cols=["line1", "line2"], marker_cols=["marker1", "marker2"], line_colors=line_colors[:1], marker_colors=marker_colors
        )

    # At least one of `line_cols` or `marker_cols` or `band_cols`
    # must be provided (not None).
    with pytest.raises(ValueError, match="At least one of"):
        plot_lines_markers(df=df, x_col="ts", line_cols=None, marker_cols=None, band_cols=None, line_colors=None, marker_colors=None)


def test_plot_lines_markers_with_bands():
    """Tests ``plot_lines_markers`` with bands."""
    df = pd.DataFrame(
        {"x": range(4), "y": range(4), "z1": range(1, 5), "z2": range(-1, 3), "w": [(0, 1), (1, 3), (1, 5), (3, 5)], "u": [(2, 3), (3, 3), (4, 4), (6, 8)]}
    )
    ...
    assert fig.data[3].fillcolor == "rgba(99, 110, 250, 0.2)"
    assert fig.data[5].fillcolor == "rgba(239, 85, 59, 0.2)"
    assert fig.layout.title.text is None

    # Bands with custom colors and a title for the plot.
    fig = plot_lines_markers(
        df=df, x_col="x", line_cols=["y", "z1"], band_cols=["u", "w"], band_colors=["rgba(0, 255, 0, 0.2)", "rgba(255, 0, 0, 0.2)"], title="custom band colors"
    )
...
def test_plot_event_periods_multi():
    ...
    res = plot_event_periods_multi(
        periods_df=df, start_time_col="start_time", end_time_col="end_time", freq=None, grouping_col=None, min_timestamp=None, max_timestamp=None
    )
    ...
    res = plot_event_periods_multi(
        periods_df=df, start_time_col="start_time", end_time_col="end_time", freq=None, grouping_col="metric", min_timestamp=None, max_timestamp=None
    )
    ...
    assert set(labels_df.columns) == {"ts", "bookings_is_anomaly", "impressions_is_anomaly", "clicks_is_anomaly"}
    assert len(fig.data) == 3
    assert len(fig.data[0]["x"]) == 913
    assert len(ts) == 913
    assert min_timestamp == "2020-01-01"
    ...
    # Specifies ``freq``
    res = plot_event_periods_multi(
        periods_df=df, start_time_col="start_time", end_time_col="end_time", freq="min", grouping_col="metric", min_timestamp=None, max_timestamp=None
    )
    ...
    df = pd.DataFrame({"start_time": ["2020-01-03"], "end_time": ["2020-01-02"], "metric": ["impressions"]})
...
def test_add_multi_vrects():
    ...
    df = pd.DataFrame({"ts": ts, "y1": range(len(ts))})

    df["y2"] = (df["y1"] - 10) ** (1.5)

    fig = plot_lines_markers(df=df, x_col="ts", line_cols=["y1", "y2"], marker_cols=None, line_colors=None, marker_colors=None)
    ...
    assert grouping_color_dict == {"bookings": "rgba(99, 110, 250, 1.0)", "clicks": "rgba(239, 85, 59, 1.0)", "impressions": "rgba(0, 204, 150, 1.0)"}
    ...
    fig = res["fig"]
    grouping_color_dict = res["grouping_color_dict"]
    assert grouping_color_dict == {"metric": "rgba(99, 110, 250, 1.0)"}
    assert len(fig.data) == 2
    assert len(fig.layout.shapes) == 5
...
def test_plot_overlay_anomalies_multi_metric():
    ...
    assert set(is_anomaly_cols) == {"impressions_is_anomaly", "clicks_is_anomaly", "bookings_is_anomaly"}

    assert line_colors == ["rgba(99, 110, 250, 0.6)", "rgba(239, 85, 59, 0.6)", "rgba(0, 204, 150, 0.6)"]
...
def test_plot_precision_recall_curve():
    ...
    pr_df = pd.DataFrame({"precision": precision, "recall": recall, "key": 1})
    groups_df = pd.DataFrame({"groups": ["A", "B", "C"], "key": 1})
    df = pr_df.merge(groups_df, how="inner", on="key").reset_index(drop=True).drop("key", axis=1)
    # Generates the precision recall curve.
    fig = plot_precision_recall_curve(df=df, grouping_col="groups", precision_col="precision", recall_col="recall")
    ...
    fig = plot_precision_recall_curve(df=pr_df, grouping_col=None, precision_col="precision", recall_col="recall")
    ...
    with pytest.raises(ValueError, match=expected_match):
        plot_precision_recall_curve(df=df, grouping_col=None, precision_col="wrong_column", recall_col="recall")

    expected_match = "must contain"
    with pytest.raises(ValueError, match=expected_match):
        plot_precision_recall_curve(df=df, grouping_col=None, precision_col="precision", recall_col="wrong_column")

    expected_match = "is not found"
    with pytest.raises(ValueError, match=expected_match):
        plot_precision_recall_curve(df=df, grouping_col="wrong_column", precision_col="precision", recall_col="recall")
...
def test_plot_anomalies_over_forecast_vs_actual():
    ...
    df = pd.DataFrame({"ts": pd.date_range(start="2018-01-01", periods=size, freq="H"), "actual": np.random.normal(scale=10, size=size)})
