...
from abvelocity.ts.common.evaluation import ElementwiseEvaluationMetricEnum
from abvelocity.ts.common.python_utils import (
    apply_func_to_columns,
    assert_equal,
    dictionaries_values_to_lists,
    dictionary_values_to_lists,
    flatten_list,
    get_integer,
    get_pattern_cols,
    group_strs_with_regex_patterns,
    ignore_warnings,
    mutable_field,
    reorder_columns,
    split_offset_str,
...
def test_update_dictionary():
    ...
    merged_dict = update_dictionary(default_dict, overwrite_dict=overwrite_dict)
    ...
    update_dictionary(default_dict, overwrite_dict=overwrite_dict, allow_unknown_keys=False)
...
def test_update_dictionaries():
    ...
    default_dict = {"a": 1, "d": 4}
    overwrite_dict = {"a": 2, "c": 3}
    merged_dict = update_dictionaries(default_dict, overwrite_dicts=overwrite_dict)
    assert merged_dict == {"a": 2, "c": 3, "d": 4}

    # overwrite can be None or {}
    default_dict = {"a": 1, "b": 4}
    merged_dict = update_dictionaries(default_dict, overwrite_dicts=None)
    assert merged_dict == default_dict
    merged_dict = update_dictionaries(default_dict, overwrite_dicts={})
    assert merged_dict == default_dict

    default_dict = {"a": 1, "b": 4, "c": 5}
    overwrite_dict = {"a": 1, "b": 4}
    update_dictionaries(default_dict, overwrite_dicts=overwrite_dict, allow_unknown_keys=False)
    # overwrite cannot be a strict superset of default
    with pytest.raises(ValueError, match=r"Unexpected key\(s\) found"):
        overwrite_dict = {"a": 1, "b": 4, "d": 1}
        update_dictionaries(default_dict, overwrite_dicts=overwrite_dict, allow_unknown_keys=False)
    ...
    default_dict = {"a": 1, "d": 4}
    overwrite_dict = [{"a": 1, "b": 4}, None, {}, {"a": 1, "b": 4, "d": 1}]
    merged_dict = update_dictionaries(default_dict, overwrite_dicts=overwrite_dict, allow_unknown_keys=True)
    assert merged_dict == [{"a": 1, "b": 4, "d": 4}, default_dict, default_dict, {"a": 1, "b": 4, "d": 1}]
...
def test_get_pattern_cols():
    ...
    #
    # from abvelocity.ts.gk.sklearn.estimator.silverkite_estimator import \
    #     SilverkiteEstimator
    # from abvelocity.ts.gk.sklearn.estimator.testing_utils import \
    #     params_components
    # from abvelocity.ts.common.testing_utils import daily_data_reg
    ...
    feature_cols = [
        ...
        "Q('events_Christmas')[T.event]",
        "Q('events_Independence')[T.event]",
        "Q('events_Thanksgiving')[T.event]",
        "Q('events_Labor')[T.event]",
        "Q('events_Memorial')[T.event]",
        "Q('events_Veterans')[T.event]",
        ...
        "Q('events_Christmas_minus_1')[T.event]",
        "Q('events_Christmas_minus_2')[T.event]",
        "Q('events_Christmas_plus_1')[T.event]",
        "Q('events_Christmas_plus_2')[T.event]",
        "Q('events_Independence_minus_1')[T.event]",
        "Q('events_Independence_minus_2')[T.event]",
        "Q('events_Independence_plus_1')[T.event]",
        "Q('events_Independence_plus_2')[T.event]",
        ...
        "Q('events_Labor_minus_1')[T.event]",
        "Q('events_Labor_minus_2')[T.event]",
        "Q('events_Labor_plus_1')[T.event]",
        "Q('events_Labor_plus_2')[T.event]",
        "Q('events_Memorial_minus_1')[T.event]",
        "Q('events_Memorial_minus_2')[T.event]",
        "Q('events_Memorial_plus_1')[T.event]",
        "Q('events_Memorial_plus_2')[T.event]",
        "Q('events_Veterans_minus_1')[T.event]",
        "Q('events_Veterans_minus_2')[T.event]",
        "Q('events_Veterans_plus_1')[T.event]",
        "Q('events_Veterans_plus_2')[T.event]",
    ...
    expected_trend_cols = ["ct_sqrt", "ct1", "ct1:tod", "changepoint0_2018_01_01_00", "changepoint1_2019_01_02_16", "changepoint2_2019_01_03_00"]
...
def test_assert_equal_basic():
    ...
    assert_equal([1.234], [1.234])
    assert_equal(["str", 0, 1.234, 2.345], ["str", 0, 1.234, 2.345])
    ...
    # dictionary
    assert_equal({"df": df1}, {"df": df2}, check_dtype=False)

    assert_equal(
        actual={"a": {"a_a": {"a_a_a": df1}, "a_b": s1}, "b": "string", "c": 1},
        expected={"a": {"a_a": {"a_a_a": df2}, "a_b": s2}, "b": "string", "c": 1},
        check_dtype=False,
    )
    ...
    assert_equal([1, 2, 3, 4.12345], [1, 2, 3, 4.1233], rel=1e-2)

    assert_equal([3, 2, 1], [1, 2, 3], ignore_list_order=True)

    assert_equal([s1, df1, "df"], [s2, df1, "df"])

    assert_equal([s1, df1, "df"], [s2, df2, "df"], check_dtype=False)

    # negative cases
    with pytest.raises(AssertionError, match=r"Actual does not match expected\. Actual: 1\. Expected: 2\."):
        assert_equal([1], [2])

    with pytest.raises(AssertionError, match=r"Attribute \"dtype\" are different"):
        assert_equal(df1, df2)

    with pytest.raises(AssertionError, match=r"Actual does not match expected\. Actual: 4\.123\. Expected: 4\.124\."):
        assert_equal([1, 2, 3, 4.123], [1, 2, 3, 4.124], rel=1e-4)

    with pytest.raises(AssertionError, match=r"Not equal to tolerance rtol=1e-05, atol=0"):
        assert_equal(np.array([1, 2, 3, 4.12345]), np.array([1, 2, 3, 4.123]), rel=1e-5)

    with pytest.raises(AssertionError, match=r"Not equal to tolerance rtol=1e-07, atol=0"):
        assert_equal(np.array([1, 2, 3, 4.123]), np.array([1, 2, 3, 4.120]), rel=1e-7)

    assert_equal(np.array([1, 2, 3, 4.123]), np.array([1, 2, 3, 4.120]), rel=1e-2)

    with pytest.raises(AssertionError, match=r"Actual should be numeric, found None"):
        assert_equal(actual=None, expected=1)

    with pytest.raises(AssertionError, match=r"Actual should be None, found 1"):
        assert_equal(actual=1, expected=None)

    with pytest.raises(AssertionError, match=r"Actual should be a pandas Series"):
        assert_equal(n1, s1)

    with pytest.raises(AssertionError, match=r"Actual should be a numpy array"):
        assert_equal(s1, n1)
    ...
    with pytest.raises(AssertionError, match=r"Error at dictionary location: dict\['a'\]\['a'\]\['a'\].\n" r"Actual should be a pandas DataFrame, found df1\."):
        assert_equal(
            actual={"a": {"a": {"a": "df1"}, "b": s1}, "b": "string", "c": 1}, expected={"a": {"a": {"a": df2}, "b": s2}, "b": "string", "c": 1}  # mismatch
        )

    with pytest.raises(AssertionError, match=r"Error at dictionary location: dict\['d'\].\n" r"Actual should be a pandas DataFrame, found df1\."):
        assert_equal(
            actual=[1, {"a": {"a": {"a": df1}, "b": s1}, "b": "string", "c": 1, "d": "df1"}],  # mismatch
            expected=[1, {"a": {"a": {"a": df1}, "b": s2}, "b": "string", "c": 1, "d": df1}],
        )


def test_assert_equal_ignore_keys():
    """Tests assert_equal ignore_keys parameter"""
    actual = {"a": {"a": {"a": 1}, "b": 2, "c": "3"}, "b": 3, "c": [{"a": 4}]}  # matches expected
    expected = {"a": {"a": {"a": "1"}, "b": "2", "c": "3"}, "b": "3", "c": [{"a": "4"}]}

    # ignores keys at the first level
    assert_equal(actual, expected, ignore_keys={"a": None, "b": None, "c": None})
    # ignores keys at the second level.
    # the value can be anything that's not a dict
    assert_equal(actual, expected, ignore_keys={"a": {"a": False, "b": True}, "b": None, "c": "not_a_dict"})
    # ignores keys at the third level
    assert_equal(actual, expected, ignore_keys={"a": {"a": {"a": None}, "b": None}, "b": None, "c": None})
    with pytest.raises(AssertionError, match=r"Error at dictionary location: dict\['c'\]\['a'\]"):
        ...
        assert_equal(actual, expected, ignore_keys={"a": {"a": {"a": None}, "b": None}, "b": None})

    with pytest.raises(AssertionError, match=r"Error at dictionary location: dict\['a'\]\['a'\]\['a'\]"):
        # ["a"]["a"]["b"] is ignored, but not ["a"]["a"]["a"]
        assert_equal(actual, expected, ignore_keys={"a": {"a": {"b": None}, "b": None}, "b": None, "c": None})

    with pytest.raises(AssertionError, match=r"Error at dictionary location: dict\['a'\]\['b'\]"):
        # ["a"]["a"]["a"] is ignored, but not ["a"]["b"]
        assert_equal(actual, expected, ignore_keys={"a": {"a": {"a": None}}, "b": None, "c": None})

    with pytest.raises(AssertionError, match=r"Error at dictionary location: dict\['c'\]\['a'\]"):
        with pytest.warns(Warning) as record:
            # can't ignore keys within in a list (because there are no keys to ignore)
            assert_equal(actual, expected, ignore_keys={"a": {"a": {"a": None}, "b": None}, "b": None, "c": {"a": None}})
            assert (
                r"At dictionary location: dict['c']. `ignore_keys` is {'a': None}, " r"but found a list. No keys will be ignored" in record[0].message.args[0]
            )
...
def test_dictionary_values_to_lists():
    ...
    exponential_distribution = scipy.stats.expon(scale=0.1)
    hyperparameter_grid = {
        "param1": [],
        "param2": [None],
    ...
    hyperparameters_list_type = set({f"param{i+1}" for i in range(12)})
    result = dictionary_values_to_lists(hyperparameter_grid, hyperparameters_list_type=hyperparameters_list_type)
    ...
    hyperparameters_list_type = {k: [None] for k in hyperparameters_list_type}
    ...
    # Checks exception
    with pytest.raises(ValueError, match=r"The value for param1 must be a list, tuple, or one of \[None\], found {'k': 'v'}."):
        hyperparameter_grid = {"param1": {"k": "v"}}
        dictionary_values_to_lists(hyperparameter_grid, hyperparameters_list_type={"param1"})
...
def test_dictionaries_values_to_lists():
    ...
    original_grid = hyperparameter_grid.copy()
    result = dictionaries_values_to_lists(hyperparameter_grid, hyperparameters_list_type={"param2", "param4"})
    expected_grid = {
        "param1": [],
        "param2": [[]],
        "param3": [None],
    ...
    result = dictionaries_values_to_lists(hyperparameter_grids, hyperparameters_list_type={"param2", "param4"})
...
def test_unique_in_list():
    ...
    assert unique_in_list([0, 1, 2, [3]], ignored_elements=(1, 3)) == [0, 2]
    assert unique_in_list([0, 0, [None], [[None]], [[[None]]]], ignored_elements=(None,)) == [0]
    assert (
        unique_in_list(
            [0, 0, [None], [[None]]],
            ignored_elements=(
...
def test_apply_func_to_columns():
    ...
    fn = apply_func_to_columns(row_func=ElementwiseEvaluationMetricEnum.Residual.get_metric_func(), cols=["a", "b"])
    assert fn(row) == row["a"] - row["b"]

    fn = apply_func_to_columns(row_func=ElementwiseEvaluationMetricEnum.Residual.get_metric_func(), cols=["c", "b"])
    ...
    row = [4.0, 8.0, 5.0]
    fn = apply_func_to_columns(row_func=ElementwiseEvaluationMetricEnum.Residual.get_metric_func(), cols=[2, 1])
...
def test_ignore_warnings(recwarn):
    ...
    assert len(recwarn) == 0  # `recwarn` records warnings, so its length should be 0`
...
def test_group_strs_with_regex_patterns():
    ...
    result = group_strs_with_regex_patterns(strings=strings, regex_patterns=regex_patterns)

    assert result == {"str_groups": [["sd2"], ["sd", "sd1"], ["urr"], ["rr"]], "remainder": ["uu"]}
    ...
    assert result == {"str_groups": [["11", "12"], ["sd", "sd1", "sd2"], ["urr"], ["rr"]], "remainder": ["uu"]}
    ...
    assert result == {"str_groups": [["sd1", "sd2", "11", "12"], ["sd"], ["urr"], ["rr"]], "remainder": ["uu"]}
    ...
    assert result == {"str_groups": [["sd2"], ["sd1", "sd22"]], "remainder": ["sd", "rr", "urr", "uu", "11", "12"]}
