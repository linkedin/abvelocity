...
def test_gen_moving_timeseries_forecast():
    ...
    compare_df = gen_moving_timeseries_forecast(
        df=df, time_col="ts", value_col="y", train_forecast_func=train_forecast_func, train_move_ahead=10, forecast_horizon=7, min_training_end_point=100
    )["compare_df"]
    ...
    compare_df = gen_moving_timeseries_forecast(
        ...
        max_forecast_end_timestamp="2018-08-01",
    )["compare_df"]

    assert list(compare_df.head(5)["y_hat"].round(1).values) == [-3.5] * 5

    expected_match = "No reasonable train test period is found for validation"
    with pytest.raises(ValueError, match=expected_match):
        gen_moving_timeseries_forecast(
            df=df, time_col="ts", value_col="y", train_forecast_func=train_forecast_func, train_move_ahead=10, forecast_horizon=700, min_training_end_point=1000
        )["compare_df"]
...
def test_gen_moving_timeseries_forecast_with_regressors():
    ...
    def train_forecast_func(df, value_col, time_col, forecast_horizon, new_external_regressor_df):
        # Simply get last observed value and offer as forecast
...
def test_gen_moving_timeseries_forecast_extra_columns():
    ...
    compare_df = gen_moving_timeseries_forecast(
        ...
        forecast_keep_cols=["forecast_dummy"],
    )["compare_df"]
