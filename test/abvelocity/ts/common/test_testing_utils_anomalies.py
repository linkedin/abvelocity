...
from abvelocity.ts.common.testing_utils_anomalies import (
    calc_quantiles_simulated_df,
    contaminate_df_with_anomalies,
...
def test_generate_anomaly_blocks():
    res = generate_anomaly_blocks(timeseries_length=100, block_number=5, mean_block_size=5)
...
def test_contaminate_df_with_anomalies():
    # data size
    n = 2000
    res = generate_df_for_tests(freq="1D", periods=n, train_start_date=datetime.datetime(2018, 7, 1))
    df = res["df"]

    res = generate_anomaly_blocks(timeseries_length=n, block_number=20, mean_block_size=5)
...
def test_calc_quantiles_simulated_df():
    ...
    quantiles_df = calc_quantiles_simulated_df(sim_df_func=sim_df_func, quantiles=[0.25, 0.75], simulation_num=50)
    ...
    quantiles_df = calc_quantiles_simulated_df(sim_df_func=sim_df_func, quantiles=[0.25, 0.75], simulation_num=50, x=1)

    assert quantiles_df.shape == (100, 2)


def test_generate_df_with_anomalies_sim_based():
    res = generate_df_with_anomalies_sim_based(freq="5min", periods=24 * 12 * 10, block_number=10, mean_block_size=5)
    df = res["df"]
