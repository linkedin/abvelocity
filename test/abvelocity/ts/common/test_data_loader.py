...
def test_get_df():
    ...
    data_path = dl.get_data_home(data_dir=None, data_sub_dir="daily")
    file_names = dl.get_data_names(data_path=data_path)
    with pytest.raises(ValueError, match=rf"Given file 'parking.csv' \(or .csv.xz\) is not found in '{data_path}'. Available datasets: \{file_names}\."):
...
def test_load_hourly_beijing_pm():
    ...
    assert list(df.columns) == [TIME_COL, "year", "month", "day", "hour", "pm", "dewp", "temp", "pres", "cbwd", "iws", "is", "ir"]
...
def test_load_hierarchical():
    ...
    assert_equal(actuals["00"], actuals["10"] + actuals["11"], check_names=False)
    assert_equal(actuals["10"], actuals["20"] + actuals["21"] + actuals["22"], check_names=False)
    assert_equal(actuals["11"], actuals["23"] + actuals["24"], check_names=False)
...
def test_load_data():
    ...
    data_names = ("daily_peyton_manning", "hourly_bikesharing", "hourly_beijing_pm", "daily_hierarchical_actuals", "daily_hierarchical_forecasts")
    ...
    with pytest.raises(ValueError, match=rf"Input data name '{data_name}' is not recognized. " rf"Must be one of \{data_inventory}\."):
